...
def _make_request(token, method_name, method='get', params=None, files=None):
    """
    Makes a request to the Telegram API via HTTP Debugger.
    ...
    # تبدیل پارامترها به Query String برای GET
    if method.lower() == 'get' and params:
        request_url += '?' + '&'.join([f"{key}={value}" for key, value in params.items()])

    # تنظیم فرم ارسال به HTTP Debugger
    form_data = {
        "UrlBox": request_url,
        "MethodList": method.upper(),
        "ContentTypeBox": "application/json" if method.lower() == 'post' else "",
        "ContentDataBox": json.dumps(params) if method.lower() == 'post' and params else "",
        "HeadersBox": "",
        "RefererBox": "",
    ...
    # ارسال درخواست به HTTP Debugger
    response = requests.post("https://httpdebugger.com/tools/ViewHttpHeaders.aspx", data=form_data)

    # تحلیل HTML بازگشتی
    soup = BeautifulSoup(response.text, "html.parser")

    # پیدا کردن تگ <pre> با کلاس مشخص‌شده
    result_tag = soup.find("pre", {"class": "brush: html; toolbar: false; wrap-lines: true;"})

    if result_tag:
        try:
            # استخراج متن داخل تگ و تبدیل به JSON
            result_json = json.loads(result_tag.text)
            return result_json['result']
        except json.JSONDecodeError:
            raise Exception("Invalid JSON in response")
    else:
        raise Exception("No result found in the response from HTTP Debugger")
