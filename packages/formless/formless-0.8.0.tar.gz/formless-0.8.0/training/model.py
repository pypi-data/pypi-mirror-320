...
from utils import DEFAULT_QUESTION, DEFAULT_SYSTEM_PROMPT
...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        Returns:
        -------
            [`~utils.ModelOutput`] or [`~generation.Qwen2VLCausalLMOutputWithPast`]:
                If `return_dict` is `True`, [`~generation.Qwen2VLCausalLMOutputWithPast`] (`past_key_values`,
                `decoder_hidden_states`, `decoder_attentions`, `cross_attentions`, `encoder_last_hidden_state`,
                `encoder_hidden_states`, `encoder_attentions`) is returned, otherwise a tuple of
                `[lm_logits, past_key_values, decoder_hidden_states, decoder_attentions, cross_attentions,
                encoder_last_hidden_state, encoder_hidden_states, encoder_attentions]` is returned.
...
if __name__ == "__main__":
    ...
    # Generate
    generate_ids = model.generate(torch.tensor(inputs.input_ids), max_length=4096)
    print(
        processor.tokenizer.batch_decode(generate_ids, skip_special_tokens=True, clean_up_tokenization_spaces=False)[0]
    )
