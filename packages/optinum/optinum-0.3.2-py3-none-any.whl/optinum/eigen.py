...
def rotation_method(A: np.array, eps: float, max_iterations: int = 1000):
    # Инициализируем матрицу поворота как единичную
    n = A.shape[0]
    J_total = np.eye(n)
    
    for iteration in range(max_iterations):
        # Находим индексы p, q максимального элемента выше диагонали
        def find_max(A):
            A_upper = abs(np.triu(A))
            np.fill_diagonal(A_upper, 0)
            return np.unravel_index(A_upper.argmax(), A_upper.shape)
        
        # p, q - индексы максимального элемента выше диагонали A
        p, q = find_max(A)
        
        if A[p, p] != A[q, q]:
            # Найдем Ф: Ф = tg(2*theta)
            F = 2 * A[p, q] / (A[p, p] - A[q, q])
            # Выразим theta
            theta = 1/2 * np.arctan(F)
        else:
            theta = np.pi / 4
        
        # Инициализируем матрицу-поворота J как единичную
        J = np.eye(n)
        # Заполним J в соответствии с найденными индексами p, q
        J[p, p] = np.cos(theta)
        J[p, q] = -np.sin(theta)
        J[q, p] = np.sin(theta)
        J[q, q] = np.cos(theta)

        # Обновляем A как J^T * A * J
        A = J.T @ A @ J
        # Множим общую матрицу поворотов
        J_total = J_total @ J

        if np.all(abs(A[~np.eye(A.shape[0], dtype=bool)]) < eps):
            print(f"Сходимость достигнута на итерации {iteration}")
            break
    
    else:
        print("Алгоритм не сошелся за максимальное число итераций.")
    
    # Возвращаем собственные значения и векторы
    return np.diag(A), J_total