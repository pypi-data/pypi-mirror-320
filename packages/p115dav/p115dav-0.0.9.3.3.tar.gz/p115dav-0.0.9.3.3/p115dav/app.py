...
""")
    ...
    class FileResource(DavPathBase, DAVNonCollection):
        ...
        def strm_data(self, /) -> bytes:
            return bytes(self.origin + self.url, "utf-8")
        ...
        def url(self, /) -> str:
            ...
            if share_code := attr.get("share_code"):
                return f"/<share/<url/{name}?share_code={share_code}&id={attr['id']}"
            else:
                return f"/<url/{name}?pickcode={attr['pickcode']}&id={attr['id']}&sha1={attr['sha1']}"
