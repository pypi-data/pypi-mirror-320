...
def timeout_handler(signum, frame):
    logger.error(f"Timeout triggered - Signal: {signum}, Frame: {frame}")
    logger.error(f"Current request: {request.path} [{request.method}]")
...
def setup_logging():
    ...
    detailed_formatter = logging.Formatter(
        '%(asctime)s - %(name)s - %(levelname)s - %(process)d - %(threadName)s - %(pathname)s:%(lineno)d - %(message)s'
    )
...
def create_app():
    ...
    def setup_timeout():
        client_info = {
            ...
            'api_key': request.headers.get('X-API-Key', '')[-8:] if request.headers.get('X-API-Key') else 'none'  # Log last 8 chars only
        }
        logger.info(f"Incoming request from client: {client_info}")
        
        logger.debug(f"Setting up timeout for request: {request.path} [{request.method}]")
        signal.signal(signal.SIGALRM, timeout_handler)
        signal.alarm(120)  # 120 seconds timeout

    @app.after_request
    def clear_timeout(response):
        logger.debug(f"Clearing timeout for request: {request.path} [{request.method}]")
        signal.alarm(0)
        return response
