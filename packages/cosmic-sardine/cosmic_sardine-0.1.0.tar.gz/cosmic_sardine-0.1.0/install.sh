...
if [[ "${SKIP_INSTALL}" == "NO" ]]; then
    ...
    command -v $DEFAULT_CONAN || $DEFAULT_PYTHON -m pip install conan
...
if [[ "${SKIP_CONFIGURE}" == "NO" ]]; then
    ...
    $DEFAULT_CONAN profile detect --force

    conan_home=$($DEFAULT_CONAN config home)
...
if [[ "${SKIP_POPULATE}" == "NO" ]]; then
    ...
    $DEFAULT_CONAN remote add -t local-recipes-index $INDEX_NAME $INDEX_LOCATION --force -verror
