...
int decodeReferenceBlock(unsigned int* data,unsigned int* end){
	...
	//Cas ID_LOW, FS est cod� sur 5 bits au lieu de 4
	if (id == ID_LOW)
	{
...
int uncompress(unsigned char* compressed,int compressedLen,unsigned int* data,int nbInput){
	...
	//Decodage du premier bloc (bloc de r�f�rence)
	...
	try {
		...
		char buffer[128];
		snprintf(buffer, 128, "%s : block %i/%i (size: %i/%i)", e.what(), i, nbBlock, ((size_t)s-(size_t)data)/2,nbInput*2);
...
static void fillDataBuffer(){
	if (data_bits <= 16)
	{
		...
		if ((size_t)input_ptr > (size_t)input_ptr_end) {
			char buffer[36];
			snprintf(buffer,36, "Illegal access to %X > %X", (size_t)input_ptr, (size_t)input_ptr_end);
			throw std::runtime_error(buffer);
		}
#endif
