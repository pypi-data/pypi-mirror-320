...
LS_FILE_DEF_DIR = TemporaryDirectory().name
...
class Lstable:
    ...
    def get_lstable_file(target_dir=None, overwrite=False, url=URL):
        ...
        try:
            ...
            logger.exception("Cannot download CDFLeapSeconds.txt")
            logger.debug(e)
        else:
            return True
...
def download_data(url, encoding="utf-8", target_file=None, binary=False):
