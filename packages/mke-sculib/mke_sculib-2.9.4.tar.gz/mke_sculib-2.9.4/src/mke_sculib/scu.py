...
if __name__ == '__main__':
    ...
    cnt = 0
    for df in dish.get_from_scu_influx_batched(Time('2024-12-23T00:00:00'), Time('2024-12-23T01:00:00'), sample_rate=50, batch_size=100):
        cnt += 1
        dt = pd.Series(df.index).diff().dt.total_seconds().abs()
        print(cnt, df.index.min(), df.index.max(), df.shape, dt.mean(), dt.min(), dt.max())
    print('done!')
