    ...
    def train(self, epochs = None):
        ...
        for idx_epoch in range(epochs):
            ...
            for idx_iter ,(img , mask) in enumerate(tbar):
                ...
                tbar.set_description('Train Epoch {}/{}, loss {}, lr {:.6f}/{:.6f}-{:.6f}'.format(idx_epoch + 1, epochs, loss.item(), self.optimizer.param_groups[0]['lr'], self.config.lr, self.config.lr_min))
    ...
    def test(self):
        ...
        with torch.no_grad():
            for idx_iter ,(img , mask) in enumerate(tbar):
                ...
                tbar.set_description('Test Epoch {}/{}, miou {:.6f}/{:.6f}, F1 {:.6f}/{:.6f}'.format(self.epoch + 1, self.config.epochs, miou, self.best_miou, fmeasure, self.fmeasure))
            ...
            with open(self.file_path, 'a+') as f:
                # 写入一些记录
                f.write('Test Epoch {}/{}, miou {:.6f}/{:.6f}, F1 {:.6f}/{:.6f}\n'.format(self.epoch + 1, self.config.epochs, miou, self.best_miou, fmeasure, self.fmeasure))
                # 刷新缓冲区，确保写入的数据立即保存到文件
                f.flush()
