...
def calculate_subscore_for_sentence(analysis_type, decisions, success_categories):
    ...
    if len(decisions) > 0:
        if analysis_type == "tenses":
            subscore = sum(decisions.count(key) for key in success_categories) / len(decisions)
...
def analyze_the_current_sentence(sentence, context_window, analysis_type, success_categories, terms_to_check, query_count):
    ...
    subscore = calculate_subscore_for_sentence(analysis_type, decisions, success_categories)
...
def check_filter_for_analysis(sentence, context_window, analysis_type, filter, success_categories, filter_cutoff, terms_to_check, query_count):
    ...
    if filter:
        ...
        if success not in ['', 'nan', None]:
            ...
            if success >= filter_cutoff:
                decisions_count, subscore = analyze_the_current_sentence(sentence, context_window, analysis_type, success_categories, terms_to_check, query_count)
    else:
        decisions_count, subscore = analyze_the_current_sentence(sentence, context_window, analysis_type, success_categories, terms_to_check, query_count)
...
def document_analysis(log_file, filter, success_categories, filter_cutoff, terms_to_check):
    with open(log_file, 'a', newline='') as log:
        writer = csv.writer(log)
        writer.writerow(['filter', 'success_categories', 'filter_cutoff', 'terms_to_check'])
        writer.writerow([filter, success_categories, filter_cutoff, terms_to_check])

def analyze_all_sentences(sentences, save_name, analysis_type, context_length=0, terms_to_check=[], success_categories=None, filter=None, filter_cutoff=0, query_count=3, directory = '.'):
    ...
    print(f"Starting '{analysis_type}' analysis with {success_categories} success categories, '{filter}' filter (filter cutoff: {filter_cutoff}) at document {start_document_id}, sentence {start_sentence_id}")
    ...
    for sentence in tqdm(sentences):
        ...
        decisions_count, subscore = check_filter_for_analysis(sentence, context_window, analysis_type, filter, success_categories, filter_cutoff, terms_to_check, query_count)
    ...
    document_analysis(log_file, filter, success_categories, filter_cutoff, terms_to_check)
