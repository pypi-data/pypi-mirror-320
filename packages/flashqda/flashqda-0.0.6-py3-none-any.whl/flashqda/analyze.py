...
def calculate_subscore_for_sentence(analysis_type, decisions, subscore_keys):

    """For the current sentence and the current type of analysis, calculate a subscore."""

    if subscore_keys is None:
        subscore_keys = []    

    if len(decisions) > 0:
        if analysis_type == "tenses":
            subscore = sum(decisions.count(key) for key in subscore_keys) / len(decisions)
...
def analyze_the_current_sentence(sentence, context_window, analysis_type, subscore_keys, terms_to_check, query_count):
    ...
    subscore = calculate_subscore_for_sentence(analysis_type, decisions, subscore_keys)
...
def check_filter_for_analysis(sentence, context_window, analysis_type, filter, subscore_keys, filter_cutoff, terms_to_check, query_count):
    ...
    if filter:
        ...
        if success not in ['', 'nan', None]:
            ...
            if success >= filter_cutoff:
                decisions_count, subscore = analyze_the_current_sentence(sentence, context_window, analysis_type, subscore_keys, terms_to_check, query_count)
    else:
        decisions_count, subscore = analyze_the_current_sentence(sentence, context_window, analysis_type, subscore_keys, terms_to_check, query_count)
...
def document_analysis(log_file, filter, subscore_keys, filter_cutoff, terms_to_check):
    with open(log_file, 'a', newline='') as log:
        writer = csv.writer(log)
        writer.writerow(['filter', 'subscore_keys', 'filter_cutoff', 'terms_to_check'])
        writer.writerow([filter, subscore_keys, filter_cutoff, terms_to_check])

def analyze_all_sentences(sentences, save_name, analysis_type, context_length=0, terms_to_check=[], filter=None, subscore_keys=None, filter_cutoff=0, query_count=3, directory = '.'):
    ...
    print(f"Starting '{analysis_type}' analysis with {subscore_keys} subscore keys, '{filter}' filter (filter cutoff: {filter_cutoff}) at document {start_document_id}, sentence {start_sentence_id}")
    ...
    for sentence in tqdm(sentences):
        ...
        decisions_count, subscore = check_filter_for_analysis(sentence, context_window, analysis_type, filter, subscore_keys, filter_cutoff, terms_to_check, query_count)
    ...
    document_analysis(log_file, filter, subscore_keys, filter_cutoff, terms_to_check)
