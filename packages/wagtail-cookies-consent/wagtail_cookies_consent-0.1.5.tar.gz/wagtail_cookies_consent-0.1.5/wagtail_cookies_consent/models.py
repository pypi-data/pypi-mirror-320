...
class CookieGroup(models.Model):
    ...
    name = models.CharField(_("Name EN"), max_length=100, blank=True)
    name_ar = models.CharField(_("Name AR"), max_length=250)
    ...
    @property
    def get_translation_of_name(self):
        if get_language() == 'ar':
            return self.name_ar
        return self.name
        
    @property
    def get_translation_of_description(self):
        if get_language() == 'ar':
...
class Cookie(models.Model):
    ...
    name = models.CharField(_("Name EN"), max_length=250)
    name_ar = models.CharField(_("Name AR"), max_length=250)
    description_en = models.TextField(_("Description EN"), blank=True)
    description_ar = models.TextField(_("Description AR"), blank=True)

    path = models.TextField(_("Path"),help_text=_('The url path of the cookies police'), blank=True, default="/")
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=("cookiegroup", "name_ar",'name', "domain"),
                name="natural_key",
            ),
        ]
        ordering = ["-created"]
...
class CookiesBanner(models.Model):
    ...
    def __str__(self):
        return f" {_('Cookies banner')}-{self.id}"
    ...
    @property
    def get_translation_of_title(self):
        if get_language() == 'ar':
