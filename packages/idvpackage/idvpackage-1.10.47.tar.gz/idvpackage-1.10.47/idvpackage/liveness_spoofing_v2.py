...
def process_frame(frame, models, image_cropper):
    ...
    # Resize to a standard size while preserving aspect ratio
    target_height = 640
    ...
    if not check_image(frame):
        return None  # Don't mark as spoof just for proportion/size issues
        
    bbox = get_bbox(frame)
    if not bbox:
        return "SPOOF"  # Consider it a spoof if no face detected
    ...
    # Adjust thresholds to be more balanced
    label_text = "LIVE" if (label == 1 and value > 0.55) or (label == 2 and value < 0.45) else "SPOOF"
    # print(f"\nLabel: {label}, Value: {value}, Label Text: {label_text}\n")
    return label_text
...
def test(video_path):
    ...
    # Make the decision criteria more balanced
    spoof_count = all_predictions.count('SPOOF')
    # print(f'spoof_count: {spoof_count}')
    total_frames = len(all_predictions)
    # print(f'total_frames: {total_frames}')
    
    # Require at least 40% of frames to be marked as spoof
    if spoof_count / total_frames >= 0.4:
        return 'consider'
