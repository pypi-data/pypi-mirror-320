...
class Response:
    ...
    def __init__(self, client = None, name = ""):
        if client:
            self.GetResponseInfo(client, name)

    def GetResponseInfo(self, client, name):
        ...
        self.__name = result['payload'].get('name', self.__name)
        self.__units = result['payload'].get('units', '')
        self.__values = tuple(result['payload'].get('values', []))
        self.__is_equation_only = result['payload'].get('is_equation_only',False)
