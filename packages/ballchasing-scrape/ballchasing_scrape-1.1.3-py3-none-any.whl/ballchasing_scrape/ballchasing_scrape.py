...
def post_replay(file_path, authkey, visibility="public", groupid=""):
    ...
    #Upload request
    res = requests.post(url,headers=head,data=param,files=file)
...
def post_group(name, authkey, parent_id="",p_id="by-id",t_id="by-distinct-players"):
    ...
    #Upload request
    res = requests.post(url,headers=head,json=param)
...
def player_group_stats_parser(scrape,loc="", by_group=True, groupurl="",authkey="",param={}):
    import pandas as pd
