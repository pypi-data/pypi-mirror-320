...
class TestBiasMetrics:
    ...
    def test_tpr_gap_torch(self, sample_data_torch):
        ...
        bias = calculate_bias_metric_torch(BiasMetrics.TPR_GAP, y_pred, y_true, protected_attribute)
    ...
    def test_fpr_gap_torch(self, sample_data_torch):
        y_pred, y_true, protected_attribute = sample_data_torch
        bias = calculate_bias_metric_torch(BiasMetrics.FPR_GAP, y_pred, y_true, protected_attribute)
    ...
    def test_tnr_gap_torch(self, sample_data_torch):
        y_pred, y_true, protected_attribute = sample_data_torch
        bias = calculate_bias_metric_torch(BiasMetrics.TNR_GAP, y_pred, y_true, protected_attribute)
        assert abs(bias.item() - 1.0) < 1e-6

    def test_fnr_gap_torch(self, sample_data_torch):
        y_pred, y_true, protected_attribute = sample_data_torch
        bias = calculate_bias_metric_torch(BiasMetrics.FNR_GAP, y_pred, y_true, protected_attribute)
        assert abs(bias.item() - 0.5) < 1e-6

    def test_eo_gap_torch(self, sample_data_torch):
        y_pred, y_true, protected_attribute = sample_data_torch
        bias = calculate_bias_metric_torch(BiasMetrics.EO_GAP, y_pred, y_true, protected_attribute)
        assert abs(bias.item() - 0.75) < 1e-6

    def test_dp_gap_torch(self, sample_data_torch):
        y_pred, y_true, protected_attribute = sample_data_torch
        bias = calculate_bias_metric_torch(BiasMetrics.DP_GAP, y_pred, y_true, protected_attribute)
        assert abs(bias.item() - 0.5) < 1e-6

    # Numpy version tests
    def test_tpr_gap_np(self, sample_data_np):
        y_pred, y_true, protected_attribute = sample_data_np
        bias = calculate_bias_metric_np(BiasMetrics.TPR_GAP, y_pred, y_true, protected_attribute)
        assert abs(bias - 0.5) < 1e-6

    def test_invalid_metric(self, sample_data_torch):
        y_pred, y_true, protected_attribute = sample_data_torch
        with pytest.raises(ValueError):
            calculate_bias_metric_torch("INVALID_METRIC", y_pred, y_true, protected_attribute)
