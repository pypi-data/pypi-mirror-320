    ...
    try:
        with pkg_resources.path(package, filename) as file_path:
            ...
            if not file_path.exists():
                raise FileNotFoundError(f"Файл {filename} не найден в пакете {package}.")

            # Создаем объект PDF и отображаем его
            pdf = PDF(filename=str(file_path))
            display(pdf)
