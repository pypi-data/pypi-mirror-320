...
def compute_clips_for_video_fps(num_frame_vid: int, clip_length: int, step: int, fps_video: float, target_frame_rate: int) -> np.ndarray:
    """
    ...
    def __init__(self, name, guid, device, inputs, params, cparams, node_attr, fw_params):
        ...
        if self.fixed_clip_mode == False:
            ...
            self.video_clips = VideoClipsFps(
                self.frames_per_clip, self.step_between_clips, self.target_frame_rate, self.vid_list)
