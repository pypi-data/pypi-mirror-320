...
# If g_extra_frame_stride is True, last Frame in clip will require extra frames needed by stride as well
...
def compute_clips_for_video_stride(num_frame_vid: np.uint32, clip_length: int, step: int, start_index: int, stride: int) -> np.ndarray:
    """
...
def get_clip_len(num_frame_vid: np.uint32, clip_length: int, step: int, start_index: int, stride: int) -> int:
    """
...
class VideoClipsStride:
    def __init__(self, frames: int, step: int, vid_list: np.ndarray, start_frame_index: int, stride: int) -> None:
        """
    ...
    def compute_clips_stride(self):
        ...
        self.ref_vid_clip = compute_clips_for_video_stride(
            max_frame, self.frames_per_clip, self.step_between_clips, self.start_frame_index, self.stride)
...
class read_video_from_dir_gen(MediaReaderNode):
    ...
    def __init__(self, name, guid, device, inputs, params, cparams, node_attr, fw_params):
        ...
        if (self.seed == None):
            # seed only needed in case of RANDOM_SAMPLER or CONTIGUOUS_RANDOM_SAMPLER, not needed for UNIFORM_SAMPLER or CONTIGUOUS_SAMPLER
            if (self.num_slices > 1):
                if (self.sampler == cs.RANDOM_SAMPLER) or (self.sampler == cs.CONTIGUOUS_RANDOM_SAMPLER):
        ...
        print("video reader sampler: {} clips_per_video: {} frames_per_clip: {} stride: {} step_between_clips: {} start_frame_index: {}".format(
            self.sampler, self.clips_per_video, self.frames_per_clip, self.stride, self.frames_between_clips, self.start_frame_index))  # self.last_batch_strategy
        self.batch_size = fw_params.batch_size

        self.video_clips_stride = VideoClipsStride(
            self.frames_per_clip, self.frames_between_clips, self.vid_list, self.start_frame_index, self.stride)

        self.ref_vid_clip, self.video_clips_list = self.video_clips_stride.get_video_clip_list()
        self.shuffle = False
        if self.sampler == cs.RANDOM_SAMPLER:
            assert self.seed != None, "seed not set for RANDOM_SAMPLER"
            ...
            self.sampler_inst = ContiguousSampler(
                self.video_clips_list, self.clips_per_video)
            self.shuffle = True
            assert self.seed != None, "seed not set for ContiguousSampler"
        else:
        ...
        print("num video: {} num clips: {} batch size: {} sliced, rounded clips: {} num batches: {}".format(len(self.vid_list), sampler_clip_length,
              self.batch_size, self.shuffler.GetNumIterableEle(), self.num_batches_slice))
