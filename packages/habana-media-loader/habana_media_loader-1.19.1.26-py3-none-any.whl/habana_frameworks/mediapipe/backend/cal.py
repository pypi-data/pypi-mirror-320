...
class graph_handler():
    ...
    def __create_c_dummy_nodes__(self, op, params, device, node_type, exec_type):
        opnode = mpn.OpNode(op.name, node_type, device, node_type, exec_type)
        if (op.guid == None):
            raise ValueError("No CPU Guid found for ", op.name)
        opnode.guid = op.guid
        if len(op.output_tensors) > 1:
    ...
    def __init__(self, batch_size, const_nodes, reader_nodes, cpu_nodes, decoder_nodes, hpu_nodes, dummy_nodes, outputs, fw_type, proxy):
        ...
        for r in self.__reader_nodes__:
            if (isinstance(r, MediaPyReaderNode)):
                self.__create_c_py_reader_nodes__(
                    r, r.params, mpn.Device_t.DEVICE_CPU, mpn.NodeType_t.NODE_READER, mpn.ExecType_t.EXEC_BATCHED)
            else:
                self.__create_c_nodes__(
                    r, r.params, mpn.Device_t.DEVICE_CPU, mpn.NodeType_t.NODE_READER, mpn.ExecType_t.EXEC_NON_BATCHED)
        for op in self.__const_nodes__:
            self.__create_c_nodes__(
                op, op.params, mpn.Device_t.DEVICE_CPU, mpn.NodeType_t.NODE_CONSTANT, mpn.ExecType_t.EXEC_BATCHED)
        for op in self.__dummy_nodes__:
            self.__create_c_dummy_nodes__(
                op, op.params, mpn.Device_t.DEVICE_CPU, mpn.NodeType_t.NODE_DUMMY, mpn.ExecType_t.EXEC_NON_BATCHED)
        for op in self.__cpu_nodes__:
            if (isinstance(op, MediaFuncDataNode)):
                self.__create_c_py_func_nodes__(
                    op, op.params, mpn.Device_t.DEVICE_CPU,  mpn.NodeType_t.NODE_OPERATOR, mpn.ExecType_t.EXEC_BATCHED)
            else:
                self.__create_c_nodes__(
                    op, op.params, mpn.Device_t.DEVICE_CPU, mpn.NodeType_t.NODE_OPERATOR, mpn.ExecType_t.EXEC_NON_BATCHED)
        for op in self.__decoder_nodes__:
            self.__create_c_nodes__(
                op, op.params, mpn.Device_t.DEVICE_CPU_HPU, mpn.NodeType_t.NODE_DECODER, mpn.ExecType_t.EXEC_NON_BATCHED)
        for op in self.__hpu_nodes__:
            self.__create_c_nodes__(
                op, op.params, mpn.Device_t.DEVICE_HPU, mpn.NodeType_t.NODE_OPERATOR,  mpn.ExecType_t.EXEC_BATCHED)
