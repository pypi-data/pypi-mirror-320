...
def validate_timestamp_len(t: AnyStr) -> bool:
    """len(t) == 10"""
    return len(t) == 10


def validate_randomness_len(r: AnyStr) -> bool:
    """len(r) == 16"""
    return len(r) == 16


def validate_ulid_len(ulid: AnyStr) -> bool:
    """len(ulid) == 26"""
    return len(ulid) == 26


def build(t: bytes, r: bytes) -> bytes:
    """base32.encode_crockford_without_padding(t + r)"""
    return base32.encode_crockford_nopad(t + r)
...
def split(ulid: AnyStr) -> tuple[AnyStr, AnyStr]:
    """ulid[:10], ulid[10:]"""
