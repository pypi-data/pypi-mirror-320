...
def batch_encode_samples(data,features,start,limit,num_parallel=8,pbar_text="",progress=True):
    ...
    if progress:
        with tqdm_joblib(total=real_limit-start) as progress_bar:
            progress_bar.set_description(pbar_text)
            results= Parallel(n_jobs=num_parallel)(delayed(encode_sample)(sample) for sample in data.select(range(start,real_limit)))
            #filter None
...
def save_shard(data,features,start,limit,file,pbar_text)->int:
    ...
    pq.write_table(pa_table, file, row_group_size=4, use_dictionary=True,compression="snappy")
    return len(examples)