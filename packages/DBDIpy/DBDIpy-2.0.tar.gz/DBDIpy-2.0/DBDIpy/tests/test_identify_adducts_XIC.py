...
def test_input_TypeErr():
    with pytest.raises(TypeError):
        ...
        dbdi.identify_adducts_XIC([212], masses = annotation_metadata["ThMass.Ion"],
...
def test_input_ValErr():
    with pytest.raises(ValueError):
        ...
        dbdi.identify_adducts_XIC(df = imputed, masses = annotation_metadata["ThMass.Ion"][0:5],
                              custom_adducts = None, method = "spearman", 
                              threshold = 0.90, mass_error = 2)
...
def test_input_NaN():
    with pytest.raises(ValueError):
        ...
        dbdi.identify_adducts_XIC(df = imputed, masses = annotation_metadata["ThMass.Ion"],
                              custom_adducts = None, method = "spearman", 
                              threshold = 0.90, mass_error = 2)
...
def test_impute_outputn():
    ...
    trtest= 0.90
    ertest = 5
    res_adductsearch =  dbdi.identify_adducts_XIC(df = imputed, masses = annotation_metadata["ThMass.Ion"],
