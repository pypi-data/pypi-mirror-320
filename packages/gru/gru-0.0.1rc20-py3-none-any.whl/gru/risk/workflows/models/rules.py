...
class RiskRule(BaseModel):
    ...
    stage: Literal['REGISTERED', 'BACKTESTING', 'STAGING', 'PRODUCTION_EXPERIMENT', 'LIVE']
...
class UpdateRuleStageRequest(BaseModel):
    ...
    def from_str(cls, stage: str) -> 'UpdateRuleStageRequest':
        ...
        if stage_upper not in ('REGISTERED', 'BACKTESTING', 'STAGING', 'PRODUCTION_EXPERIMENT', 'LIVE'):
            raise ValueError(
                "Stage must be one of:'REGISTERED', 'BACKTESTING', 'STAGING', 'PRODUCTION_EXPERIMENT', 'LIVE'"
            )
