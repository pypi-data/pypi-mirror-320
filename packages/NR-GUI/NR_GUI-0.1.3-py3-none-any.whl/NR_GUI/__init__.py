...
def Text(Title, LocationX: float, LocationY: float, ScaleX: float, ScaleY: float):
    ...
    Title.place(x=LocationX, y=LocationY, width=ScaleY, height=ScaleX)
...
def Bake(Title, ScaleX: int, ScaleY: int,DefaultPng: bool ,CanResize: bool):
    if DefaultPng == True:
        try:
            with open('C:\Program Files\NR\GUI\icon\defaulticon.png', 'r') as deficon:
                icon = tk.PhotoImage(file=f"C:\\Program Files\\NR\\GUI\\icon\\defaulticon.png")
                root.iconphoto(True, icon)
        except:
            icon_id = "1jc4wuMhC_7VzyXIzokONz1onlaQE5JTo"
            url = f'https://drive.google.com/uc?export=download&id={icon_id}'
            output_path = f'C:\Program Files\NR\GUI\icon\defaulticon.png'
