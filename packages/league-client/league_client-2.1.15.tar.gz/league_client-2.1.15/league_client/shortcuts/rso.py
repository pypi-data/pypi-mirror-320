...
def _get_account_data(
    access_token: str, id_token: str, proxy: Optional[ProxyT] = None
) -> dict[str, Any]:
            ...
            if future is future_proessat:
                ...
                if restrictions:
                    ...
                    if "TEXT_CHAT_RESTRICTION" in restrictions:
                        account_data["chat_restriction"] = True
                    if "TIME_BAN" in restrictions:
                        account_data["time_restriction"] = True
                    raise AccountCheckError(
                        f"Account has restrictions: {restrictions}."
                    )
...
def get_account_data(
    ...
    return _get_account_data(access_token, id_token, proxy)
