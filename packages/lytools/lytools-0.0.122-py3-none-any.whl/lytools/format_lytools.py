...
class Tools:
    ...
    def mask_999999_arr(self, arr, warning=True):
        ...
        if warning == True:
            raise UserWarning(
    '\33[7m' +
    "Fatal Bug !!!  \t  Need to change !!!  \t  Value return added" +
     '\33[0m')
        return arr
    ...
    def pix_to_address(self, pix, outf, pix_to_lon_lat_dic_f):
        ...
        if not os.path.isfile(outf):
            np.save(outf, {0: 0})
        # pix_to_lon_lat_dic_f =
        # DIC_and_TIF().this_class_arr+'pix_to_lon_lat_dic.npy'
    ...
    x, val_new, kind='nearest', fill_value="extrapolate")
        ...
        interp_1 = interpolate.interp1d(
    xii, val_new_ii, kind='nearest', fill_value="extrapolate")
    ...
    def interp_nan(self, val, kind='nearest', valid_percent=0.1):
        ...
        if flag / len(val) < valid_percent:
            return [None]
        interp = interpolate.interp1d(
    x, val_new, kind=kind, fill_value="extrapolate")
    ...
    def detrend_vals(self, vals):
        ...
        m, b, r_val, p_val, std_err = stats.linregress(
            x[not_nan_ind], vals[not_nan_ind])
        ...
        for r in range(arrs.shape[1]):
            ...
            for c in range(arrs.shape[2]):
                ...
                if self.is_all_nan(val_list):
                    ...
                    p_i.append(np.nan)
                    continue
                a, b, R, p = self.nan_line_fit(
                    np.arange(len(val_list)), val_list)
                trend_i.append(a)
                p_i.append(p)
    ...
    def point_to_shp(self, inputlist, outSHPfn):
        ...
        if len(inputlist) > 0:
            ...
            outLayer = outDataSource.CreateLayer(
                outSHPfn, geom_type=ogr.wkbPoint)
    ...
    def line_to_shp(self, inputlist, outSHPfn):
        ...
        outDataSource = shpDriver.CreateDataSource(outSHPfn)
        outLayer = outDataSource.CreateLayer(
    outSHPfn, geom_type=ogr.wkbLineString)
    ...
    def group_consecutive_vals(self, in_list):
        ...
        for _, group in groupby(
            enumerate(in_list), lambda index_item: index_item[0] - index_item[1]):
    ...
    def nan_correlation(self, val1_list, val2_list, method='pearson'):
        ...
        if not len(val1_list) == len(val2_list):
            raise UserWarning(
                'val1_list and val2_list must have the same length')
        val1_list_new = []
        val2_list_new = []
        ...
        if len(val1_list_new) <= 3:
            ...
            if method == 'pearson':
                ...
                raise UserWarning(
                    'method must be pearson or spearman or kendall')

        return r, p

    def moving_window_correlation(
        self, arr1, arr2, window_size: int = 10, date_list: list = None):
    ...
    def normalize(self, vals, norm_max=1., norm_min=-
                  1., up_limit=None, bottom_limit=None):
    ...
    def convert_val_to_time_series_obj(
        self, data, time_stamp_list, name='value'):
    ...
    def monthly_vals_to_annual_val(
        self, monthly_vals, grow_season=None, method='mean'):
        '''
        from
        [1,2,3....,46,47,48]
        to
        ...
        monthly_vals_reshape_T_gs = Tools().pick_vals_from_1darray(
            monthly_vals_reshape_T, grow_season)
    ...
    def monthly_to_annual_with_datetime_obj(
        self, vals, date_range, grow_season: list, method='mean'):
        if grow_season is None:
            grow_season = list(range(12))
        else:
            grow_season = np.array(grow_season, dtype=int)
    ...
    def vals_to_time_sereis_annual(
        self, vals, yearlist: list = None, start_year: int = None):
        if yearlist == start_year is None:
            raise UserWarning(
                'You must input at least one parameter of "year list" or "start year"')
    ...
    *unique_value_list, is_unique=is_unique)
            cross_df_dict = {}
            for x in cross_list:
    ...
    def cmap_diverging(self, start_color_hue, end_color_hue,
    ...
    def nc_to_tif(self, fname, var_name, outdir):
        ...
        try:
            ...
            try:
                basetime = datetime.datetime.strptime(
                    basetime, '%Y-%m-%d %H:%M:%S')
            except BaseException:
                try:
                    basetime = datetime.datetime.strptime(
                        basetime, '%Y-%m-%d %H:%M:%S.%f')
                except BaseException:
                    try:
                        basetime = datetime.datetime.strptime(
                            basetime, '%Y-%m-%d %H:%M')
        ...
        for time_i in tqdm(range(len(time))):
            ...
            DIC_and_TIF().lon_lat_val_to_tif(
    lon_list_new, lat_list_new, val_list_new, outpath)

    def uncertainty_err(self, vals):
        mean = np.nanmean(vals)
        std = np.nanstd(vals)
        up, bottom = stats.t.interval(
            0.95, len(vals) - 1, loc=mean, scale=std / np.sqrt(len(vals)))
        err = mean - bottom
    ...
    def bootstrap_data(self, data: pd.DataFrame, n: int, ratio: float):
        ...
        data_len = len(data)
        for i in range(n):
            bootstraped_data = data.sample(
                n=int(data_len * ratio), replace=True)
...
class SMOOTH:
    ...
    def smooth_interpolate(self, inx, iny, zoom):
        ...
        x_new = np.arange(min(inx), max(
            inx), ((max(inx) - min(inx)) / float(len(inx))) / float(zoom))
...
class DIC_and_TIF:
    ...
    def pix_dic_to_tif_every_time_stamp(
        self, spatial_dic, outdir, filename_list: list = None):
        Tools().mkdir(outdir)
        # check values number
        vals_number_list = []
        for pix in spatial_dic:
    ...
    def plot_back_ground_arr_north_sphere(
        self, rasterized_world_tif, ax=None, **kwargs):
        ndv = ToRaster().get_ndv(rasterized_world_tif)
        arr = ToRaster().raster2array(rasterized_world_tif)[0]
        back_ground = []
        for i in range(len(arr)):
        ...
        back_ground = np.array(back_ground)
        if ax is None:
            plt.imshow(back_ground[:int(len(arr) / 2)],
                       'gray', vmin=0, vmax=1.4, zorder=-1, **kwargs)
        else:
            ax.imshow(back_ground[:int(len(arr) / 2)],
                      'gray', vmin=0, vmax=1.4, zorder=-1, **kwargs)
    ...
    def unify_raster(self, in_tif, out_tif, ndv=-999999):
        ...
        bottom_line_num = abs(
    (90. + originY + pixelHeight * len(array)) / pixelHeight)
        ...
        right_line_num = abs(
            (180. - (originX + pixelWidth * len(array[0]))) / pixelWidth)
        ...
        for i in array_unify_top_bottom:
            if len(left_array_insert) != 0:
                arr_temp = np.insert(
    i, obj=0, values=left_array_insert, axis=0)
            else:
                arr_temp = i
        ...
        newRasterfn = out_tif
        ToRaster().array2raster(newRasterfn, -180, 90, pixelWidth,
                 pixelHeight, array_unify_left_right, ndv=ndv)

    # todo: need to be tested
    def unify_raster1(self, in_tif, out_tif, res, srcSRS='EPSG:4326', dstSRS='EPSG:4326'):
        row = len(self.arr_template)
        col = len(self.arr_template[0])
        sY = self.originY
        sX = self.originX
    ...
    def resample_reproj(self, in_tif, out_tif, res,
    ...
    def plot_sites_location(self, lon_list, lat_list, background_tif=None, inshp=None, out_background_tif=None,
        ...
        if inshp:
            ...
            if os.path.isfile(out_background_tif):
                ...
                background_tif = ToRaster().shp_to_raster(
    inshp, out_background_tif, pixel_size=pixel_size, ndv=255)
    ...
    def calculate_pixel_area(self):
        ...
        for pix in tqdm(pix_list, desc='calculate_pixel_area'):
            ...
            upper_left_to_upper_right = self.GetDistance(upper_left_lon, upper_left_lat, upper_right_lon,
                                                         upper_right_lat)
            upper_left_to_lower_left = self.GetDistance(
    upper_left_lon, upper_left_lat, lower_left_lon, lower_left_lat)
    ...
    def run(self, process=4, process_or_thread='p', **kwargs):
        ...
        if process > 0:
            if process_or_thread == 'p':
                ...
                raise IOError(
                    'process_or_thread key error, input keyword such as "p" or "t"')

            results = list(
    tqdm(
        pool.imap(
            self.func, self.params), total=len(
                self.params), **kwargs))
            pool.close()
            pool.join()
            return results
...
class KDE_plot:
    ...
    def plot_fit_line(self, a, b, r, p, X, ax=None, title='',
                      is_label=True, is_formula=True, line_color='k', **argvs):
        '''
        画拟合直线 y=ax+b
        画散点图 X,Y
        :param a:
    ...
    def plot_scatter(self, val1, val2,
        ...
        if len(val1) > max_n:
            ...
            val_range_index = random.sample(
    val_range_index, max_n)  # 从val中随机选择n个点，目的是加快核密度算法
            new_val1 = []
            new_val2 = []
    ...
    def plot_scatter_hex(self, x, y, kind="hex",
                         color="#4CB391", xlim=None, ylim=None, gridsize=80):
    ...
    name=f'{cmap}_transparency', colors=color_array)
...
class Pre_Process:
    ...
    def compose_tif_list(self, flist, outf, less_than=-9999, method='mean'):
        ...
        for pix in tqdm(void_dic, desc='calculating mean...'):
            ...
            if method == 'mean':
                ...
                raise UserWarning(
                    f'{method} is invalid, should be "mean" "max" or "sum"')
            spatial_dic[pix] = compose_val
        DIC_and_TIF(
    tif_template=tif_template).pix_dic_to_tif(
        spatial_dic, outf)
    ...
    def monthly_compose(self, indir, outdir,
                        date_fmt='yyyymmdd', method='mean'):
        '''
    ...
    def time_series_dic_to_tif(self, spatial_dic, tif_template, outf_list):
        for i in tqdm(range(len(outf_list))):
            ...
            for pix in spatial_dic:
                ...
                spatial_dic_i[pix] = val
            arr = DIC_and_TIF(
    tif_template=tif_template).pix_dic_to_spatial_arr(spatial_dic_i)
...
class Plot:
    ...
    def plot_line_with_gradient_error_band(self, x, y, yerr, color_gradient_n=100, c=None,
        ...
        y = np.array(y)
        yerr = np.array(yerr)
        alpha_range_ = np.linspace(
    min_alpha, math.pow(
        max_alpha, int(pow)), int(
            color_gradient_n / 2))
    ...
    def plot_line_with_error_bar(
        self, x, y, yerr, c=None, alpha=0.2, **kwargs):
    ...
    def plot_hist_smooth(self, arr, interpolate_window=5,
                         range=None, **kwargs):
        weights = np.ones_like(arr) / float(len(arr))
    ...
    def plot_hist_smooth_colorful(self, arr, palette='PiYG', color_range=None,hist_range=None, **kwargs):
        ...
        color_list_new = Tools().gen_colors(
    len(patches) - left - right, palette=palette)
        for i, p in enumerate(patches):
            x_pos = p.xy[0]
            if x_pos < range_min:
    ...
    def multi_step_pdf(self, df, pdf_colname, bin_colname, bins, pdf_bin_n=40, pdf_range=None, discard_limit_n=10,
        ...
        for name, df_group_i in df_group:
            ...
            color_len += 1
        # exit()
        color_list = Tools().cmap_blend(
    blend_color_list, as_cmap=False, n_colors=color_len)
    ...
    np.arange(
        ...
        for obj in meridict:
            ...
            line.set_clip_path(
    clip_circle.get_path(),
     clip_circle.get_transform())
    ...
    def plot_Robinson_significance_scatter(self, m, fpath_p, temp_root, sig_level=0.05, ax=None, linewidths=0.5, s=20,
        ...
        fpath_spatial_dict = DIC_and_TIF(
    tif_template=fpath_p).spatial_tif_to_dic(fpath_p)
        ...
        DIC_and_TIF(tif_template=fpath_p).pix_dic_to_tif(
            fpath_clip_spatial_dict_clipped, fpath_clip)
        ...
        self.Robinson_reproj(
    fpath_resample,
    fpath_resample_ortho,
     res=res * 100000)
        arr, originX, originY, pixelWidth, pixelHeight = ToRaster(
        ).raster2array(fpath_resample_ortho)
    ...
    vmax=vmax,
        ...
        m.drawparallels(np.arange(-60., 90., 30.), zorder=99,
                        dashes=[8, 8], linewidth=.5)
    ...
    ticks=bounds,
     orientation='horizontal')
                ...
                cbar = plt.colorbar(
    ret, ax=ax, shrink=0.5, location='bottom', pad=0.05)
        return m, ret
    ...
    def plot_China_Albers(self, fpath, in_shpfile, shp_provinces_f,ax=None, cmap=None, vmin=None, vmax=None, is_plot_colorbar=True, is_reproj=True,res=10000,is_discrete=False,colormap_n=11):
        ...
        if cmap is None:
            cmap = Tools().cmap_blend(color_list)
        arr_deg, originX_deg, originY_deg, pixelWidth_deg, pixelHeight_deg = ToRaster(
        ).raster2array(fpath)
    ...
    color='k',
     zorder=100)
        ...
        #     line = meridict[obj][0][0]
        # coastlines = m.drawcoastlines(zorder=100, linewidth=0.2)
        # polys = m.fillcontinents(color='#D1D1D1',
        # lake_color='#EFEFEF',zorder=90)
        if is_plot_colorbar:
            if is_discrete:
                bounds = np.linspace(vmin, vmax, colormap_n)
                # norm = mpl.colors.BoundaryNorm(bounds, cmap.N, extend='both')
...
class ToRaster:
    ...
    def array2raster_GDT_Byte(self, newRasterfn, longitude_start,
                              latitude_start, pixelWidth, pixelHeight, array):
        cols = array.shape[1]
        rows = array.shape[0]
    ...
    def array2raster(self, newRasterfn, longitude_start,
                     latitude_start, pixelWidth, pixelHeight, array, ndv=-999999):
        cols = array.shape[1]
        rows = array.shape[0]
        originX = longitude_start
        originY = latitude_start
    ...
    def shp_to_raster(self, in_shp, output_raster, pixel_size,
                      in_raster_template=None, ndv=-999999):
    ...
    def mask_array(self, in_raster, out_raster, in_mask_raster):
        in_arr, originX, originY, pixelWidth, pixelHeight = self.raster2array(
            in_raster)
        in_mask_arr, originX, originY, pixelWidth, pixelHeight = self.raster2array(
            in_mask_raster)
...
class HANTS:
    ...
    def hants_interpolate(self, values_list, dates_list,
                          valid_range, nan_value=np.nan):
        '''
    ...
    def __get_starter_matrix(self, base_period_len,
                             sample_count, frequencies_considered_count):
        ...
        for column in range(sample_count):
            ...
            # index looks like 000, 123, 246, etc, until it wraps around (for
            # len(i)==3)
    ...
    def __hants(self, sample_count, inputs,
        ...
        mat = self.__get_starter_matrix(
    base_period_len,
    sample_count,
     frequencies_considered_count)

        # repeat the mat array over the number of arrays in inputs
        # and create arrays with ones with shape inputs where high and low
        # values are set to 0
        mat = np.tile(mat[None].T, (1, inputs.shape[0])).T
        p = np.ones_like(inputs)
        p[(low >= inputs) | (inputs > high)] = 0
        # count the outliers for each timeseries
        nout = np.sum(p == 0, axis=-1)

        # prepare for while loop
        # all timeseries set to false
        ready = np.zeros((inputs.shape[0]), dtype=bool)
        ...
        for _ in range(sample_count):
            ...
            # multiply mat with the multiplication of multiply diagonal of p
            # with transpose of mat
    ...
    np.einsum(
        ...
        diag,
         mat.T))
            ...
            A = A + np.tile(np.diag(np.ones(nr))
                            [None].T, (1, inputs.shape[0])).T * delta
            A[:, 0, 0] = A[:, 0, 0] - delta
