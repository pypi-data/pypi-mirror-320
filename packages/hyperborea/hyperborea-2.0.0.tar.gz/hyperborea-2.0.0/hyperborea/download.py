...
def ref_sort_key(value: str) -> tuple[bool, Union[Version, str]]:
    try:
...
class _Fetcher(QtCore.QObject):
    ...
    def __init__(self, logger: Logger, extra: Optional[Any] = None):
        super().__init__()
        self.logger = logger
        self.extra = extra
...
class FirmwareFinder(QtCore.QObject):
    ...
    def find_firmware(self, build_type: Optional[str],
                      board_info: Optional[tuple[str, int]] = None,
...
class RefFinder(QtCore.QObject):
    ...
    def get_firmware_refs(self, board_info: Optional[tuple[str, int]] = None,
