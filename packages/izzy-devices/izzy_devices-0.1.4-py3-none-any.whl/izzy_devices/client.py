...
class Client:
    ...
    def build_base_response(self):
        ...
        A bytearray.
        """
        return (bytearray(self.name.encode()) + self.delimiter +
                bytearray(struct.pack("i", self.status.value)) + self.delimiter +
                bytearray(struct.pack("i", self.position["x"])) + self.delimiter +
                bytearray(struct.pack("i", + self.position["y"])) + self.delimiter +
                bytearray(struct.pack("i", self.position["z"])) + self.delimiter +
                bytearray(struct.pack("i", self.heading)) + self.delimiter +
                bytearray(struct.pack("i", self.speed)))

    def build_following_response(self):
        data = self.build_base_response() + self.delimiter
        data += (bytearray(struct.pack("f", self.pid_kp)) + self.delimiter +
                 bytearray(struct.pack("f", self.pid_ki)) + self.delimiter +
                 bytearray(struct.pack("f", self.pid_kd)) + self.delimiter +
                 bytearray(struct.pack("f", self.error)) + self.delimiter +
                 bytearray(struct.pack("f", self.error_angle)) + self.delimiter +
                 bytearray(struct.pack("f", self.line_sensor_1_min)) + self.delimiter +
                 bytearray(struct.pack("f", self.line_sensor_1_max)) + self.delimiter +
                 bytearray(struct.pack("f", self.line_sensor_1_read)) + self.delimiter +
                 bytearray(struct.pack("f", self.line_sensor_2_min)) + self.delimiter +
                 bytearray(struct.pack("f", self.line_sensor_2_max)) + self.delimiter +
                 bytearray(struct.pack("f", self.line_sensor_2_read)))
        return data
