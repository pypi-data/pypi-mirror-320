...
def run_sird(data_path):
    ...
    # Set necessary keys in R
    R['equa_name'] = 'simulated_SIRD3'  # This resolves the KeyError
    R['ODE_type'] = 'SIRD'

    # Read data and set parameters
    temp_df = pd.read_csv(data_path)
    Total_observations = len(temp_df)
    R['size2train'] = math.floor(Total_observations * (14 / 15))
    R['batch_size2train'] = math.floor(Total_observations * (1 / 15))
    R['batch_size2test'] = 10
    R['loss_type'] = 'L2_loss'
    R['learning_rate'] = 4e-3
    R['model2NN'] = 'Fourier_DNN'
    R['hidden_layers'] = (40, 50, 25, 25, 10)
    R['name2act_in'] = 'tanh'
    R['name2act_hidden'] = 'sinAddcos'
