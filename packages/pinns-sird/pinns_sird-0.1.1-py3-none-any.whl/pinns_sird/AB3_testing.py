...
def solve_SIRD(R):
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, 50, gamma=0.975)
    # 每100(50, 75)步改为原97.5%
    ...
    s_obs_test = dataUtils.sample_testData_serially(test_data2s[2: ], batchsize2test - 1, normalFactor=1.0, is_torch=True)
    i_obs_test = dataUtils.sample_testData_serially(test_data2i[2: ], batchsize2test - 1, normalFactor=1.0, is_torch=True)
    r_obs_test = dataUtils.sample_testData_serially(test_data2r[2: ], batchsize2test - 1, normalFactor=1.0, is_torch=True)
    d_obs_test = dataUtils.sample_testData_serially(test_data2d[2: ], batchsize2test - 1, normalFactor=1.0, is_torch=True)

    init2S_test = test_data2s[1]
    init2I_test = test_data2i[1]
    init2R_test = test_data2r[1]
    init2D_test = test_data2d[1]
    S_1test = test_data2s[0]
    I_1test = test_data2i[0]
    R_1test = test_data2r[0]
    D_1test = test_data2d[0]
    ...
    for epoch in range(R['max_epoch'] + 1):
        ...
        if epoch % 2000 == 0:
            ...
            fix_paras_nn, S_predict2fix, I_predict2fix, R_predict2fix, D_predict2fix = \
                SIRDmodel.evaluate_AB3DNN_FixedParas(t=last_train_t, s_init=init2S_test, i_init=init2I_test,
    ...
    BetaFin = SIRDmodel.BetaNN(datetensor, scale=SIRDmodel.factor2freq).detach().numpy()
    MuFin = SIRDmodel.MuNN(datetensor, scale=SIRDmodel.factor2freq).detach().numpy()
    GammaFin = SIRDmodel.GammaNN(datetensor, scale=SIRDmodel.factor2freq).detach().numpy()
    ...
    test_t_bach_numpy = test_t_bach[1: ].cpu().detach().numpy()
