...
impl crate::types::request::OrderIntent {
    pub fn is_taker_price_within_range(&self, max_deviation: Decimal, mark_price: Decimal) -> bool {
        match self.side {
            OrderSide::Bid => {
                let max = mark_price * (Decimal::one() + max_deviation);
                *self.price <= max
            }
            OrderSide::Ask => {
                let min = mark_price * (Decimal::one() - max_deviation);
                *self.price >= min
            }
        }
    }
