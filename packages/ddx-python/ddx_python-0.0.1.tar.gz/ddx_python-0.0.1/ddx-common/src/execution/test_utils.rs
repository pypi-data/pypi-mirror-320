...
use crate::{
    ...
    types::{
        ...
        primitives::{
            Bytes32, Hash, OrderHash, OrderSide, ProductSymbol, TimeValue, TokenSymbol,
            TraderAddress, UnscaledI128,
        },
        request::{
            CancelOrderIntent, OrderIntent, OrderType, ProfileUpdate, WithdrawDDXIntent,
            WithdrawIntent,
        },
    ...
    F: Into<f64>,
{
    ...
    (OrderHash::try_from(&order).unwrap(), order)
}

pub fn fake_order_with_key<F>(
    key: &str,
    symbol: &str,
    strategy: &str,
    side: OrderSide,
...
pub fn fake_orders(fake_set: FakeOrderSet) -> Vec<(OrderHash, OrderIntent)> {
    let mut orders = vec![];
