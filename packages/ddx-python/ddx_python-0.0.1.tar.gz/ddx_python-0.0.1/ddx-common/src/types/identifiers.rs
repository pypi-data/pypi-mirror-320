...
use crate::{
    ...
    types::{
        ...
        primitives::{
            Bytes21, Bytes25, Bytes32, EpochId, FixedBytesWrapper, Hash, IndexPriceHash, OrderHash,
            ProductSymbol, TraderAddress,
        },
        state::{
...
impl From<StrategyIdHash> for Bytes32 {
    fn from(value: StrategyIdHash) -> Self {
        // Place the identifier in the first four bytes and zero pad the remainder
        let mut bytes = vec![];
        bytes.extend_from_slice(value.0.as_slice());
        bytes.resize(Bytes32::BYTE_LEN, 0_u8);
