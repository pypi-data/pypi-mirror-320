...
class CsvDocumentBackend(DeclarativeDocumentBackend):
    def __init__(self, in_doc: "InputDocument", path_or_stream: Union[BytesIO, Path]):
        _log.debug(f"Initializing CsvDocumentBackend with path_or_stream: {path_or_stream}")
        ...
        try:
            ...
            # Add debug logging
            _log.info(f"Loaded {len(self.rows)} rows from CSV file using {self.encoding} encoding")
            self.valid = True
