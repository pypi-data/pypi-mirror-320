...
class Fragment:
    ...
    @classmethod
    def replace(cls, content, list_of_pairs):
        if not list_of_pairs:
            return content
        fragment = cls(content, *(pairs[0] for pairs in list_of_pairs), sep=True)
        result = ''
        for i, pairs in enumerate(list_of_pairs):
            result += fragment[2 * i] + pairs[1]
    ...
    @classmethod
    def replace_safe_again(cls, content, list_of_pairs):
        try:
            content = cls.replace(content, list_of_pairs)
        except IndexError as e:
            try:
                cls(content, *(pairs[1] for pairs in list_of_pairs))
            except IndexError:
                raise e
        else:
            return content
