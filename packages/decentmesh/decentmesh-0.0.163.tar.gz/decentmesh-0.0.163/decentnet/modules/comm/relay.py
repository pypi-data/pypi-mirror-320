...
class Relay:
    ...
    async def erase_edge_connection(self):
        ...
        for beam in all_alive_beams:
            if beam.pub_key == self.beam_pub_key:
                continue
            serialized_broadcast = cbor2.dumps(await assemble_disconnect_data(self.public_key_id,
                                                                              self.beam.pub_key,
                                                                              self.beam_pub_key))

            # Send disconnect details to another process with correct blockchain for block assembly
            self.beam_pipe_comm[beam.pub_key][1].send(serialized_broadcast)
