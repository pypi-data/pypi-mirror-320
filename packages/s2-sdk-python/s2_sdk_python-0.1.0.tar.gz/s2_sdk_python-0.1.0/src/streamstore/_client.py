...
class S2:
    ...
    Note:
        Connections are opened lazily i.e. connection get established only when
    ...
    # MYDO: revisit if async ctx mgr is needed
    ...
    async def create_basin(
        ...
        """
        Create a basin. Returned Basin object can be used for performing basin operations.

        Args:
            name: Name of the basin.
            default_stream_storage_class: Default storage class for streams of this basin.
            default_stream_retention_age: Default thresold for automatic trimming of records in the streams of this basin.
        ...
        return Basin(self, response.info.name, response.info.scope, response.info.cell)
    ...
    async def list_basins(
        ...
        Args:
            ...
            limit: Number of items to return in one page. Maximum number of items that can be
                returned in one page is 1000. A value of 0 signifies no limit, thus maximum possible
                number of items gets returned in that case.
        """
        ...
        return schemas.Page(
            items=[
                schemas.BasinInfo(
                    b.name, b.scope, b.cell, schemas.BasinState(b.state.value)
                )
                for b in response.basins
            ],
...
class Basin:
    ...
    def __repr__(self) -> str:
        return f"Basin(name={self.name}, scope={self.scope}, cell={self.cell}"
    ...
    async def create_stream(
        ...
        """
        Create a stream. Returned Stream object can be used for performing stream operations.
        ...
        return Stream(self, response.info.name)
    ...
    async def list_streams(
        ...
        return schemas.Page(
            items=[
                schemas.StreamInfo(
                    s.name,
                    datetime.fromtimestamp(s.created_at),
                    datetime.fromtimestamp(s.deleted_at)
...
class Stream:
    ...
    async def read(
        ...
        Args:
            ...
            limit: Number of records to return, upto a maximum of 1000 or 1MiB of metered bytes.
