...
def bp2(f, task):
    ...
    auto = np.dstack(
        list(data["PA_LFR_SC_BP2_AUTO_A{0}_F0".format(x)] for x in range(5))
    )
    cross_re = np.dstack(
        list(data["PA_LFR_SC_BP2_CROSS_RE_{0}_F0".format(x)] for x in range(10))
    )
    cross_im = np.dstack(
        list(data["PA_LFR_SC_BP2_CROSS_IM_{0}_F0".format(x)] for x in range(10))
    )
...
def cwf(f, task):
    ...
    in_data = {name: fdata[name][...] for name in fdata.keys()}
