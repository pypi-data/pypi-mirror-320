...
class AutoSlicer:
    ...
    def _add_skin_label(self, source_volume: np.ndarray, segmentation_result: np.ndarray) -> np.ndarray:
        ...
        updated_seg[skin_label_area] = 100
        return updated_seg
...
if __name__ == "__main__":
    ...
    # # slicer.set_threshold(lower_val=-100, upper_val=200)
    #
    # # If you need to adjust density for a specific label:
    # # slicer.set_density(label=100, value=1.08)  # for example, set label 100 (skin) to density=1.08
    #
    # dicom_folder = r"D:\Curtin_Hive_Data\head-male female\1-1"
    # results = slicer.run_automation(dicom_folder)
    # print("Final automation results:", results)
