...
class ProxyToken(BaseModel):
    ...
    expire_at: datetime = Field(
        default_factory=lambda: datetime.now() + timedelta(seconds=MACAROON_TTL)
    )
...
def get_proxy_token_sync() -> ProxyToken:
    with CACHE_LOCK:
        ...
        try:
            ...
            if cache.expire_at < datetime.now():
                raise TimeoutError("Token expired")

        except (ValidationError, TimeoutError, FileNotFoundError) as exc:
