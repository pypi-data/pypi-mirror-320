...
class AnonymousFileUploadTestCase(BaseDrfFileUploadTestCase):
    ...
    def test_upload_model_serializer_valid_data(self):
        ...
        self.assertFalse(
            models.AuthenticatedUploadedFile.objects.filter(
        ...
        self.assertTrue(
            os.path.isfile(test_file_model.file.path), "File on filesystem is preserved"
        )
    ...
    def create_serializer(self, file_uuid):
        sut = test_serializers.TestAnonFileUploadSerializer(
            data={"file": str(file_uuid)}
        )
        return sut
