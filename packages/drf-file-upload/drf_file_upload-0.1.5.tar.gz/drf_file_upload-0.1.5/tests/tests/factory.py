...
def create_simple_uploaded_file(
    filename="best_file_eva.pdf", size=(100, 100), color=(155, 0, 0)
):
    file = io.BytesIO()
...
def create_authenticated_uploaded_file(user, filename="test_file.pdf", size=(100, 100)):
    ...
    return models.AuthenticatedUploadedFile.objects.create(
        file=File(file_contents), user=user, size=size[0] * size[1], name=filename
    )
