...
if __name__ == '__main__':
    ...
    for i in dataset:
        list_i_length = []
        db, data_label, itemset, max_sequence_length = datainput('/home/junjie/CodeAll/ISCT/Interpretable-Sequence-Clustering-Tree/dataset/{}.txt'.format(i))    
        for j in db:
            list_i_length.append(len(j))
        # write to the file
        with open('/home/junjie/CodeAll/ISCT/Interpretable-Sequence-Clustering-Tree/dataset/{}_length.txt'.format(i), 'w') as f:
            for item in list_i_length:
                f.write("%s \n" % item)
        f.close()
    db, data_label, itemset, max_sequence_length = datainput('/home/junjie/CodeAll/ISCT/Interpretable-Sequence-Clustering-Tree/dataset/{}.txt'.format('context'))
