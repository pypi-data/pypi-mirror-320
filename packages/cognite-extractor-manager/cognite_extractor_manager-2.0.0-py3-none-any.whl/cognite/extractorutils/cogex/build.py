...
def update_version(initfile: str) -> None:
    ...
    pyproject_version = get_pyproject()["project"]["version"]
    ...
    with fileinput.input(initfile, inplace=True) as file:
        for line in file:
            print(line.replace(init_version, pyproject_version), end="")
...
def build(args: Namespace) -> None:
    ...
    try:
        ...
        packages = [pyproject["project"]["name"]]
    ...
    headerprint("Making sure virtual environment is up to date")
    if os.system("uv sync --frozen"):
        fail("Could not install dependencies.")

    headerprint("Checking pyinstaller")
    if os.system("uv pip show pyinstaller"):
        fail(
            "No instance of pyinstaller found inside virtual environment",
    ...
    for specfile in specfiles:
        if os.system(f"uv run pyinstaller {specfile}"):
