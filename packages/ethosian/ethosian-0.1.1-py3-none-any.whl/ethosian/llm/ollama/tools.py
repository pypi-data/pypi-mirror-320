...
from ethosian.llm.base import LLM
from ethosian.llm.message import Message
from ethosian.llm.exceptions import InvalidToolCallException
...
from ethosian.utils.tools import (
...
class OllamaTools(LLM):
    ...
    def invoke(self, messages: List[Message]) -> Mapping[str, Any]:
        return self.client.chat(
            model=self.model,
            messages=[self.to_llm_message(m)
    ...
    def response(self, messages: List[Message]) -> str:
        ...
        # -*- Parse response
        response_message: Mapping[str, Any] = response.get(
            "message")  # type: ignore
                ...
                if "<tool_call>" in response_content and "</tool_call>" in response_content:
                    ...
                    for tool_call_response in tool_call_responses:
                        ...
                        if "<tool_call>" in tool_call_response and "</tool_call>" in tool_call_response:
                            ...
                            try:
                                logger.debug(f"Tool call content: {
                                             tool_call_content}")
                                tool_call_dict = json.loads(tool_call_content)
                            except json.JSONDecodeError:
                                raise ValueError(f"Could not parse tool call from: {
                                                 tool_call_content}")
                            ...
                            if tool_call_args is not None:
                                function_def["arguments"] = json.dumps(
                                    tool_call_args)
                            tool_calls.append(
                                {
                                    "type": "function",
        ...
        if assistant_message.tool_calls is not None and self.run_tools:
            ...
            for tool_call in assistant_message.tool_calls:
                ...
                if _function_call.error is not None:
                    messages.append(
                        Message(role="user", tool_call_error=True, content=_function_call.error))
                    continue
                function_calls_to_run.append(_function_call)

            if self.show_tool_calls:
                if len(function_calls_to_run) == 1:
                    final_response += f" - Running: {
            ...
            if len(function_call_results) > 0:
                ...
                for _fc_message in function_call_results:
                    fc_responses.append(
                        json.dumps({"name": _fc_message.tool_call_name,
    ...
    def response_stream(self, messages: List[Message]) -> Iterator[str]:
        ...
        for response in self.invoke_stream(messages=messages):
            ...
            response_content: str = response_message.get(
                "content", "") if response_message else ""
                ...
                for tool_call_response in tool_call_responses:
                    ...
                    if "<tool_call>" in tool_call_response and "</tool_call>" in tool_call_response:
                        ...
                        try:
                            ...
                            raise InvalidToolCallException(f"Error parsing tool call: {
                                                           tool_call_content}. Error: {e}")
