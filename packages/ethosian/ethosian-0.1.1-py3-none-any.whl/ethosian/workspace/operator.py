...
from ethosian.api.workspace import log_workspace_event
from ethosian.api.schemas.workspace import (
    WorkspaceSchema,
    WorkspaceCreate,
    WorkspaceUpdate,
    WorkspaceEvent,
)
from ethosian.api.schemas.team import TeamSchema, TeamIdentifier
from ethosian.cli.config import ethosianCliConfig
from ethosian.cli.console import (
    console,
    print_heading,
    print_info,
    print_subheading,
...
from ethosian.workspace.enums import WorkspaceStarterTemplate
from ethosian.utils.common import str_to_int
...
TEMPLATE_TO_REPO_MAP: Dict[WorkspaceStarterTemplate, str] = {
    WorkspaceStarterTemplate.agent_app: "https://github.com/ethosianhq/agent-app.git",
    WorkspaceStarterTemplate.agent_api: "https://github.com/ethosianhq/agent-api.git",
}
...
def create_workspace(
    ...
    from ethosian.utils.filesystem import rmdir_recursive
    from ethosian.workspace.helpers import get_workspace_dir_path
    from ethosian.utils.git import GitCloneProgress

    current_dir: Path = Path(".").resolve()

    # ethosian should be initialized before creating a workspace
    ...
    if not ethosian_config:
        ...
        if not ethosian_config:
            log_config_not_available_msg()
            return None
    ethosian_config = cast(ethosianCliConfig, ethosian_config)
    ...
    if repo_to_clone is None:
        ...
        if template is None:
            ...
            print_info(
                "Select starter template or press Enter for default (agent-app)")
            for template_id, template_name in enumerate(templates, start=1):
                print_info("  [b][{}][/b] {}".format(template_id,
                           WorkspaceStarterTemplate(template_name).value))

            # Get starter template from the user
            template_choices = [str(idx)
                                for idx, _ in enumerate(templates, start=1)]
    ...
    if ws_dir_name is None:
        ...
        if url is not None:
            ...
            default_ws_name = TEMPLATE_TO_NAME_MAP.get(
                ws_template, "agent-app")
...
def setup_workspace(ws_root_path: Path) -> Optional[WorkspaceConfig]:
    """Setup a ethosian workspace at `ws_root_path` and return the WorkspaceConfig

    1. Pre-requisites
    1.1 Check ws_root_path exists and is a directory
    1.2 Create ethosianCliConfig if needed
    ...
    2.1 Check if a ws_schema exists for this workspace, meaning this workspace has a record in ethosian-api
    ...
    from ethosian.cli.operator import initialize_ethosian
    from ethosian.utils.git import get_remote_origin_for_dir
    from ethosian.workspace.helpers import get_workspace_dir_path
    ...
    ######################################################
    # 1.1 Check ws_root_path exists and is a directory
    ws_is_valid: bool = ws_root_path is not None and ws_root_path.exists(
    ) and ws_root_path.is_dir()
    if not ws_is_valid:
    ...
    ws_config: Optional[WorkspaceConfig] = ethosian_config.get_ws_config_by_path(
        ws_root_path)
    if ws_config is None:
        # There's no record of this workspace, reasons:
        # - The user is setting up a new workspace
        # - The user ran `ethosian init -r` which erased existing workspaces
        ...
        logger.debug(
            f"Found the `workspace` configuration at: {workspace_ws_dir_path}")
        ws_config = ethosian_config.create_or_update_ws_config(
            ws_root_path=ws_root_path, set_as_active=True)
        if ws_config is None:
            logger.error(
                f"Failed to create WorkspaceConfig for {ws_root_path}")
            return None
    else:
        logger.debug(f"Found workspace at {ws_root_path}")

    # 1.4 Get the workspace name
    workspace_name = ws_root_path.stem.replace(
        " ", "-").replace("_", "-").lower()
    logger.debug(f"Workspace name: {workspace_name}")

    # 1.5 Get the git remote origin url
    git_remote_origin_url: Optional[str] = get_remote_origin_for_dir(
        ws_root_path)
    logger.debug("Git origin: {}".format(git_remote_origin_url))

    # 1.6 Create anon user if the user is not logged in
    if ethosian_config.user is None:
        from ethosian.api.user import create_anon_user

        logger.debug("Creating anon user")
        with Live(transient=True) as live_log:
            status = Status("Creating user...", spinner="aesthetic",
                            speed=2.0, refresh_per_second=10)
    ...
    if ethosian_config.user is not None:
        # 2.2 Create WorkspaceSchema if it doesn't exist
        if ws_schema is None or ws_schema.id_workspace is None:
            from ethosian.api.team import get_teams_for_user
            from ethosian.api.workspace import create_workspace_for_user
            ...
            logger.debug(f"Getting teams for user: {
                         ethosian_config.user.email}")
            ...
            with Live(transient=True) as live_log:
                ...
                live_log.update(status)
                teams = get_teams_for_user(ethosian_config.user)
                status.stop()
            if teams is not None and len(teams) > 0:
                logger.debug(
                    f"The user has {len(teams)} available teams. Checking if they want to use one of them")
                print_info(
                    "Which account would you like to create this workspace in?")
                print_info("  [b][1][/b] Personal (default)")
                for team_idx, team_schema in enumerate(teams, start=2):
                    print_info(
                        "  [b][{}][/b] {}".format(team_idx, team_schema.name))

                account_choices = ["1"] + [str(idx)
                                           for idx, _ in enumerate(teams, start=2)]
                account_inp_raw = Prompt.ask(
                    "Account Number", choices=account_choices, default="1", show_choices=False)
                account_inp = str_to_int(account_inp_raw)

                if account_inp is not None:
                    if account_inp == 1:
                        print_info(
                            "Creating workspace in your personal account")
                    else:
                        selected_team = teams[account_inp - 2]
                        print_info(
                            f"Creating workspace in {selected_team.name}")
                        team_identifier = TeamIdentifier(
                            id_team=selected_team.id_team, team_url=selected_team.url)

            with Live(transient=True) as live_log:
                status = Status(
                    "Creating workspace...", spinner="aesthetic", speed=2.0, refresh_per_second=10)
                live_log.update(status)
                ws_schema = create_workspace_for_user(
                    user=ethosian_config.user,
        ...
        if git_remote_origin_url is not None and ws_schema is not None and ws_schema.git_url != git_remote_origin_url:
            from ethosian.api.workspace import update_workspace_for_user, update_workspace_for_team
            ...
            if ws_config is not None and ws_config.ws_team is not None:
                updated_workspace_schema = update_workspace_for_team(
                    ...
                    team=TeamIdentifier(
                        id_team=ws_config.ws_team.id_team, team_url=ws_config.ws_team.url),
                )
            else:
    ...
    if ws_config is not None:
        ...
        print_info("\tethosian ws up")
        print_info("2. Stop workspace:")
        print_info("\tethosian ws down")
        ...
        if ws_config.ws_schema is not None and ethosian_config.user is not None:
            log_workspace_event(
                user=ethosian_config.user,
...
def start_workspace(
    ...
    """Start a ethosian Workspace. This is called from `ethosian ws up`"""
    if ws_config is None:
    ...
    if ethosian_config.user is not None and ws_config.ws_schema is not None and ws_config.ws_schema.id_workspace is not None:
...
def stop_workspace(
    ...
    """Stop a ethosian Workspace. This is called from `ethosian ws down`"""
    if ws_config is None:
        logger.error("WorkspaceConfig invalid")
        return
...
def update_workspace(
    ...
    """Update a ethosian Workspace. This is called from `ethosian ws patch`"""
    if ws_config is None:
        logger.error("WorkspaceConfig invalid")
        return
...
def delete_workspace(ethosian_config: ethosianCliConfig, ws_to_delete: Optional[List[Path]]) -> None:
    if ws_to_delete is None or len(ws_to_delete) == 0:
        print_heading("No workspaces to delete")
        return

    for ws_root in ws_to_delete:
        ethosian_config.delete_ws(ws_root_path=ws_root)
...
def set_workspace_as_active(ws_dir_name: Optional[str]) -> None:
    ...
    ######################################################
    ######################################################
    # 1.1 Check ethosianConf is valid
    ######################################################
    ethosian_config: Optional[ethosianCliConfig] = ethosianCliConfig.from_saved_config(
    )
    if not ethosian_config:
    ...
    if ws_dir_name is None:
        # If the user does not provide a ws_name, that implies `ethosian set` is ran from
        # the workspace directory.
        ws_root_path = Path(".").resolve()
    else:
        # If the user provides a workspace name manually, we find the dir for that ws
        ws_config: Optional[WorkspaceConfig] = ethosian_config.get_ws_config_by_dir_name(
            ws_dir_name)
        if ws_config is None:
    ...
    ws_dir_is_valid: bool = ws_root_path is not None and ws_root_path.exists(
    ) and ws_root_path.is_dir()
    ...
    active_ws_config: Optional[WorkspaceConfig] = ethosian_config.get_ws_config_by_path(
        ws_root_path)
    if active_ws_config is None:
        ...
        print_info(
            "If this workspace has not been setup, please run `ethosian ws setup` from the workspace directory")
        return
    ...
    ######################################################
    print_heading(
        f"Setting workspace {active_ws_config.ws_root_path.stem} as active")
    ethosian_config.set_active_ws_dir(active_ws_config.ws_root_path)
