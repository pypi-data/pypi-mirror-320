...
class MoviePyVideoTools(Toolkit):
    ...
    def split_text_into_lines(self, words: List[Dict]) -> List[Dict]:
        ...
        for idx, word_data in enumerate(words):
            ...
            maxgap_exceeded = idx > 0 and word_data["start"] - \
                words[idx - 1]["end"] > MAX_GAP
    ...
    def create_caption_clips(
        ...
        for word_data in text_json["textcontents"]:
            ...
            # Create space clip
            space_clip = (
                TextClip(text=" ", font=font, font_size=int(
                    fontsize), color=color, method="label")
                .with_start(text_json["start"])
                .with_duration(full_duration)
            )
    ...
    def embed_captions(
        ...
        try:
            ...
            for line in subtitle_lines:
                ...
                bg_clip = bg_clip.with_start(
                    line["start"]).with_position(bg_position)

                # Create word clips
                word_clips = self.create_caption_clips(
                    line, (video.w, bg_height))
