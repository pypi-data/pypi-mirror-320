...
def get_json_schema_for_arg(t: Any) -> Optional[Dict[str, Any]]:
    ...
    if type_origin is not None:
        if type_origin in (list, tuple, set, frozenset):
            json_schema_for_items = get_json_schema_for_arg(
                type_args[0]) if type_args else {"type": "string"}
            return {"type": "array", "items": json_schema_for_items}
        elif type_origin is dict:
            # Handle both key and value types for dictionaries
            key_schema = get_json_schema_for_arg(type_args[0]) if type_args else {
                "type": "string"}
            value_schema = get_json_schema_for_arg(type_args[1]) if len(
                type_args) > 1 else {"type": "string"}
...
def get_json_schema(
    ...
    for k, v in type_hints.items():
        ...
        try:
            ...
            is_optional = type_origin is Union and len(
                type_args) == 2 and any(arg is type(None) for arg in type_args)
            ...
            if arg_json_schema is not None:
                if is_optional:
                    ...
                    if isinstance(arg_json_schema["type"], list):
                        ...
                        arg_json_schema["type"] = [
                            arg_json_schema["type"], "null"]
