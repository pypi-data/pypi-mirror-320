...
from ethosian.infra.base import InfraBase
...
class ResourceBase(InfraBase):
    ...
    def get_resource_type_list(self) -> List[str]:
        ...
        type_list: List[str] = [resource_type.lower()
    ...
    def get_input_file_path(self) -> Optional[Path]:
        ...
        if workspace_dir is None:
            from ethosian.workspace.helpers import get_workspace_dir_from_env
    ...
    def save_output_file(self) -> bool:
        ...
        if output_file_path is not None:
            try:
                from ethosian.utils.yaml_io import write_yaml_file
                ...
                logger.error(f"Could not write {
                             self.get_resource_name()} to file: {e}")
        return False
    ...
    def read_resource_from_file(self) -> Optional[Dict[str, Any]]:
        ...
        if output_file_path is not None:
            try:
                from ethosian.utils.yaml_io import read_yaml_file
                ...
                if output_file_path.exists() and output_file_path.is_file():
                    ...
                    if data_from_file is not None and isinstance(data_from_file, dict):
                        ...
                        logger.warning(f"Could not read {self.get_resource_name()} from {
                                       output_file_path}")
            except Exception as e:
                logger.error(f"Could not read {
                             self.get_resource_name()} from file: {e}")
        return None
        ...
        if output_file_path is not None:
            try:
                if output_file_path.exists() and output_file_path.is_file():
                    ...
                    logger.debug(f"Output file deleted: {
