...
class GlueCrawler(AwsResource):
    ...
    def get_glue_crawler_targets(self) -> Optional[Dict[str, List[dict]]]:
        ...
        if self.s3_targets is not None:
            ...
            for s3_target in self.s3_targets:
                _new_s3_target_path = f"s3://{
                    s3_target.bucket.name}/{s3_target.dir}"
                # start with the only required argument
                _new_s3_target_dict: Dict[str, Any] = {
                    "Path": _new_s3_target_path}
    ...
    def _read(self, aws_client: AwsApiClient) -> Optional[Any]:
        ...
        try:
            ...
            self.creation_time = get_crawler_response.get(
                "Crawler", {}).get("CreationTime", None)
            self.last_crawl = get_crawler_response.get(
                "Crawler", {}).get("LastCrawl", None)
    ...
    def start_crawler(self, aws_client: Optional[AwsApiClient] = None) -> bool:
        ...
        print_info(f"Starting {self.get_resource_type()}: {
                   self.get_resource_name()}")
        try:
            # Get the service_client
            ...
            try:
                start_crawler_response = service_client.start_crawler(
                    Name=self.name)
                ...
                print_info(f"{self.get_resource_type()}: {
                           self.get_resource_name()} already running")
                return True

            if start_crawler_response is not None:
                print_info(f"{self.get_resource_type()}: {
                           self.get_resource_name()} started")
                return True
