...
class DbInstance(AwsResource):
    ...
    def get_master_username(self, aws_client: Optional[AwsApiClient] = None) -> Optional[str]:
        ...
        if master_username is None and self.secrets_file is not None:
            ...
            if secret_data is not None:
                master_username = secret_data.get(
                    "MASTER_USERNAME", master_username)
        if master_username is None and self.aws_secret is not None:
            # read from aws_secret
            logger.debug(f"Reading MASTER_USERNAME from secret: {
                         self.aws_secret.name}")
    ...
    def get_master_user_password(self, aws_client: Optional[AwsApiClient] = None) -> Optional[str]:
        ...
        if master_user_password is None and self.aws_secret is not None:
            # read from aws_secret
            logger.debug(f"Reading MASTER_USER_PASSWORD from secret: {
                         self.aws_secret.name}")
    ...
    def get_db_name(self, aws_client: Optional[AwsApiClient] = None) -> Optional[str]:
        ...
        if db_name is None and self.aws_secret is not None:
            # read from aws_secret
            logger.debug(f"Reading DB_NAME from secret: {
                         self.aws_secret.name}")
            db_name = self.aws_secret.get_secret_value(
                "DB_NAME", aws_client=aws_client)
            if db_name is None:
                logger.debug(f"Reading DATABASE_NAME from secret: {
                             self.aws_secret.name}")
                db_name = self.aws_secret.get_secret_value(
    ...
    def _create(self, aws_client: AwsApiClient) -> bool:
        ...
        """
        print_info(f"Creating {self.get_resource_type()}: {
        ...
        try:
            ...
            if resource_dict is not None:
                logger.debug(f"DBInstance created: {
                             self.get_db_instance_identifier()}")
    ...
    def post_create(self, aws_client: AwsApiClient) -> bool:
        ...
        if self.wait_for_create:
            try:
                print_info(f"Waiting for {
                           self.get_resource_type()} to be active.")
                waiter = self.get_service_client(
                    aws_client).get_waiter("db_instance_available")
                waiter.wait(
                    DBInstanceIdentifier=self.get_db_instance_identifier(),
                    WaiterConfig={
    ...
    def _read(self, aws_client: AwsApiClient) -> Optional[Any]:
        ...
        Args:
            aws_client: The AwsApiClient for the current cluster
        """
        logger.debug(f"Reading {self.get_resource_type()}: {
                     self.get_resource_name()}")
    ...
    def _delete(self, aws_client: AwsApiClient) -> bool:
        ...
        Args:
            aws_client: The AwsApiClient for the current cluster
        """
        print_info(f"Deleting {self.get_resource_type()}: {
                   self.get_resource_name()}")
        ...
        try:
            ...
            # Validate resource creation
            if resource_dict is not None:
                logger.debug(f"DBInstance deleted: {
                             self.get_db_instance_identifier()}")
                self.active_resource = resource_dict
                return True
        except Exception as e:
    ...
    def post_delete(self, aws_client: AwsApiClient) -> bool:
        ...
        if self.wait_for_delete:
            try:
                print_info(f"Waiting for {
                           self.get_resource_type()} to be deleted.")
                waiter = self.get_service_client(
                    aws_client).get_waiter("db_instance_deleted")
                waiter.wait(
                    DBInstanceIdentifier=self.get_db_instance_identifier(),
                    WaiterConfig={
                        "Delay": self.waiter_delay,
    ...
    def _update(self, aws_client: AwsApiClient) -> bool:
        ...
        print_info(f"Updating {self.get_resource_type()}: {
                   self.get_resource_name()}")
        ...
        existing_db_subnet_group_name = db_instance.get(
    ...
    def get_db_port(self, aws_client: Optional[AwsApiClient] = None) -> Optional[str]:
        ...
        if self.active_resource:
            _db_port = self.active_resource.get(
                "Endpoint", {}).get("Port", None)
        if _db_port is None:
            client: AwsApiClient = aws_client or self.get_aws_client()
            resource = self._read(aws_client=client)
            if resource is not None:
