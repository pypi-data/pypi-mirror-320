...
class SecurityGroup(AwsResource):
    ...
    def _create(self, aws_client: AwsApiClient) -> bool:
        ...
        # Build description
        description = self.description or "Created by ethosian"
        ...
        if vpc_id is None and self.subnets is not None:
            from ethosian.aws.resource.ec2.subnet import get_vpc_id_from_subnet_ids
        ...
        if self.wait_for_create:
            try:
                print_info(f"Waiting for {
                           self.get_resource_type()} to be created.")
                waiter = self.get_service_client(
                    aws_client).get_waiter("security_group_exists")
                waiter.wait(
                    Filters=[
                        {
                            "Name": "group-name",
        ...
        try:
            ...
            if ce_resp is not None:
                if ce_resp.get("Error", {}).get("Code", "") == "DependencyViolation":
                    logger.warning(
                        f"SecurityGroup {
                            self.get_resource_name()} could not be deleted "
                        f"as it is being used by another resource."
                    )
                    if ce_resp.get("Error", {}).get("Message", "") != "":
                        logger.warning(f"Error: {ce_resp.get(
                            'Error', {}).get('Message', '')}")
                    logger.warning(
    ...
    def add_inbound_rules(self, aws_client: AwsApiClient) -> bool:
        ...
        if group_id is None:
            logger.warning(
                f"GroupId for {self.get_resource_name()} not found.")
            return False
        ...
        if self.ingress_ip_permissions is not None:
            try:
                ...
                if response is None or response.get("Return") is False:
                    logger.error(f"Ingress rules could not be added to {
                                 self.get_resource_name()}")
                    return False
            except ClientError as ce:
                ce_resp = ce.response
            ...
            for rule in self.inbound_rules:
                ...
                if rule.cidr_ip is not None:
                    ...
                    try:
                        ...
                        logger.warning(f"Error getting cidr_ip for {
                                       self.get_resource_name()}: {e}")
                ...
                if rule.cidr_ipv6 is not None:
                    ...
                    try:
                        ...
                        logger.warning(f"Error getting cidr_ipv6 for {
                                       self.get_resource_name()}: {e}")
    ...
    def add_outbound_rules(self, aws_client: AwsApiClient) -> bool:
        ...
        if self.egress_ip_permissions is not None:
            try:
                ...
                # Validate the response
                if response is None or response.get("Return") is False:
                    logger.error(f"Egress rules could not be added to {
                                 self.get_resource_name()}")
                    return False
            except ClientError as ce:
                ce_resp = ce.response
