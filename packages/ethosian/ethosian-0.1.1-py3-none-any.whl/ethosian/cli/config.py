...
from ethosian.cli.console import print_heading, print_info
from ethosian.cli.settings import ethosian_cli_settings
from ethosian.api.schemas.user import UserSchema
from ethosian.api.schemas.team import TeamSchema
from ethosian.api.schemas.workspace import WorkspaceSchema
...
class ethosianCliConfig:
    """The ethosianCliConfig class manages user data for the ethosian cli"""
    ...
    def __init__(
        ...
        # Active ws dir - used as the default for `ethosian` commands
        ...
        self.ws_config_map: Dict[str,
                                 WorkspaceConfig] = ws_config_map or OrderedDict()
    ...
    def user(self, user: Optional[UserSchema]) -> None:
        ...
        if user is not None:
            ...
            clear_user_cache = (
                ...
                and (user.email != self._user.email or user.id_user != self._user.id_user)
            )
            self._user = user
            if clear_user_cache:
    ...
    def clear_user_cache(self) -> None:
        ...
        ethosian_cli_settings.ai_conversations_path.unlink(missing_ok=True)
        logger.info(
            "Workspaces cleared, please setup again using `ethosian ws setup`")
    ...
    def _add_or_update_ws_config(
        ...
        if existing_ws_config is None:
            logger.error(f"Could not find workspace at: {
                         ws_root_str}, please run `ethosian ws setup`")
            return None
    ...
    def add_new_ws_to_config(
        ...
        """Adds a newly created workspace to the ethosianCliConfig"""
    ...
    def delete_ws(self, ws_root_path: Path) -> None:
        """Handles Deleting a workspace from the ethosianCliConfig and api"""

        ws_root_str = str(ws_root_path)
        print_heading(f"Deleting record for workspace: {ws_root_str}")

        ws_config: Optional[WorkspaceConfig] = self.ws_config_map.pop(
            ws_root_str, None)
        ...
        print_info(
            "Note: this does not delete any data locally or from ethosian.app, please delete them manually\n")
    ...
    def save_config(self):
        ...
        write_json_file(
            file_path=ethosian_cli_settings.config_file_path, data=config_data)

    @classmethod
    def from_saved_config(cls) -> Optional["ethosianCliConfig"]:
        try:
            config_data = read_json_file(
                file_path=ethosian_cli_settings.config_file_path)
            ...
            user_schema = UserSchema.model_validate(
                user_dict) if user_dict else None
            ...
            logger.warning(e)
            logger.warning(
                "Please setup the workspace using `ethosian ws setup`")
            return None

    ######################################################
    # Print ethosianCliConfig
    ######################################################
    ...
    def print_to_cli(self, show_all: bool = False):
        ...
        if self.active_ws_dir:
            print_heading(f"Active workspace directory: {
                          self.active_ws_dir}\n")
        else:
            print_info("No active workspace found.")
            print_info(
                "Please create a workspace using `ethosian ws create` " "or setup existing workspace using `ethosian ws setup`"
            )
