...
class Metrics:
    ...
    def log(self):
        ...
        logger.debug(f"* Tokens per second:           {
                     self.output_tokens / self.response_timer.elapsed:.4f} tokens/s")
        logger.debug(
        ...
        if self.prompt_tokens_details is not None:
            logger.debug(
                f"* Prompt tokens details:       {self.prompt_tokens_details}")
...
class HuggingFaceChat(Model):
    ...
    def get_client_params(self) -> Dict[str, Any]:
        ...
        if not self.api_key:
            logger.error(
                "HF_TOKEN not set. Please set the HF_TOKEN environment variable.")
    ...
    def _update_usage_metrics(
        ...
        if response_usage:
            ...
            if completion_tokens is not None:
                ...
                self.metrics["completion_tokens"] = self.metrics.get(
            ...
            if response_usage.prompt_tokens_details is not None:
                ...
                if metrics.prompt_tokens_details is not None:
                    for k, v in metrics.prompt_tokens_details.items():
                        self.metrics.get(
                            "prompt_tokens_details", {}).get(k, 0) + v
            if response_usage.completion_tokens_details is not None:
                ...
                if metrics.completion_tokens_details is not None:
                    for k, v in metrics.completion_tokens_details.items():
                        self.metrics.get(
                            "completion_tokens_details", {}).get(k, 0) + v
    ...
    def _update_stream_metrics(self, assistant_message: Message, metrics: Metrics):
        ...
        if metrics.input_tokens is not None:
            ...
            self.metrics["input_tokens"] = self.metrics.get(
                "input_tokens", 0) + metrics.input_tokens
        if metrics.output_tokens is not None:
            assistant_message.metrics["output_tokens"] = metrics.output_tokens
            self.metrics["output_tokens"] = self.metrics.get(
                "output_tokens", 0) + metrics.output_tokens
        if metrics.prompt_tokens is not None:
            assistant_message.metrics["prompt_tokens"] = metrics.prompt_tokens
            self.metrics["prompt_tokens"] = self.metrics.get(
                "prompt_tokens", 0) + metrics.prompt_tokens
        if metrics.completion_tokens is not None:
            assistant_message.metrics["completion_tokens"] = metrics.completion_tokens
            self.metrics["completion_tokens"] = self.metrics.get(
                "completion_tokens", 0) + metrics.completion_tokens
        if metrics.total_tokens is not None:
            assistant_message.metrics["total_tokens"] = metrics.total_tokens
            self.metrics["total_tokens"] = self.metrics.get(
                "total_tokens", 0) + metrics.total_tokens
        if metrics.prompt_tokens_details is not None:
            assistant_message.metrics["prompt_tokens_details"] = metrics.prompt_tokens_details
            for k, v in metrics.prompt_tokens_details.items():
    ...
    def response_stream(self, messages: List[Message]) -> Iterator[ModelResponse]:
        ...
        for response in self.invoke_stream(messages=messages):
            if len(response.choices) > 0:
                ...
                response_tool_calls: Optional[List[ChatCompletionStreamOutputDeltaToolCall]
        ...
        if stream_data.response_tool_calls is not None:
            _tool_calls = self._build_tool_calls(
                stream_data.response_tool_calls)
    ...
    async def aresponse_stream(self, messages: List[Message]) -> Any:
        ...
        """
        logger.debug(
            "---------- HuggingFace Hub Async Response Start ----------")
        self._log_messages(messages)
        stream_data: StreamData = StreamData()
        ...
        if assistant_message.tool_calls is not None and len(assistant_message.tool_calls) > 0 and self.run_tools:
            ...
            async for model_response in self.aresponse_stream(messages=messages):
                yield model_response
        logger.debug(
            "---------- HuggingFace Hub Async Response End ----------")
