...
class OpenBBTools(Toolkit):
    def __init__(
        ...
        provider: Literal["benzinga", "fmp", "intrinio",
                          "polygon", "tiingo", "tmx", "yfinance"] = "yfinance",
        ...
        try:
            if openbb_pat or getenv("OPENBB_PAT"):
                self.obb.account.login(pat=openbb_pat or getenv(
                    "OPENBB_PAT"))  # type: ignore
        except Exception as e:
            logger.error(f"Error logging into OpenBB: {e}")

        self.provider: Literal["benzinga", "fmp", "intrinio",
                               "polygon", "tiingo", "tmx", "yfinance"] = provider
    ...
    def get_stock_price(self, symbol: str) -> str:
        ...
        try:
            result = self.obb.equity.price.quote(
                # type: ignore
                symbol=symbol, provider=self.provider).to_polars()
            clean_results = []
    ...
    def search_company_symbol(self, company_name: str) -> str:
        ...
        result = self.obb.equity.search(
            company_name).to_polars()  # type: ignore
        clean_results = []
        if len(result) > 0:
            for row in result.to_dicts():
                clean_results.append(
    ...
    def get_company_news(self, symbol: str, num_stories: int = 10) -> str:
        ...
        try:
            result = self.obb.news.company(
                # type: ignore
                symbol=symbol, provider=self.provider, limit=num_stories).to_polars()
            clean_results = []
            if len(result) > 0:
                for row in result.to_dicts():
                    row.pop("images")
