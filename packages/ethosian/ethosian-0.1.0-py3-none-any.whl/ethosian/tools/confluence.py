...
class ConfluenceTools(Toolkit):
    def __init__(
        ...
        self.password = api_key or getenv(
            "CONFLUENCE_API_KEY") or password or getenv("CONFLUENCE_PASSWORD")
    ...
    def get_page_content(self, space_name: str, page_title: str, expand: Optional[str] = "body.storage"):
        ...
        try:
            ...
            page = self.confluence.get_page_by_title(
                key, page_title, expand=expand)
            if page:
                logger.info(f"Successfully retrieved page '{
                            page_title}' from space '{space_name}'")
                return json.dumps(page)

            logger.warning(
                f"Page '{page_title}' not found in space '{space_name}'")
