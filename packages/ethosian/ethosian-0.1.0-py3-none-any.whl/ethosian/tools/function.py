...
class Function(BaseModel):
    ...
    def from_callable(cls, c: Callable, strict: bool = False) -> "Function":
        ...
        from ethosian.utils.json_schema import get_json_schema
        ...
        try:
            ...
            if docstring := getdoc(c):
                ...
                if param_docs is not None:
                    for param in param_docs:
                        ...
                        param_descriptions[param_name] = f"({param_type}) {
                            param.description}"
            ...
            logger.warning(f"Could not parse args for {
    ...
    def process_entrypoint(self, strict: bool = False):
        ...
        self.description = self.description or get_entrypoint_docstring(
            self.entrypoint)
...
class FunctionCall(BaseModel):
    ...
    def get_call_str(self) -> str:
        ...
        call_str = f"{self.function.name}({', '.join(
            [f'{k}={v}' for k, v in trimmed_arguments.items()])})"
        return call_str
