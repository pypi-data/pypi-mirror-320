...
from ethosian.embedder import Embedder
from ethosian.vectordb.base import VectorDb
from ethosian.vectordb.distance import Distance
from ethosian.vectordb.search import SearchType
from ethosian.vectordb.pgvector.index import Ivfflat, HNSW
from ethosian.utils.log import logger
from ethosian.reranker.base import Reranker
...
class PgVector(VectorDb):
    ...
    def __init__(
        ...
        if db_engine is None and db_url is None:
            raise ValueError(
                "Either 'db_url' or 'db_engine' must be provided.")

        if db_engine is None:
            if db_url is None:
                raise ValueError(
                    "Must provide 'db_url' if 'db_engine' is None.")
            try:
        ...
        logger.debug(f"Initialized PgVector with table '{
    ...
    def get_table_v1(self) -> Table:
        ...
        table = Table(
            ...
            Column("filters", postgresql.JSONB, server_default=text(
                "'{}'::jsonb"), nullable=True),
    ...
    def get_table(self) -> Table:
        ...
        if self.schema_version == 1:
            ...
            raise NotImplementedError(f"Unsupported schema version: {
                                      self.schema_version}")
        ...
        try:
            with self.Session() as sess:
                for i in range(0, len(documents), batch_size):
                    ...
                    logger.debug(f"Processing batch starting at index {
                                 i}, size: {len(batch_docs)}")
                    try:
                        ...
                        logger.info(f"Inserted batch of {
                                    len(batch_records)} documents.")
                    except Exception as e:
                        logger.error(
                            f"Error with batch starting at index {i}: {e}")
                ...
                for i in range(0, len(documents), batch_size):
                    ...
                    try:
                        ...
                        insert_stmt = postgresql.insert(
                            self.table).values(batch_records)
                        ...
                        sess.commit()  # Commit batch independently
                        logger.info(f"Upserted batch of {
                                    len(batch_records)} documents.")
                    except Exception as e:
                        logger.error(
                            f"Error with batch starting at index {i}: {e}")
                ...
                with self.Session() as sess, sess.begin():
                    if self.vector_index is not None:
                        if isinstance(self.vector_index, Ivfflat):
                            sess.execute(text(f"SET LOCAL ivfflat.probes = {
                                         self.vector_index.probes}"))
                        elif isinstance(self.vector_index, HNSW):
                            sess.execute(text(f"SET LOCAL hnsw.ef_search = {
                                         self.vector_index.ef_search}"))
    ...
    def keyword_search(self, query: str, limit: int = 5, filters: Optional[Dict[str, Any]] = None) -> List[Document]:
        ...
        try:
            ...
            ts_vector = func.to_tsvector(
                self.content_language, self.table.c.content)
            # Create the ts_query using websearch_to_tsquery with parameter binding
            processed_query = self.enable_prefix_matching(
                query) if self.prefix_match else query
    ...
    def hybrid_search(
        ...
        try:
            ...
            if self.distance == Distance.l2:
                ...
                vector_distance = self.table.c.embedding.l2_distance(
                    query_embedding)
            ...
            hybrid_score = (self.vector_score_weight *
                            vector_score) + (text_rank_weight * text_rank)
    ...
    def drop(self) -> None:
        ...
        if self.table_exists():
            try:
                ...
                logger.info(
                    f"Table '{self.table.fullname}' dropped successfully.")
            except Exception as e:
                logger.error(f"Error dropping table '{
                             self.table.fullname}': {e}")
                raise
        else:
    ...
    def get_count(self) -> int:
        ...
        try:
            with self.Session() as sess, sess.begin():
                stmt = select(func.count(self.table.c.name)
                              ).select_from(self.table)
                result = sess.execute(stmt).scalar()
                return int(result) if result is not None else 0
        except Exception as e:
            logger.error(f"Error getting count from table '{
                         self.table.fullname}': {e}")
            return 0
    ...
    def _drop_index(self, index_name: str) -> None:
        ...
        try:
            with self.Session() as sess, sess.begin():
                drop_index_sql = f'DROP INDEX IF EXISTS "{
    ...
    def _create_vector_index(self, force_recreate: bool = False) -> None:
        ...
        if self.vector_index is None:
            logger.debug(
                "No vector index specified, skipping vector index optimization.")
            return

        # Generate index name if not provided
        if self.vector_index.name is None:
            index_type = "ivfflat" if isinstance(
                self.vector_index, Ivfflat) else "hnsw"
        ...
        if vector_index_exists:
            logger.info(f"Vector index '{
                        self.vector_index.name}' already exists.")
            if force_recreate:
                logger.info(f"Force recreating vector index '{
                            self.vector_index.name}'. Dropping existing index.")
                self._drop_index(self.vector_index.name)
            else:
                logger.info(f"Skipping vector index creation as index '{
                            self.vector_index.name}' already exists.")
                return
        ...
        try:
            with self.Session() as sess, sess.begin():
                ...
                if self.vector_index.configuration:
                    logger.debug(f"Setting configuration: {
                                 self.vector_index.configuration}")
                    for key, value in self.vector_index.configuration.items():
                        sess.execute(text(f"SET {key} = :value;"), {
                                     "value": value})

                if isinstance(self.vector_index, Ivfflat):
                    self._create_ivfflat_index(
                        sess, table_fullname, index_distance)
                elif isinstance(self.vector_index, HNSW):
                    self._create_hnsw_index(
                        sess, table_fullname, index_distance)
                else:
                    logger.error(f"Unknown index type: {
                                 type(self.vector_index)}")
                    return
        except Exception as e:
            logger.error(f"Error creating vector index '{
                         self.vector_index.name}': {e}")
            raise
    ...
    def _create_ivfflat_index(self, sess: Session, table_fullname: str, index_distance: str) -> None:
        ...
        # Set ivfflat.probes
        sess.execute(text("SET ivfflat.probes = :probes;"),
                     {"probes": self.vector_index.probes})

        logger.debug(
            f"Creating Ivfflat index '{
                self.vector_index.name}' on table '{table_fullname}' with "
    ...
    def _create_hnsw_index(self, sess: Session, table_fullname: str, index_distance: str) -> None:
        ...
        logger.debug(
            f"Creating HNSW index '{self.vector_index.name}' on table '{
                table_fullname}' with "
            f"m: {self.vector_index.m}, ef_construction: {
                self.vector_index.ef_construction}, "
            f"and distance metric: {index_distance}"
        )
    ...
    def _create_gin_index(self, force_recreate: bool = False) -> None:
        ...
        if gin_index_exists:
            ...
            if force_recreate:
                logger.info(f"Force recreating GIN index '{
                            gin_index_name}'. Dropping existing index.")
                self._drop_index(gin_index_name)
            else:
                logger.info(f"Skipping GIN index creation as index '{
                            gin_index_name}' already exists.")
                return

        # Proceed to create GIN index
        try:
            with self.Session() as sess, sess.begin():
                logger.debug(f"Creating GIN index '{gin_index_name}' on table '{
                             self.table.fullname}'.")
                # Create index
                create_gin_index_sql = text(
                    f'CREATE INDEX "{gin_index_name}" ON {
                        self.table.fullname} '
                    f"USING GIN (to_tsvector({
    ...
    def delete(self) -> bool:
        ...
        try:
            ...
            logger.error(f"Error deleting rows from table '{
                         self.table.fullname}': {e}")
            sess.rollback()
            return False
