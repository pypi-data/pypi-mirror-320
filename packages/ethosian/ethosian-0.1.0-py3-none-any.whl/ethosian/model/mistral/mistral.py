...
from ethosian.model.base import Model
from ethosian.model.message import Message
from ethosian.model.response import ModelResponse
from ethosian.tools.function import FunctionCall
from ethosian.utils.log import logger
from ethosian.utils.timer import Timer
from ethosian.utils.tools import get_function_call_for_tool_call
...
MistralMessage = Union[models.UserMessage, models.AssistantMessage,
                       models.SystemMessage, models.ToolMessage]
...
class MistralChat(Model):
    ...
    def invoke(self, messages: List[Message]) -> ChatCompletionResponse:
        ...
        for m in messages:
            ...
            if m.role == "user":
                mistral_message = models.UserMessage(
                    role=m.role, content=m.content)
            elif m.role == "assistant":
                if m.tool_calls is not None:
                    mistral_message = models.AssistantMessage(
                        role=m.role, content=m.content, tool_calls=m.tool_calls)
                else:
                    mistral_message = models.AssistantMessage(
                        role=m.role, content=m.content)
            elif m.role == "system":
                mistral_message = models.SystemMessage(
                    role=m.role, content=m.content)
            elif m.role == "tool":
                mistral_message = models.ToolMessage(
                    name=m.name, content=m.content, tool_call_id=m.tool_call_id)
            else:
    ...
    def invoke_stream(self, messages: List[Message]) -> Iterator[Any]:
        ...
        for m in messages:
            ...
            if m.role == "user":
                ...
                raise ValueError(f"Unknown role: {m.role}")
            mistral_messages.append(mistral_message)
        logger.debug(f"Mistral messages sending to stream endpoint: {
                     mistral_messages}")
        ...
        if assistant_message.tool_calls is not None and len(assistant_message.tool_calls) > 0:
            ...
            for tool_call in assistant_message.tool_calls:
                ...
                if _function_call is None:
                    messages.append(
                        Message(role="tool", tool_call_id=_tool_call_id,
    ...
    def response(self, messages: List[Message]) -> ModelResponse:
        ...
        response_timer.stop()
        logger.debug(f"Time to generate response: {
                     response_timer.elapsed:.4f}s")
    ...
    def _update_stream_metrics(self, stream_data: StreamData, assistant_message: Message):
        ...
        self.metrics["response_times"].append(
            stream_data.response_timer.elapsed)
        if stream_data.time_to_first_token is not None:
            if "time_to_first_token" not in self.metrics:
                self.metrics["time_to_first_token"] = []
            self.metrics["time_to_first_token"].append(
                stream_data.time_to_first_token)
        ...
        self.metrics["prompt_tokens"] = self.metrics.get(
            "prompt_tokens", 0) + stream_data.response_prompt_tokens
        self.metrics["input_tokens"] = self.metrics.get(
            "input_tokens", 0) + stream_data.response_prompt_tokens
        ...
        self.metrics["completion_tokens"] = (
            self.metrics.get("completion_tokens", 0) +
            stream_data.response_completion_tokens
        )
        self.metrics["output_tokens"] = self.metrics.get(
            "output_tokens", 0) + stream_data.response_completion_tokens

        assistant_message.metrics["total_tokens"] = stream_data.response_total_tokens
        self.metrics["total_tokens"] = self.metrics.get(
            "total_tokens", 0) + stream_data.response_total_tokens
    ...
    def response_stream(self, messages: List[Message]) -> Iterator[ModelResponse]:
        ...
        for response in self.invoke_stream(messages=messages):
            ...
            if response_content is not None:
                ...
                if stream_data.completion_tokens == 1:
                    ...
                    logger.debug(f"Time to first token: {
                                 stream_data.time_to_first_token:.4f}s")
        ...
        if completion_tokens > 0:
            logger.debug(f"Time per output token: {
                         stream_data.response_timer.elapsed / completion_tokens:.4f}s")
            logger.debug(f"Throughput: {
                         completion_tokens / stream_data.response_timer.elapsed:.4f} tokens/s")

        # -*- Create assistant message
        assistant_message = Message(
            role=(assistant_message_role or "assistant"))
        ...
        if stream_data.response_tool_calls is not None:
            assistant_message.tool_calls = [
                t.model_dump() for t in stream_data.response_tool_calls]
