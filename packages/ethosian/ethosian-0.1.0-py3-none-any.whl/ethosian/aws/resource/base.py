...
from ethosian.resource.base import ResourceBase
...
class AwsResource(ResourceBase):
    ...
    def _read(self, aws_client: AwsApiClient) -> Any:
        logger.warning(
            f"@_read method not defined for {self.get_resource_name()}")
        return True
    ...
    def _create(self, aws_client: AwsApiClient) -> bool:
        logger.warning(
            f"@_create method not defined for {self.get_resource_name()}")
        return True
    ...
    def create(self, aws_client: Optional[AwsApiClient] = None) -> bool:
        ...
        if self.use_cache and self.is_active(client):
            ...
            print_info(f"{self.get_resource_type()}: {
                       self.get_resource_name()} already exists")
        # Step 3: Create the resource
        else:
            self.resource_created = self._create(client)
            if self.resource_created:
                print_info(f"{self.get_resource_type()}: {
                           self.get_resource_name()} created")
        ...
        logger.error(f"Failed to create {self.get_resource_type()}: {
    ...
    def _update(self, aws_client: AwsApiClient) -> Any:
        logger.warning(
            f"@_update method not defined for {self.get_resource_name()}")
        return True
    ...
    def _delete(self, aws_client: AwsApiClient) -> Any:
        logger.warning(
            f"@_delete method not defined for {self.get_resource_name()}")
        return True
