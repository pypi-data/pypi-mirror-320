...
class LoadBalancer(AwsResource):
            ...
            for sg in self.security_groups:
                if isinstance(sg, SecurityGroup):
                    security_group_ids.append(
                        sg.get_security_group_id(aws_client))
                else:
    ...
    def post_create(self, aws_client: AwsApiClient) -> bool:
        ...
        if self.wait_for_create:
            try:
                ...
                waiter = self.get_service_client(
                    aws_client).get_waiter("load_balancer_exists")
                waiter.wait(
        ...
        if elb is None:
            logger.error(f"Error reading {
                         self.get_resource_type()}. Please get DNS name manually.")
        else:
            dns_name = elb.get("DNSName", None)
            print_info(f"LoadBalancer DNS: {
                       self.protocol.lower()}://{dns_name}")
        return True
    ...
    def _read(self, aws_client: AwsApiClient) -> Optional[Any]:
        ...
        service_client = self.get_service_client(aws_client)
        try:
            describe_response = service_client.describe_load_balancers(Names=[
                                                                       self.name])
    ...
    def post_delete(self, aws_client: AwsApiClient) -> bool:
        ...
        if self.wait_for_delete:
            try:
                ...
                waiter = self.get_service_client(
                    aws_client).get_waiter("load_balancers_deleted")
                waiter.wait(
                    Names=[self.get_resource_name()],
                    WaiterConfig={
                        "Delay": self.waiter_delay,
