...
class CloudFormationStack(AwsResource):
        ...
        if self.wait_for_create:
            try:
                ...
                waiter = self.get_service_client(
                    aws_client).get_waiter("stack_create_complete")
                waiter.wait(
                    StackName=self.name,
                    WaiterConfig={
    ...
    def post_delete(self, aws_client: AwsApiClient) -> bool:
        ...
        if self.wait_for_delete:
            try:
                ...
                waiter = self.get_service_client(
                    aws_client).get_waiter("stack_delete_complete")
                waiter.wait(
                    StackName=self.name,
                    WaiterConfig={
                        "Delay": self.waiter_delay,
    ...
    def get_stack_resource(self, aws_client: AwsApiClient, logical_id: str) -> Optional[Any]:
        ...
        try:
            service_resource = self.get_service_resource(aws_client)
            stack_resource = service_resource.StackResource(
    ...
    def get_stack_resource_physical_id(self, stack_resource: Any) -> Optional[str]:
        ...
        try:
            ...
            logger.debug(f"{stack_resource.logical_id}: {
