...
from ethosian.utils.py_io import get_python_objects_from_module
...
def get_workspace_objects_from_file(resource_file: Path) -> dict:
    ...
    try:
        ...
        if not aws_resources_available and create_default_aws_resources:
            from ethosian.aws.resources import AwsResource, AwsApp
...
class WorkspaceConfig(BaseModel):
    """The WorkspaceConfig stores data for a ethosian workspace."""
    ...
    def workspace_dir_path(self) -> Optional[Path]:
        if self._workspace_dir_path is None:
            if self.ws_root_path is not None:
                ...
                self._workspace_dir_path = get_workspace_dir_path(
                    self.ws_root_path)
    ...
    def validate_workspace_settings(self, obj: Any) -> bool:
        if not isinstance(obj, WorkspaceSettings):
            raise Exception(
                "WorkspaceSettings must be of type WorkspaceSettings")

        if self.ws_root_path is not None and obj.ws_root is not None:
            if obj.ws_root != self.ws_root_path:
                raise Exception(f"WorkspaceSettings.ws_root ({
                                obj.ws_root}) must match {self.ws_root_path}")
    ...
    def set_local_env(self) -> None:
        ...
        if self.ws_root_path is not None:
            ...
            if self.workspace_settings is not None:
                environ[WORKSPACE_NAME_ENV_VAR] = str(
        ...
        if self.ws_schema is not None:
            if self.ws_schema.id_workspace is not None:
                environ[WORKSPACE_ID_ENV_VAR] = str(
                    self.ws_schema.id_workspace)
    ...
    def get_resources(
        ...
        if workspace_dir_path is not None:
            ...
            logger.debug(
                f"--^^-- Loading workspace from: {workspace_dir_path}")
            ...
            for resource_file in resource_files:
                ...
                resource_file_parts_after_ws = resource_file_parts[len(
                    workspace_dir_path_parts):]
                # Check if file in ignored directory
                if any([ignored_dir in resource_file_parts_after_ws for ignored_dir in ignored_dirs]):
                    logger.debug(f"Skipping file in ignored directory: {
                                 resource_file}")
                    continue
                logger.debug(f"Reading file: {resource_file}")
                try:
                    python_objects = get_python_objects_from_module(
                        resource_file)
                    # logger.debug(f"python_objects: {python_objects}")
                    for obj_name, obj in python_objects.items():
                        if isinstance(
        ...
        logger.debug(f"Getting resources for env: {
                     env} | infra: {infra} | order: {order}")
        if infra is None:
        ...
        if self._workspace_settings is not None:
            for resource_group in env_filtered_resource_groups:
                logger.debug(f"Setting workspace settings for {
                             resource_group.__class__.__name__}")
    ...
    def get_resources_from_file(
        ...
        temporary_ws_config = WorkspaceConfig(
            ws_root_path=resource_file_parent_dir)
