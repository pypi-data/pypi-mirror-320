...
from ethosian.memory.row import MemoryRow
    ...
    def update_model(self) -> None:
        if self.model is None:
            try:
                ...
                logger.exception(e)
                logger.error(
                    "ethosian uses `openai` as the default model provider. "
        ...
        try:
            if self.db:
                self.db.upsert_memory(
                    MemoryRow(user_id=self.user_id, memory=Memory(
                        memory=memory, input=self.input_message).to_dict())
                )
    ...
    def update_memory(self, id: str, memory: str) -> str:
        ...
        try:
            if self.db:
                self.db.upsert_memory(
                    MemoryRow(
                        id=id, user_id=self.user_id, memory=Memory(
                            memory=memory, input=self.input_message).to_dict()
                    )
                )
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        user_prompt_message = Message(
            role="user", content=message, **kwargs) if message else None
