...
from ethosian.memory.classifier import MemoryClassifier
from ethosian.memory.db import MemoryDb
from ethosian.memory.manager import MemoryManager
from ethosian.memory.memory import Memory
from ethosian.memory.summary import SessionSummary
from ethosian.memory.summarizer import MemorySummarizer
...
class AgentMemory(BaseModel):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.memories:
            _memory_dict["memories"] = [memory.to_dict()
    ...
    def add_system_message(self, message: Message, system_message_role: str = "system") -> None:
        ...
        # If it is, update the system message if content has changed and update_system_message_on_change is True
            ...
            system_message_index = next((i for i, m in enumerate(
                self.messages) if m.role == system_message_role), None)
            ...
            if system_message_index is not None:
                if (
                    ...
                    logger.info(
                        "Updating system message in memory with new content")
    ...
    def get_messages_from_last_n_runs(
        ...
        if last_n is None:
            ...
            for prev_run in self.runs:
                if prev_run.response and prev_run.response.messages:
                    if skip_role:
                        prev_run_messages = [
                            m for m in prev_run.response.messages if m.role != skip_role]
                    else:
                        prev_run_messages = prev_run.response.messages
                    messages_from_all_history.extend(prev_run_messages)
            logger.debug(f"Messages from previous runs: {
                         len(messages_from_all_history)}")
        ...
        logger.debug(f"Messages from last {last_n} runs: {
                     len(messages_from_last_n_history)}")
        ...
        try:
            if self.retrieval in (MemoryRetrieval.last_n, MemoryRetrieval.first_n):
                ...
                raise NotImplementedError(
                    "Semantic retrieval not yet supported.")
        ...
        for field_name, field_value in self.__dict__.items():
            if field_name not in ["db", "classifier", "manager", "summarizer"]:
                try:
                    ...
                    logger.warning(f"Failed to deepcopy field: {
                                   field_name} - {e}")
