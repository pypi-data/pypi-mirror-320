...
class CohereEmbedder(Embedder):
    ...
    def get_embedding(self, text: str) -> List[float]:
        response: Union[EmbeddingsFloatsEmbedResponse,
                        EmbeddingsByTypeEmbedResponse] = self.response(text=text)
        try:
    ...
    def get_embedding_and_usage(self, text: str) -> Tuple[List[float], Optional[Dict[str, Any]]]:
        ...
        if isinstance(response, EmbeddingsFloatsEmbedResponse):
            ...
            embedding = response.embeddings.float_[
                0] if response.embeddings.float_ else []
