...
from ethosian.app.base import AppBase
from ethosian.app.context import ContainerContext
from ethosian.docker.app.context import DockerBuildContext
from ethosian.utils.log import logger

if TYPE_CHECKING:
    from ethosian.docker.resource.base import DockerResource
...
class DockerApp(AppBase):
    ...
    # -*- ethosian Volume
    # Mount ~/.ethosian directory from host machine to the container
    mount_ethosian_config: bool = True
        ...
        container_env.update(
            {
                ...
                ethosian_RUNTIME_ENV_VAR: "docker",
    ...
    def get_container_volumes(self, container_context: ContainerContext) -> Dict[str, dict]:
        ...
        if self.create_volume:
            ...
            if self.volume_dir is not None:
                volume_host = str(
                    self.workspace_root.joinpath(self.volume_dir))
        ...
        if self.mount_resources:
            resources_dir_path = str(
                self.workspace_root.joinpath(self.resources_dir))
        ...
        # Add ~/.ethosian as a volume
        if self.mount_ethosian_config:
            ethosian_config_host_path = str(Path.home().joinpath(".ethosian"))
            ethosian_config_container_path = f"{
                self.workspace_dir_container_path}/.ethosian"
            logger.debug(f"Mounting: {ethosian_config_host_path}")
            logger.debug(f"      to: {ethosian_config_container_path}")
            container_volumes[ethosian_config_host_path] = {
                "bind": ethosian_config_container_path,
                "mode": "ro",
            }
    ...
    def build_resources(self, build_context: DockerBuildContext) -> List["DockerResource"]:
        from ethosian.docker.resource.base import DockerResource
        from ethosian.docker.resource.network import DockerNetwork
        ...
        docker_container = DockerContainer(
            ...
            command=" ".join(
                container_cmd) if container_cmd is not None else None,
