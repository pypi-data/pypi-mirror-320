...
from ethosian.app.group import AppGroup
from ethosian.resource.group import ResourceGroup
from ethosian.docker.app.base import DockerApp
from ethosian.docker.app.context import DockerBuildContext
from ethosian.docker.api_client import DockerApiClient
...
class DockerResources(InfraResources):
    ...
    def create_resources(
        ...
        from ethosian.cli.console import print_info, print_heading, confirm_yes_no
        from ethosian.docker.resource.types import DockerContainer, DockerResourceInstallOrder
        ...
        for resource in final_docker_resources:
            ...
            try:
                ...
                logger.error(f"Failed to create {resource.get_resource_type()}: {
        ...
        if num_resources_to_create != num_resources_created:
            logger.error(
                f"Resources created: {num_resources_created} do not match resources required: {
                    num_resources_to_create}"
            )  # noqa: E501
        return num_resources_created, num_resources_to_create
    ...
    def delete_resources(
        ...
        # Sort the DockerResources in install order
        resources_to_delete.sort(key=lambda x: DockerResourceInstallOrder.get(
            x.__class__.__name__, 5000), reverse=True)
        ...
        for resource in final_docker_resources:
            ...
            try:
                ...
                logger.error(f"Failed to delete {resource.get_resource_type()}: {
                             resource.get_resource_name()}")
                logger.error(e)
                logger.error("Please fix and try again...")
        ...
        if num_resources_to_delete != num_resources_deleted:
            logger.error(
                f"Resources deleted: {num_resources_deleted} do not match resources required: {
                    num_resources_to_delete}"
            )  # noqa: E501
        return num_resources_deleted, num_resources_to_delete
    ...
    def update_resources(
        ...
        # Sort the DockerResources in install order
        resources_to_update.sort(key=lambda x: DockerResourceInstallOrder.get(
            x.__class__.__name__, 5000), reverse=True)
        ...
        for resource in final_docker_resources:
            ...
            # logger.debug(resource)
            try:
                _resource_updated = resource.update(
                    docker_client=self.docker_client)
                ...
                logger.error(f"Failed to update {resource.get_resource_type()}: {
                             resource.get_resource_name()}")
                logger.error(e)
                logger.error("Please fix and try again...")
        ...
        if num_resources_to_update != num_resources_updated:
            logger.error(
                f"Resources updated: {num_resources_updated} do not match resources required: {
                    num_resources_to_update}"
            )  # noqa: E501
        return num_resources_updated, num_resources_to_update
