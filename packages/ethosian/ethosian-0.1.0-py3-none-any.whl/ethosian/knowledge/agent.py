...
from ethosian.vectordb import VectorDb
...
class AgentKnowledge(AssistantKnowledge):
    ...
    In ethosian 3.0.0, AssistantKnowledge will be deprecated and this class will inherit directly from BaseModel.
    """
    ...
    def load(
        ...
        for document_list in self.document_lists:
            ...
            logger.info(f"Added {len(documents_to_load)
    ...
    def load_documents(
        ...
        # Filter out documents which already exist in the vector db
        documents_to_load = (
            [document for document in documents if not self.vector_db.doc_exists(
                document)]
            if skip_existing
            else documents
        )
        ...
        if len(documents_to_load) > 0:
            ...
            logger.info(f"Loaded {len(documents_to_load)
                                  } documents to knowledge base")
        else:
