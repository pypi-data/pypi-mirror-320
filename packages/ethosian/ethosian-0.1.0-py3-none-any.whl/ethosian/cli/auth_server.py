...
class CliAuthRequestHandler(BaseHTTPRequestHandler):
    ...
    TODO:
        * Fix the header and limit to only localhost or ethosian.com
    """
    ...
    def do_POST(self):
        ...
        ethosian_cli_settings.tmp_token_path.parent.mkdir(
            parents=True, exist_ok=True)
        ethosian_cli_settings.tmp_token_path.touch(exist_ok=True)
        ethosian_cli_settings.tmp_token_path.write_text(decoded_post_data)
...
def get_auth_token_from_web_flow(port: int) -> Optional[str]:
    ...
    if ethosian_cli_settings.tmp_token_path.exists() and ethosian_cli_settings.tmp_token_path.is_file():
        auth_token_str = ethosian_cli_settings.tmp_token_path.read_text()
        auth_token_json = json.loads(auth_token_str)
        ethosian_cli_settings.tmp_token_path.unlink()
