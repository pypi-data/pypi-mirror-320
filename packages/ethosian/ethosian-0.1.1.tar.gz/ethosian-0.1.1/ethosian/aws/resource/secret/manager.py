...
class SecretsManager(AwsResource):
    ...
    def _create(self, aws_client: AwsApiClient) -> bool:
        ...
        secret_string: Optional[str] = json.dumps(
            secret_dict) if len(secret_dict) > 0 else None
    ...
    def _read(self, aws_client: AwsApiClient) -> Optional[Any]:
        ...
        service_client = self.get_service_client(aws_client)
        try:
            describe_response = service_client.describe_secret(
                SecretId=self.name)
    ...
    def get_secrets_as_dict(self, aws_client: Optional[AwsApiClient] = None) -> Optional[Dict[str, Any]]:
        ...
        logger.debug(f"Getting {self.get_resource_type()}: {
                     self.get_resource_name()}")
