    ...
    def _create(self, aws_client: AwsApiClient) -> bool:
        ...
        if task_role_arn is None and self.create_task_role:
            ...
            try:
                ...
                logger.error(
                    "IamRole creation failed, please fix and try again")
                logger.error(e)
                return False
        ...
        if execution_role_arn is None and self.create_execution_role:
            ...
            try:
                ...
                print_info(f"ARN for {execution_role.name}: {
                           execution_role_arn}")
            except Exception as e:
                logger.error(
                    "IamRole creation failed, please fix and try again")
        ...
        if self.containers is not None:
            container_definitions = [c.get_container_definition(
                aws_client=aws_client) for c in self.containers]
            not_null_args["containerDefinitions"] = container_definitions
        if self.volumes is not None:
            volume_definitions = [v.get_volume_definition()
    ...
    def _delete(self, aws_client: AwsApiClient) -> bool:
        ...
        try:
            ...
            if task_definition_arns:
                ...
                print_info(f"EcsTaskDefinition deleted: {
                           self.get_resource_name()}")
                return True
        except Exception as e:
