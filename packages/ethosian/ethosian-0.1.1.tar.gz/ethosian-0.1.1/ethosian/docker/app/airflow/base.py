...
from ethosian.docker.app.base import DockerApp, ContainerContext  # noqa: F401
from ethosian.app.db_app import DbApp
...
class AirflowBase(DockerApp):
    ...
    # -*- Image Configuration
    image_name: str = "ethosian/airflow"
    image_tag: str = "2.7.1"
    ...
    def get_container_env(self, container_context: ContainerContext) -> Dict[str, str]:
        ...
        container_env.update(
            {
                ...
                # INIT_AIRFLOW env var is required for ethosian to generate DAGs from workflows
        ...
        if self.set_python_path:
            ...
            if python_path is None:
                python_path = f"{container_context.workspace_root}:{
        ...
        container_env[AIRFLOW_DAGS_FOLDER_ENV_VAR] = f"{
            container_context.workspace_root}/{self.airflow_dags_dir}"
        ...
        if self.db_app is not None and isinstance(self.db_app, DbApp):
            logger.debug(f"Reading db connection details from: {
                         self.db_app.name}")
            if db_user is None:
        ...
        db_connection_url = f"{
            db_driver}://{db_user}:{db_password}@{db_host}:{db_port}/{db_database}"

        # Set the AIRFLOW__DATABASE__SQL_ALCHEMY_CONN
        if "None" not in db_connection_url:
            logger.debug(f"AIRFLOW__DATABASE__SQL_ALCHEMY_CONN: {
                         db_connection_url}")
        ...
        if self.executor == "CeleryExecutor":
            ...
            celery_result_backend_url = (
                f"{celery_result_backend_driver}://{db_user}:{
            ...
            if self.redis_app is not None and isinstance(self.redis_app, DbApp):
                logger.debug(f"Reading redis connection details from: {
                             self.redis_app.name}")
            ...
            # Set the AIRFLOW__CELERY__RESULT_BACKEND
            celery_broker_url = f"{
                redis_driver}://{redis_password}{redis_host}:{redis_port}/{redis_schema}"
            if "None" not in celery_broker_url:
                logger.debug(f"AIRFLOW__CELERY__BROKER_URL: {
                             celery_broker_url}")
    ...
    def get_container_volumes(self, container_context: ContainerContext) -> Dict[str, dict]:
        ...
        if self.mount_logs:
            ...
            if logs_volume_container_path_str is None:
                logs_volume_container_path_str = f"{
                    self.get_airflow_home()}/logs"
