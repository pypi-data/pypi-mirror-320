...
class CohereReranker(Reranker):
    ...
    def _rerank(self, query: str, documents: List[Document]) -> List[Document]:
        ...
        if top_n and not (0 < top_n):
            logger.warning(f"top_n should be a positive integer, got {
                           self.top_n}, setting top_n to None")
            top_n = None

        compressed_docs: list[Document] = []
        _docs = [doc.content for doc in documents]
        response = self.client.rerank(
            query=query, documents=_docs, model=self.model)
        ...
        compressed_docs.sort(
            key=lambda x: x.reranking_score if x.reranking_score is not None else float(
                "-inf"),
            reverse=True,
        )
    ...
    def rerank(self, query: str, documents: List[Document]) -> List[Document]:
        try:
            ...
            logger.error(f"Error reranking documents: {
                         e}. Returning original documents")
            return documents
