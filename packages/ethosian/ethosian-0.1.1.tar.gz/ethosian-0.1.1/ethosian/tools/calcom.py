...
class CalCom(Toolkit):
    def __init__(
        ...
        self.event_type_id = event_type_id or int(
            event_type_str) if event_type_str is not None else 0
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for date, times in slots.items():
                    for slot in times:
                        user_time = self._convert_to_user_timezone(
                            slot["time"])
    ...
    def create_booking(
        ...
        try:
            ...
            start_time = datetime.fromisoformat(start_time).astimezone(
                pytz.utc).isoformat(timespec="seconds")
            payload = {
                "start": start_time,
                "eventTypeId": self.event_type_id,
            ...
            if response.status_code == 200:
                ...
                for booking in bookings:
                    user_time = self._convert_to_user_timezone(
                        booking["start"])
                    booking_info.append(
                        f"uid: {booking['uid']}, Title: {booking['title']}, Time: {
                            user_time}, Status: {booking['status']}"
                    )
    ...
    def reschedule_booking(
        ...
        try:
            ...
            new_start_time = datetime.fromisoformat(new_start_time).astimezone(
                pytz.utc).isoformat(timespec="seconds")
