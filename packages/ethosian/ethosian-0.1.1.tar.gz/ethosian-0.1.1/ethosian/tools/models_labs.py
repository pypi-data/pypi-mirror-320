...
from ethosian.model.content import Video, Image
        ...
        try:
            ...
            for media_url in url_links:
                if self.file_type == FileType.MP4:
                    agent.add_video(
                        Video(id=str(video_id), url=media_url, eta=str(eta)))
            ...
            if self.wait_for_completion and isinstance(eta, int):
                ...
                logger.info(f"Waiting for {
                            time_to_wait} seconds for video to be ready")
                while not video_ready and seconds_waited < time_to_wait:
                    ...
                    logger.debug(f"Fetching video from {
                                 self.fetch_url}/{video_id}")
