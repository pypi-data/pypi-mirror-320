...
from ethosian.storage.agent.base import AgentStorage
...
class S2AgentStorage(AgentStorage):
    def __init__(
        ...
        if _engine is None and db_url is not None:
            _engine = create_engine(db_url, connect_args={
                                    "charset": "utf8mb4"})
    ...
    def upsert(self, session: AgentSession) -> Optional[AgentSession]:
        ...
        with self.Session.begin() as sess:
            ...
            upsert_sql = text(
                ...
                (session_id, agent_id, user_id, memory, agent_data,
                 user_data, session_data, created_at, updated_at)
                VALUES
