...
from ethosian.vectordb.clickhouse.index import HNSW
...
class ClickhouseDb(VectorDb):
    ...
    def create(self) -> None:
        if not self.table_exists():
            ...
            if isinstance(self.index, HNSW):
                index = (
                    f"INDEX embedding_index embedding TYPE vector_similarity('hnsw', 'L2Distance', {
                        self.index.quantization}, "
                    f"{self.index.hnsw_max_connections_per_layer}, {
                        self.index.hnsw_candidate_list_size_for_construction})"
                )
                self.client.command(
                    "SET allow_experimental_vector_similarity_index = 1")
            else:
                raise NotImplementedError(f"Not implemented index {
                                          type(self.index)!r} is passed")
    ...
    def optimize(self) -> None:
        logger.debug(
            "==== No need to optimize Clickhouse DB. Skipping this step ====")
