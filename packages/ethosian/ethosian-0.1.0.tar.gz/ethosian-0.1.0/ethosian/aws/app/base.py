...
from ethosian.app.base import AppBase  # noqa: F401
from ethosian.app.context import ContainerContext
from ethosian.aws.app.context import AwsBuildContext
...
if TYPE_CHECKING:
    ...
    from ethosian.aws.resource.ecs.container import EcsContainer
...
class AwsApp(AppBase):
    ...
    def get_container_context(self) -> Optional[ContainerContext]:
        ...
        if self.workspace_settings is not None and self.workspace_settings.scripts_dir is not None:
            self.container_context.scripts_dir = f"{
                workspace_root_in_container}/{self.workspace_settings.scripts_dir}"

        if self.workspace_settings is not None and self.workspace_settings.storage_dir is not None:
            self.container_context.storage_dir = f"{
                workspace_root_in_container}/{self.workspace_settings.storage_dir}"
        ...
        if self.requirements_file is not None:
            self.container_context.requirements_file = f"{
                workspace_root_in_container}/{self.requirements_file}"

        return self.container_context

    def get_container_env(self, container_context: ContainerContext, build_context: AwsBuildContext) -> Dict[str, str]:
        from ethosian.constants import (
            ethosian_RUNTIME_ENV_VAR,
            PYTHONPATH_ENV_VAR,
        ...
        try:
            if container_context.workspace_schema is not None:
                if container_context.workspace_schema.id_workspace is not None:
                    container_env[WORKSPACE_ID_ENV_VAR] = str(
                        container_context.workspace_schema.id_workspace) or ""
        except Exception:
            pass
        ...
        self.set_aws_env_vars(env_dict=container_env,
                              aws_region=build_context.aws_region)
    ...
    def get_load_balancer_security_groups(self) -> Optional[List["SecurityGroup"]]:
        from ethosian.aws.resource.ec2.security_group import SecurityGroup, InboundRule
        ...
        if load_balancer_security_groups is None:
            ...
            if self.create_load_balancer and self.create_security_groups:
                ...
                lb_sg = SecurityGroup(
                    ...
                    description=f"Security group for {
                        self.get_app_name()} load balancer",
                    inbound_rules=[
                        InboundRule(
        ...
        if load_balancer_security_groups is not None:
            ...
            for lb_sg in load_balancer_security_groups:
                app_sg.inbound_rules.append(
                    InboundRule(
                        description=f"Allow traffic from {
                            lb_sg.name} to the {self.get_app_name()}",
        ...
        if self.inbound_security_groups is not None:
            ...
            for inbound_sg in self.inbound_security_groups:
                app_sg.inbound_rules.append(
                    InboundRule(
                        description=f"Allow traffic from {
                            inbound_sg.name} to the {self.get_app_name()}",
                        port=self.container_port,
                        source_security_group_id=AwsReference(
        ...
        if self.inbound_security_group_ids is not None:
            ...
            for inbound_sg_id in self.inbound_security_group_ids:
                app_sg.inbound_rules.append(
                    InboundRule(
                        description=f"Allow traffic from {
                            inbound_sg_id} to the {self.get_app_name()}",
                        port=self.container_port,
    ...
    def get_ecs_cluster(self) -> "EcsCluster":
        ...
        if self.ecs_cluster is None:
            ...
            raise Exception(f"Invalid ECSCluster: {
                            self.ecs_cluster} - Must be of type EcsCluster")
    ...
    def get_load_balancer(self) -> Optional["LoadBalancer"]:
        ...
        if self.load_balancer is None:
            ...
            raise Exception(f"Invalid LoadBalancer: {
                            self.load_balancer} - Must be of type LoadBalancer")
    ...
    def get_target_group(self) -> Optional["TargetGroup"]:
        ...
        if self.target_group is None:
            ...
            raise Exception(f"Invalid TargetGroup: {
                            self.target_group} - Must be of type TargetGroup")
        ...
        if self.listeners is None:
            ...
            for listener in self.listeners:
                if not isinstance(listener, Listener):
                    raise Exception(f"Invalid Listener: {
                                    listener} - Must be of type Listener")
            return self.listeners
        else:
            raise Exception(f"Invalid Listener: {
                            self.listeners} - Must be of type List[Listener]")
    ...
    def build_resources(self, build_context: AwsBuildContext) -> List["AwsResource"]:
        ...
        from ethosian.aws.resource.ecs.task_definition import EcsTaskDefinition
        from ethosian.aws.resource.ecs.service import EcsService
        from ethosian.aws.resource.ecs.volume import EcsVolume
        from ethosian.docker.resource.image import DockerImage
        from ethosian.utils.defaults import get_default_volume_name

        logger.debug(
            f"------------ Building {self.get_app_name()} ------------")
        ...
        logger.debug(f"ContainerContext: {
                     container_context.model_dump_json(indent=2)}")
