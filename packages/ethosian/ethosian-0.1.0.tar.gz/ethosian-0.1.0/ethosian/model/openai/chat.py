...
try:
    ...
    if installed_version_tuple < MIN_OPENAI_VERSION:
        raise ImportError(
            f"`openai` version must be >= {'.'.join(map(str, MIN_OPENAI_VERSION))}, but found {
                installed_version}. "
...
class OpenAIChat(Model):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.response_format is not None:
            model_dict["response_format"] = (
                self.response_format if isinstance(
                    self.response_format, dict) else str(self.response_format)
            )
        if self.seed is not None:
    ...
    def invoke(self, messages: List[Message]) -> Union[ChatCompletion, ParsedChatCompletion]:
        ...
        return self.get_client().chat.completions.create(
            model=self.id,
            messages=[self.format_message(m)
    ...
    def response_stream(self, messages: List[Message]) -> Iterator[ModelResponse]:
        ...
        if stream_data.response_tool_calls is not None:
            _tool_calls = self.build_tool_calls(
                stream_data.response_tool_calls)
