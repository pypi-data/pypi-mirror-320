...
class CohereChat(Model):
            ...
            CohereTool(
                ...
                parameter_definitions={
                    param_name: ToolParameterDefinitionsValue(
                        type=param_info["type"] if isinstance(
                            param_info["type"], str) else param_info["type"][0],
    ...
    def _prepare_function_calls(self, agent_message: Message) -> Tuple[List[FunctionCall], List[Message]]:
        ...
        for tool_call in agent_message.tool_calls:
            ...
            if _function_call.error is not None:
                error_messages.append(
                    Message(role="user", content=_function_call.error))
                continue
    ...
    def _handle_tool_calls(
        ...
        if response_tool_calls:
            tool_results = [
                ToolResult(
                    call=tool_call,
                    outputs=[tool_call.parameters, {
                        "result": fn_result.content}],
                )
    ...
    def response(self, messages: List[Message], tool_results: Optional[List[ToolResult]] = None) -> ModelResponse:
        ...
        if assistant_message.tool_calls and self.run_tools:
            ...
            response_after_tool_calls = self.response(
                messages=messages, tool_results=tool_results)
    ...
    def _update_stream_metrics(self, stream_data: StreamData, assistant_message: Message):
        ...
        if stream_data.completion_tokens > 0:
            ...
            self.metrics["tokens_per_second"].append(
                f"{stream_data.completion_tokens /
                    stream_data.response_timer.elapsed:.4f}"
            )
    ...
    def response_stream(
        ...
        logger.debug(f"Time to generate response: {
                     stream_data.response_timer.elapsed:.4f}s")

        # -*- Create assistant message
        assistant_message = Message(
            role="assistant", content=stream_data.response_content)
        ...
        if tokens:
            ...
            if input_tokens is not None and output_tokens is not None:
                assistant_message.metrics["total_tokens"] = input_tokens + \
                    output_tokens
                self.metrics["total_tokens"] = self.metrics.get(
        ...
        self._update_stream_metrics(
            stream_data=stream_data, assistant_message=assistant_message)
        ...
        if assistant_message.tool_calls is not None and len(assistant_message.tool_calls) > 0 and self.run_tools:
            ...
            if stream_data.response_tool_calls is not None:
                ...
                tool_results = [
                    ToolResult(call=tool_call, outputs=[
                               tool_call.parameters, {"result": fn_result.content}])
