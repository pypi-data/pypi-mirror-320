...
class DockerContainer(DockerResource):
    ...
    def run_container(self, docker_client: DockerApiClient) -> Optional[Any]:
        ...
        try:
            ...
            with Progress(
                ...
                if self.pull:
                    try:
                        ...
                        _api_client.images.pull(
                            self.image, platform=self.platform)
                        progress.update(pull_image_task, completed=True)
                    except Exception as pull_exc:
                        logger.debug(f"Could not pull image: {
                                     self.image}: {pull_exc}")
            ...
            logger.error(f"Image {self.image} not found. Explanation: {
                         img_error.explanation}")
            raise
    ...
    def _create(self, docker_client: DockerApiClient) -> bool:
        ...
        try:
            ...
            if container_object is not None:
                logger.debug("Container Created: {}".format(
                ...
                with Progress(
                    ...
                    while self.container_status != "created":
                        logger.debug(f"Container Status: {
                                     self.container_status}, trying again in 1 seconds")
                        sleep(1)
                        container_object.reload()
    ...
    def _delete(self, docker_client: DockerApiClient) -> bool:
        ...
        try:
            logger.debug(
                "Reloading container_object: {}".format(container_object))
            for i in range(10):
                container_object.reload()
    ...
    def is_active(self, docker_client: DockerApiClient) -> bool:
        ...
        container_object: Optional[Container] = self.read(
