...
class GithubTools(Toolkit):
    ...
    def authenticate(self):
        ...
        if self.base_url:
            logger.debug(f"Authenticating with GitHub Enterprise at {
                         self.base_url}")
    ...
    def list_pull_requests(self, repo_name: str, state: str = "open") -> str:
        ...
        """
        logger.debug(f"Listing pull requests for repository: {
                     repo_name} with state: {state}")
        try:
            repo = self.g.get_repo(repo_name)
    ...
    def get_pull_request(self, repo_name: str, pr_number: int) -> str:
        ...
        """
        logger.debug(f"Getting pull request #{
                     pr_number} for repository: {repo_name}")
        try:
            repo = self.g.get_repo(repo_name)
    ...
    def get_pull_request_changes(self, repo_name: str, pr_number: int) -> str:
        ...
        """
        logger.debug(f"Getting changes for pull request #{
                     pr_number} in repository: {repo_name}")
        try:
            repo = self.g.get_repo(repo_name)
            pr = repo.get_pull(pr_number)
            files = pr.get_files()
    ...
    def list_issues(self, repo_name: str, state: str = "open") -> str:
        ...
        """
        logger.debug(f"Listing issues for repository: {
                     repo_name} with state: {state}")
        try:
            repo = self.g.get_repo(repo_name)
            ...
            filtered_issues = [
                issue for issue in issues if not issue.pull_request]
            issue_list = []
    ...
    def get_issue(self, repo_name: str, issue_number: int) -> str:
        ...
        """
        logger.debug(f"Getting issue #{
                     issue_number} for repository: {repo_name}")
        try:
            repo = self.g.get_repo(repo_name)
    ...
    def comment_on_issue(self, repo_name: str, issue_number: int, comment_body: str) -> str:
        ...
        """
        logger.debug(f"Adding comment to issue #{
                     issue_number} in repository: {repo_name}")
        try:
            repo = self.g.get_repo(repo_name)
            issue = repo.get_issue(number=issue_number)
    ...
    def reopen_issue(self, repo_name: str, issue_number: int) -> str:
        ...
        """
        logger.debug(f"Reopening issue #{
                     issue_number} in repository: {repo_name}")
        try:
            repo = self.g.get_repo(repo_name)
            issue = repo.get_issue(number=issue_number)
    ...
    def assign_issue(self, repo_name: str, issue_number: int, assignees: List[str]) -> str:
        ...
        """
        logger.debug(f"Assigning users to issue #{
                     issue_number} in repository: {repo_name}")
        try:
            repo = self.g.get_repo(repo_name)
            issue = repo.get_issue(number=issue_number)
    ...
    def list_issue_comments(self, repo_name: str, issue_number: int) -> str:
        ...
        """
        logger.debug(f"Listing comments for issue #{
                     issue_number} in repository: {repo_name}")
        try:
            repo = self.g.get_repo(repo_name)
            issue = repo.get_issue(number=issue_number)
