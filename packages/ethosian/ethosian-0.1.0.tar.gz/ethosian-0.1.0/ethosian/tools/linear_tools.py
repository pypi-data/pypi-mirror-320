...
class LinearTool(Toolkit):
    ...
    def _execute_query(self, query, variables=None):
        ...
        try:
            response = requests.post(self.endpoint, json={
                                     "query": query, "variables": variables}, headers=self.headers)
    ...
    def get_user_details(self) -> Optional[str]:
        ...
        try:
            ...
            if response.get("viewer"):
                ...
                logger.info(
                    f"Retrieved authenticated user details with name: {
    ...
    def get_issue_details(self, issue_id: str) -> Optional[str]:
        ...
        try:
            ...
            if response.get("issue"):
                ...
                logger.info(f"Issue '{issue['title']}' retrieved successfully with ID {
                            issue['id']}.")
                return str(issue)
            else:
    ...
    def create_issue(
        ...
        try:
            ...
            logger.error(f"Error creating issue '{
                         title}' for team ID {team_id}: {e}")
            raise
    ...
    def update_issue(self, issue_id: str, title: Optional[str]) -> Optional[str]:
        ...
        try:
            ...
            if response["issueUpdate"]["success"]:
                ...
                logger.error(f"Failed to update issue ID {
                             issue_id}. Success flag was false.")
                return None
    ...
    def get_user_assigned_issues(self, user_id: str) -> Optional[str]:
        ...
        try:
            ...
            if response.get("user"):
                ...
                logger.info(f"Retrieved {len(issues)} issues assigned to user '{
    ...
    def get_workflow_issues(self, workflow_id: str) -> Optional[str]:
        ...
        try:
            ...
            if response.get("workflowState"):
                ...
                logger.info(f"Retrieved {len(issues)} issues in workflow state ID {
                            workflow_id}.")
                return str(issues)
            else:
                logger.error(
            ...
            logger.error(f"Error retrieving issues for workflow state ID {
                         workflow_id}: {e}")
            raise
