...
from ethosian.tools.toolkit import Toolkit
...
class ZoomTool(Toolkit):
    ...
    def schedule_meeting(self, topic: str, start_time: str, duration: int, timezone: str = "UTC") -> str:
        ...
        """
        logger.debug(f"Attempting to schedule meeting: {
                     topic} in timezone: {timezone}")
        ...
        try:
            ...
            logger.info(f"Meeting scheduled successfully. ID: {
                        meeting_info['id']}")
    ...
    def get_upcoming_meetings(self, user_id: str = "me") -> str:
        ...
        try:
            ...
            result = {"message": "Upcoming meetings retrieved successfully",
                      "meetings": meetings.get("meetings", [])}
            logger.info(
                f"Retrieved {len(result['meetings'])} upcoming meetings")
            return json.dumps(result, indent=2)
        except requests.RequestException as e:
    ...
    def get_meeting_recordings(
        ...
        try:
            ...
            logger.info(
                f"Retrieved {result['recording_count']} recording files")
            return json.dumps(result, indent=2)
        except requests.RequestException as e:
