...
from ethosian.model.content import Image
...
class Dalle(Toolkit):
    def __init__(
        ...
        if quality not in ["standard", "hd"]:
            raise ValueError(
                "Invalid quality. Please choose from 'standard' or 'hd'.")
        if not isinstance(n, int) or n <= 0:
            raise ValueError(
                "Invalid number of images. Please provide a positive integer.")
        if model == "dall-e-3" and n > 1:
            raise ValueError(
                "Dall-e-3 only supports a single image generation.")
    ...
    def create_image(self, agent: Agent, prompt: str) -> str:
        ...
        try:
            ...
            for img in response.data:
                agent.add_image(
                    Image(id=str(uuid4()), url=img.url, original_prompt=prompt,
                          revised_prompt=img.revised_prompt)
                )
                response_str += f"Image has been generated at the URL {
                    img.url}\n"
            return response_str
