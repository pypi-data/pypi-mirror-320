...
try:
    ...
    raise ImportError(
        "`jira` not installed. Please install using `pip install jira`")
...
class JiraTools(Toolkit):
    def __init__(
        ...
        if auth:
            self.jira = JIRA(server=self.server_url,
                             basic_auth=cast(tuple[str, str], auth))
        else:
            self.jira = JIRA(server=self.server_url)
    ...
    def get_issue(self, issue_key: str) -> str:
        ...
        try:
            ...
            logger.debug(f"Issue details retrieved for {
                         issue_key}: {issue_details}")
