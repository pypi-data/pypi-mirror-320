...
class S2VectorDb(VectorDb):
    ...
    def get_table(self) -> Table:
        ...
        return Table(
            ...
            # Use BLOB for storing vector embeddings
            Column("embedding", mysql.BLOB),
            Column("usage", mysql.TEXT),
            Column("created_at", DateTime(timezone=True),
                   server_default=text("now()")),
            Column("updated_at", DateTime(
                timezone=True), onupdate=text("now()")),
    ...
    def doc_exists(self, document: Document) -> bool:
        ...
        with self.Session.begin() as sess:
            ...
            stmt = select(*columns).where(self.table.c.content_hash ==
        ...
        with self.Session.begin() as sess:
            ...
            for document in documents:
                ...
                logger.debug(f"Inserted document: {
                             document.name} ({document.meta_data})")
    ...
    def search(self, query: str, limit: int = 5, filters: Optional[Dict[str, Any]] = None) -> List[Document]:
        ...
        for neighbor in neighbors:
            meta_data_dict = json.loads(
                neighbor.meta_data) if neighbor.meta_data else {}
            ...
            embedding_list = json.loads(
                neighbor.embedding) if neighbor.embedding else []
