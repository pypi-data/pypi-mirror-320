...
class AppBase(InfraBase):
    ...
    def build_resources(self, build_context: Any) -> Optional[Any]:
        logger.debug(
            f"@build_resource_group not defined for {self.get_app_name()}")
        return None

    def get_dependencies(self) -> Optional[List[ResourceBase]]:
        return (
            [dep for dep in self.depends_on if isinstance(
                dep, ResourceBase)] if self.depends_on is not None else None
        )
    ...
    def add_app_properties_to_resources(self, resources: List[ResourceBase]) -> List[ResourceBase]:
        ...
        for resource in resources:
            ...
            resource_wait_for_create = resource_properties.get(
                "wait_for_create", None)
            resource_wait_for_update = resource_properties.get(
                "wait_for_update", None)
            resource_wait_for_delete = resource_properties.get(
    ...
    def get_resources(self, build_context: Any) -> List[ResourceBase]:
        ...
        self.cached_resources = self.add_app_properties_to_resources(
            base_resources)
    ...
    def matches_filters(self, group_filter: Optional[str] = None) -> bool:
        if group_filter is not None:
            ...
            logger.debug(f"{self.get_app_name()}: Checking {
