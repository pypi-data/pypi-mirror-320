...
def log_config_not_available_msg() -> None:
    logger.error(
        "ethosian config not found, please run `ethosian init` and try again")


def log_active_workspace_not_available() -> None:
    logger.error("Could not find an active workspace. You can:")
    logger.error(
        "- Run `ethosian ws setup` to setup a workspace at the current path")
    logger.error("- Run `ethosian ws create` to create a new workspace")


def print_available_workspaces(avl_ws_list) -> None:
    avl_ws_names = [
        w.ws_root_path.stem for w in avl_ws_list] if avl_ws_list else []
    print_info("Available Workspaces:\n  - {}".format("\n  - ".join(avl_ws_names)))


def log_ethosian_init_failed_msg() -> None:
    logger.error("ethosian initialization failed, please try again")
...
def confirm_yes_no(question, default: str = "yes") -> bool:
    ...
    """
    inp_to_result_map = {"yes": True, "y": True,
