"""ethosian Workspace Cli

This is the entrypoint for the `ethosian ws` application.
"""
...
from ethosian.utils.log import logger, set_log_level_to_debug
...
ws_cli = typer.Typer(
    ...
    help="""\b
Use `ethosian ws [COMMAND]` to create, setup, start or stop your workspace.
Run `ethosian ws [COMMAND] --help` for more info.
""",
...
def create(
    ...
    \b
    Examples:
    > ethosian ws create -t llm-app          -> Create an `llm-app` in the current directory
    > ethosian ws create -t llm-app -n llm   -> Create an `llm-app` named `llm` in the current directory
    """
    if print_debug_log:
        set_log_level_to_debug()

    from ethosian.workspace.operator import create_workspace
...
def setup(
    ...
    \b
    Examples:
    > `ethosian ws setup`           -> Setup the current directory as a workspace
    > `ethosian ws setup llm-app`   -> Setup the `llm-app` folder as a workspace
    """
    if print_debug_log:
        set_log_level_to_debug()

    from ethosian.workspace.operator import setup_workspace
...
def up(
    ...
    \b
    Examples:
    > `ethosian ws up`            -> Deploy all resources
    > `ethosian ws up dev`        -> Deploy all dev resources
    > `ethosian ws up prd`        -> Deploy all prd resources
    > `ethosian ws up prd:aws`    -> Deploy all prd aws resources
    > `ethosian ws up prd:::s3`   -> Deploy prd resources matching name s3
    """
    if print_debug_log:
        set_log_level_to_debug()
    ...
    from ethosian.workspace.operator import start_workspace, setup_workspace
    from ethosian.workspace.helpers import get_workspace_dir_path
    from ethosian.utils.resource_filter import parse_resource_filter
    ...
    ws_at_current_path: Optional[WorkspaceConfig] = ethosian_config.get_ws_config_by_path(
        current_path)
    if ws_at_current_path is not None:
        logger.debug(f"Found workspace at: {ws_at_current_path.ws_root_path}")
        if str(ws_at_current_path.ws_root_path) != ethosian_config.active_ws_dir:
            logger.debug(f"Updating active workspace to {
                         ws_at_current_path.ws_root_path}")
            ethosian_config.set_active_ws_dir(ws_at_current_path.ws_root_path)
    ...
    if ws_to_start is None:
        active_ws_config: Optional[WorkspaceConfig] = ethosian_config.get_active_ws_config(
        )
    ...
    if ws_to_start is None:
        log_active_workspace_not_available()
        avl_ws = ethosian_config.available_ws
        if avl_ws:
    ...
    if resource_filter is not None:
        if not isinstance(resource_filter, str):
            raise TypeError(f"Invalid resource_filter. Expected: str, Received: {
    ...
    print_heading("Starting workspace: {}".format(
        str(ws_to_start.ws_root_path.stem)))
    start_workspace(
...
def down(
    ...
    \b
    Examples:
    > `ethosian ws down`            -> Delete all resources
    """
    if print_debug_log:
        set_log_level_to_debug()
    ...
    from ethosian.cli.operator import initialize_ethosian
    from ethosian.workspace.config import WorkspaceConfig
    from ethosian.workspace.operator import stop_workspace, setup_workspace
    from ethosian.workspace.helpers import get_workspace_dir_path
    from ethosian.utils.resource_filter import parse_resource_filter
    ...
    logger.debug(f"\tauto_confirm : {auto_confirm}")
    logger.debug(f"\tforce        : {force}")
    print_heading("Stopping workspace: {}".format(
        str(ws_to_stop.ws_root_path.stem)))
    stop_workspace(
        ethosian_config=ethosian_config,
...
def patch(
    ...
    Examples:
    \b
    > `ethosian ws patch`           -> Patch all resources
    """
    if print_debug_log:
        set_log_level_to_debug()
    ...
    from ethosian.cli.operator import initialize_ethosian
    from ethosian.workspace.config import WorkspaceConfig
    from ethosian.workspace.operator import update_workspace, setup_workspace
    from ethosian.workspace.helpers import get_workspace_dir_path
    from ethosian.utils.resource_filter import parse_resource_filter
    ...
    logger.debug(f"\tforce        : {force}")
    logger.debug(f"\tpull         : {pull}")
    print_heading("Updating workspace: {}".format(
        str(ws_to_patch.ws_root_path.stem)))
    update_workspace(
        ethosian_config=ethosian_config,
...
def restart(
    ...
    """\b
    Restarts the active workspace. i.e. runs `ethosian ws down` and then `ethosian ws up`.
...
def config(
    ...
    \b
    Examples:
    $ `ethosian ws config`         -> Print the active workspace config
    """
    if print_debug_log:
        set_log_level_to_debug()
    ...
    from ethosian.cli.operator import initialize_ethosian
    from ethosian.workspace.config import WorkspaceConfig
    from ethosian.utils.load_env import load_env
    ...
    print_info(active_ws_config.model_dump_json(
        include={"ws_name", "ws_root_path"}, indent=2))
...
def delete(
    ...
    all_workspaces: bool = typer.Option(
        ...
        "-a",
        "--all",
        help="Delete all workspaces from ethosian",
    ),
    print_debug_log: bool = typer.Option(
        False,
        "-d",
    ...
    """\b
    Deletes the workspace record from ethosian.
    NOTE: Does not delete any physical files.

    \b
    Examples:
    $ `ethosian ws delete`         -> Delete the active workspace from ethosian
    $ `ethosian ws delete -a`      -> Delete all workspaces from ethosian
    """
    if print_debug_log:
        set_log_level_to_debug()
    ...
    from ethosian.cli.config import ethosianCliConfig
    from ethosian.cli.operator import initialize_ethosian
    from ethosian.workspace.operator import delete_workspace
    ...
    if ws_name is not None:
        ws_config = ethosian_config.get_ws_config_by_dir_name(ws_name)
        ...
        if all_workspaces:
            ws_to_delete = [
                ws.ws_root_path for ws in ethosian_config.available_ws if ws.ws_root_path is not None]
        else:
            # By default, we assume this command is run for the active workspace
            if ethosian_config.active_ws_dir is not None:
                ws_to_delete.append(Path(ethosian_config.active_ws_dir))

    delete_workspace(ethosian_config, ws_to_delete)
