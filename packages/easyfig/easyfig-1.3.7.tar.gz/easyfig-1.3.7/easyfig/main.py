# Author: Yu-Xin Tian, 2025-01-10 Version 1.3.7
...
def makefig():
    ...
    # 初始化主窗口
    root = tk.Tk()
    root.title("EasyFig仿真绘图向导 V1.3.7 - 作者：东北大学 田雨鑫 - 让天下没有难用的工具！")
...
# xy轴的名字
x_name = '{convert_latex_escape(x_name_str)}' # NEW
y_name = '{convert_latex_escape(y_name_str)}' # NEW
...
# 四个表达式分别达到最大时显示的标签、区域背景颜色和区域图案。
texts = {str(setting_dict[tab_index]['the_marker'])} # NEW
colors = {str(setting_dict[tab_index]['lcor_list']).replace("'None'", "None")}
