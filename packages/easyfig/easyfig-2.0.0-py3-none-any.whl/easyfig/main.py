# Author: Yu-Xin Tian, 2025-01-10 Version 2.0.0
...
def makefig():
    ...
    hint_text = ("请按照 `表达式名称(支持LaTex, 要由$包裹) = 表达式` 的格式书写，一行是一个表达式。特别注意：右侧表达式中的`λ`应为`lamda`。例如:\n\n"
                 "$\\pi_r^{NS}$ = alpha+2*c^2-(b+lamda)/2\n\n填写完表达式后，点击“公式识别”。如果第一次用，请点击“加载案例”，在已有例子的基础上改写，更方便快捷。"
    ...
    # 初始化主窗口
    root = tk.Tk()
    root.title("EasyFig仿真绘图向导 V2.0.0 - 作者：东北大学 田雨鑫 - 让天下没有难用的工具！")
