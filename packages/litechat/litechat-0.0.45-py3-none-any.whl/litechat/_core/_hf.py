...
class HuggingFaceAutomation:
    ...
    async def non_stream_query(self, query, web_search=False, max_wait_time=300, system_prompt="",
        ...
        try:
            complete_response = ""
            # No need to await the generator itself
            generator = self.capture_event_stream()
            async for token in generator:
                if token:  # Only add non-empty tokens
                    complete_response += token
                    # Check for stop sequences
                    stop = kwargs.get("stop", [])
                    if stop and any(s in complete_response for s in stop):
                        idx = [s in complete_response for s in stop].index(True)
                        return complete_response.split(stop[idx])[0]
            return complete_response
        except Exception as e:
            logger.error(f"Error in non_stream_query: {e}")
            return ""

    async def capture_event_stream(self):
        """Capture the server-sent events stream directly."""
        try:
            # Wait for and capture the SSE response
            async with self.page.expect_response(
                lambda response:
                response.headers.get("content-type") == "text/event-stream" and
                "/conversation/" in response.url
            ) as response_info:
            ...
            for line in raw_text.split('\n'):
                ...
                try:
                    ...
                    if json_data["type"] == "stream" and "token" in json_data:
                        # Remove null bytes that appear in the tokens
                        token = json_data["token"].replace('\u0000', '')
                        # if token.strip():  # Only non-empty tokens
                        #     yield token
                    elif json_data["type"] == "finalAnswer" and "text" in json_data:
                        final_text = json_data["text"]
                        if final_text.strip():
                            yield final_text
            ...
            logger.error(f"Error capturing event stream: {e}")
            return
    ...
    async def stream_text_logic(self, web_search=False, max_wait_time=300):
        try:
            ...
            logger.error(f"Error in stream_text_logic: {e}")
            print(f"Error in stream_text_logic: {e}")
