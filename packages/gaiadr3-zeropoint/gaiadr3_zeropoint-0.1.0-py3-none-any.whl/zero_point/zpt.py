...
def get_zpt(phot_g_mean_mag, nu_eff_used_in_astrometry, pseudocolour, ecl_lat, astrometric_params_solved,
    ...
    for i, inp in enumerate(inputs):
        # first: check is not an iterable (there could still be None values within the iterable)
        if not (isinstance(inp, np.ndarray) or isinstance(inp, list) or isinstance(inp, tuple)):
            # if not an iterable, has to be int or float, or None
            if inp is None:
                if (i==1 and inputs[-1]==95) or (i==2 and inputs[-1]==31):
                    #this is either a 5p source without pseudocolour or a 6p source without nueff: ALL OK!
                    continue
                else:
                    #this should not happen -> return NaN
                    if _warnings:
                        warnings.warn("Variable {} is None but it is necessary for the calculation! Returning NaN.".format(inputs_names[i]))
                    return np.nan
    ...
    if np.isscalar(phot_g_mean_mag):
        ...
        #by forcing the conversion to float, None values are automatically replaced by NaNs
        phot_g_mean_mag = np.array(phot_g_mean_mag).astype(float)
        nu_eff_used_in_astrometry = np.array(nu_eff_used_in_astrometry).astype(float)
        pseudocolour = np.array(pseudocolour).astype(float)
        ecl_lat = np.array(ecl_lat).astype(float)
        astrometric_params_solved = np.array(astrometric_params_solved).astype(float)
