...
def fieldid_to_radec(fieldid=None, ccdid_or_qid=None, level="focalplane", **kwargs):
    """ """
    return FieldProjection.fieldid_to_radec(fieldid=fieldid, ccdid_or_qid=ccdid_or_qid, level=level, **kwargs)
...
class FieldProjection( object ):
    ...
    def fieldid_to_radec(cls, fieldid=None, ccdid_or_qid=None, level="focalplane", as_shapely=False):
        ...
        if level == "focalplane":
            ...
            fieldid_, levelid = np.vstack(geo_df.index.str.split("_")).astype("int32").T
            geo_df["fieldid"], geo_df[key] = fieldid_, levelid
