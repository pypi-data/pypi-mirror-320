...
AllowedName = Union[Literal["werkzeug","uvicorn", "uvicorn.access", "uvicorn.error"], str]
class CustomLogger:
    """
    ...
    def __init__(self, 
                 ...
                 names: Optional[List[AllowedName]] = None, 
                 ignore: Optional[List[str]] = None, 
                 include_date: bool = True, 
                 include_level: bool = False, 
                 date_color: str = 'white'):
        if names is None:
            names = ['werkzeug']  # Default to 'werkzeug' if no names are provided
        if ignore is None:
            ignore = []

        # Install verboselogs levels
        verboselogs.install()
        ...
        # Create a single merged logger
        self.logger = logging.getLogger("merged_logger")
        self.logger.setLevel(level.upper())
        self.logger.propagate = False

        # Attach the filter to the logger
        filter_instance = EndpointFilter(ignore)
        self.logger.addFilter(filter_instance)

        # Configure logging format and coloured output
        if level is None:
        ...
        # Register the logger under all the specified names
        for name in names:
            target_logger = logging.getLogger(name)
            target_logger.handlers = self.logger.handlers  # Use merged logger's handlers
            target_logger.setLevel(self.logger.level)      # Match levels
            target_logger.addFilter(filter_instance)       # Attach the same filter
            target_logger.propagate = False               # Prevent propagation to parent loggers

        # Wrap the merged logger with an adapter
