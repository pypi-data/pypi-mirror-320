...
def call_api(url: str, method: str, headers: dict, body: dict, only_content: bool, only_res_code: bool, show_metrics: bool, average_response_time: int) -> None:
    ...
    try:
        ...
        print(f"{COLOR_RED}Error: Max Retry Error\n-Check your internet connection and try again\n-Called endpoint is not available\n-Check the endpoint URL\n-Check server response code{COLOR_RESET}")
        return
    
    except requests.exceptions.ConnectionError:
        print(f"{COLOR_RED}Error: Connection Error\n-Check your internet connection and try again\n-Called endpoint is not available\n-Check the endpoint URL{COLOR_RESET}")
        return
    
    except requests.exceptions.Timeout:
        print(f"{COLOR_RED}Error: Timeout Error\n-Check your internet connection and try again\n-Called endpoint is not available\n-Check the endpoint URL\n-Check server response code{COLOR_RESET}")
        return
...
def main() -> None:
    ...
    parser.add_argument("-H", "--header", type=json.loads, help="Header include in request as a JSON string")
    parser.add_argument("-b", "--body", type=json.loads, help="Body of the request as a JSON string")
    ...
    call_api(args.url, args.method, headers, body, args.only_content, args.only_res_code, args.metrics, args.average_response_time if args.average_response_time else 0)
