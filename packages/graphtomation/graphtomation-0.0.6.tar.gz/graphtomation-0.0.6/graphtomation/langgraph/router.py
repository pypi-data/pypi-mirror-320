...
class GraphApiRouter:
    ...
    def _add_routes(self):
        ...
        @self.router.post(
            "/{graph_name}",
            dependencies=self._get_dependencies("get_graph_schema"),
        )
        async def get_graph_schema(graph_name: str, input_data: GetGraphSchema):
            executor = await self.executor.get_executor(graph_name)
            return await executor.aget_graph_schema(input_data)
        ...
        async def list_graphs():
            return await self.executor.list_graphs()

        @self.router.get(
            "/{graph_name}",
            response_model=SerializedGraphResponse,
            dependencies=self._get_dependencies("get_graph"),
        )
        async def get_graph(graph_name: str):
            graph = await self.executor.get_executor(graph_name)
            schema = await graph.aget_graph_schema(GetGraphSchema())
