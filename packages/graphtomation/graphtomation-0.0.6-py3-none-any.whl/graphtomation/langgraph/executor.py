...
class SingleGraphExecutor:
    ...
    async def from_compiled_graph(cls, args: GraphArgs) -> "SingleGraphExecutor":
        ...
        if kwargs and kwargs.checkpointer and kwargs.checkpointer.name:
        ...
        instance.compiled_graph = await instance.state_graph.compile(
            interrupt_before=kwargs.interrupt_before if kwargs else None,
            interrupt_after=kwargs.interrupt_after if kwargs else None,
            debug=kwargs.debug if kwargs else False,
    ...
    async def initialize_checkpointer(
        ...
        if config.conn_string and config.name == "postgres":
            conn_string = str(config.conn_string)
...
class GraphExecutor:
    ...
    def get_graph(self, name: str) -> SingleGraphExecutor:
        if name not in self.graph_executors:
            graph_args = next(
                (g for g in self.graphs if g.kwargs.get("name") == name), None
            )
            if not graph_args:
    ...
    async def get_executor(self, name: str) -> SingleGraphExecutor:
        """
        Fetch or create a SingleGraphExecutor for the specified graph name.
        Ensures the graph is compiled before returning the executor.
        """
        if name not in self.graph_executors:
            graph_args = next(
                (g for g in self.graphs if g.kwargs.get("name") == name), None
    ...
    async def initialize_graph(self, name: str):
        graph_executor = await self.get_executor(name)
        await graph_executor.state_graph.compile()
