...
from duowen_agent.rag.ragflow.doc_store_conn import (
    DocStoreConnection,
    MatchDenseExpr,
    FusionExpr,
    OrderByExpr,
...
from duowen_agent.rag.ragflow.utils import rmSpace
...
class SearchDealer:
    ...
    def search(
        ...
        if not qst:
            ...
            for k in keywords:
                kwds.add(k)
                for kk in self.tokenizer.fine_grained_tokenize(k).split():
                    if len(kk) < 2:
                        continue
                    if kk in kwds:
                        continue
    ...
    def insert_citations(
        ...
        chunks_tks = [
            self.tokenizer.tokenize(self.qryr.rmWWW(ck)).split() for ck in chunks
        ]
        cites = {}
        thr = 0.63
        while thr > 0.3 and len(cites.keys()) == 0 and pieces_ and chunks_tks:
            for i, a in enumerate(pieces_):
                sim, tksim, vtsim = self.qryr.hybrid_similarity(
                    ans_v[i],
                    chunk_v,
                    self.tokenizer.tokenize(self.qryr.rmWWW(pieces_[i])).split(),
                    chunks_tks,
                    tkweight,
                    vtweight,
                )
    ...
    def hybrid_similarity(self, ans_embd, ins_embd, ans, inst):
        return self.qryr.hybrid_similarity(
            ans_embd,
            ins_embd,
            self.tokenizer.tokenize(ans).split(),
            self.tokenizer.tokenize(inst).split(),
        )
    ...
    def tag_content(
        ...
        tag_fea = sorted(
            [
                (a, round(0.1 * (c + 1) / (cnt + S) / (all_tags.get(a, 0.0001))))
                for a, c in aggs
            ],
            key=lambda x: x[1] * -1,
        )[:topn_tags]
