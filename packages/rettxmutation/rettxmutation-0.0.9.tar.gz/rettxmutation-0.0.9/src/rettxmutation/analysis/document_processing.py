...
class DynamicDocumentPreprocessor:
    ...
    def preprocess_image(
        ...
        binarization_block_size = binarization_block_size if binarization_block_size is not None else self.default_binarization_block_size
        ...
        logger.info(f"Brightness: {brightness:.2f}, Contrast: {contrast:.2f}")
        ...
        if sharpen and self._is_blurry(grayscale, threshold=sharpen_threshold):
            logger.info("Image is blurry, applying sharpening...")
    ...
    def _correct_brightness_and_contrast(
        ...
        if contrast < contrast_threshold:
            logger.info("Enhancing contrast with histogram equalization...")
    ...
    def _sharpen_image(self, gray_image):
        ...
        """
        logger.info("Applying sharpening via unsharp mask...")
    ...
    def _is_blurry(self, gray_image, threshold=100):
        ...
        logger.info(f"Laplacian variance (sharpness): {laplacian_var:.2f}")
    ...
    def _binarize_image(self, gray_image, use_otsu, block_size, C):
        ...
        """
        if use_otsu:
            logger.info("Applying OTSU threshold...")
            _, binary = cv2.threshold(gray_image, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
            return binary
        else:
            logger.info("Applying adaptive Gaussian threshold...")
