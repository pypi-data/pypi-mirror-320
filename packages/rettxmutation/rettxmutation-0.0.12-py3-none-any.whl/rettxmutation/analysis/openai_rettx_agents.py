...
class OpenAIRettXAgents:
    ...
    def extract_mutations(
        ...
        """
        try:
            logger.info("Running analysis with OpenAI")

            system_prompt = (
                "You are an expert in genetics. You must only extract MECP2 cDNA mutations explicitly mentioned in the provided text. "
            ...
            user_prompt = (
                ...
                f"Detected Keywords:\n{mecp2_keywords}\n\n"
                f"Detected Variants:\n{variant_list}\n\n"
                "Identify any cDNA mutations (e.g., c.XXXXC>T, c.XXXX_XXXXdel) related to MECP2 in the text using only the transcripts found in the keywords. "
        ...
        if not response.choices or not response.choices[0].message or response.choices[0].message.content == None:
            logger.error("No response provided by OpenAI")
            raise InvalidResponse("No response provided by OpenAI")
        logger.info(f"OpenAI extractor response: {response.choices[0].message.content}")
        ...
        for line in response.choices[0].message.content.split("\n"):
            try:
                ...
                logger.info(f"Valid mutation found: {mutation.to_hgvs_string()}")
                ...
                logger.error(f"Invalid mutation found: {line}")
                continue
    ...
    def summarize_report(
        ...
        """
        logger.info("Running report summarization with OpenAI")
    ...
    def correct_summary_mistakes(
        ...
        """
        logger.info("Running report summarization correction with OpenAI")
        ...
        system_prompt = (
            ...
            "Some mistakes are related with OCR errors (e.g., c.8080>T -> c.808C>T, mutations need to have nucleotide changes or deletions). "
