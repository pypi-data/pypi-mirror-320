...
from src.rettxmutation.analysis.ensembl_org import EnsemblOrgService, InvalidMutationError, GeneMutationCollection
...
def mock_latest_transcripts_file():
    ...
    with patch("src.rettxmutation.analysis.ensembl_org.open", mock_open(read_data=test_json)):
        yield
...
@patch("src.rettxmutation.analysis.ensembl_org.requests.Session.get")
def test_fetch_variations_success(mock_get, ensembl_service):
    ...
    # Call the method
    result = ensembl_service.fetch_variations("NM_004992.4", "c.916C>T")
...
def test_parse_to_model_success(mock_get, ensembl_service):
    ...
    api_response = ensembl_service.fetch_variations("NM_004992.4", "c.916C>T")

    # Parse the response into a GeneMutationCollection model
    result = ensembl_service.parse_to_model(api_response)
...
def test_fetch_variations_invalid_mutation(mock_get, ensembl_service):
    ...
    with pytest.raises(InvalidMutationError, match="The mutation position or base is invalid."):
        ensembl_service.fetch_variations("NM_004992.4", "invalid_variant")
...
def test_fetch_variations_connection_error(mock_get, ensembl_service, caplog):
    ...
    with pytest.raises(requests.exceptions.ConnectionError) as exc_info:
        ensembl_service.fetch_variations("NM_004992.4", "c.916C>T")
...
def test_get_gene_mutation_collection_success(mock_get, ensembl_service, mock_latest_transcripts_file):
    ...
    assert len(collection.mutations) == 1
