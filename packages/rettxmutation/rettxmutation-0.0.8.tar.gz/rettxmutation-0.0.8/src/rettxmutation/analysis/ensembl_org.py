...
class GeneMutationCollection(BaseModel):
    mutations: List[MutationDetail] = Field(..., description="List of mutations for the given gene")
    ...
    def get_mutation_by_transcript(self, transcript: str, notation_type: str = "hgvsc") -> Optional[str]:
        ...
        Parameters:
        - transcript: The transcript name to query.
        - notation_type: The type of notation to query ("hgvsc" for coding, "hgvsp" for protein).

        Returns:
        - The corresponding mutation string or None if no match is found.
        """
        if notation_type not in {"hgvsc", "hgvsp"}:
            raise ValueError("Invalid notation_type. Use 'hgvsc' or 'hgvsp'.")

        for mutation in self.mutations:
            notation_list = getattr(mutation, notation_type, [])
            for notation in notation_list:
                if notation.startswith(transcript):
                    # Return the part of the string after the colon (':') for hgvsc/hgvsp
...
class EnsemblOrgService:
    ...
    def _get_latest_transcript(self, transcript: str) -> str:
        ...
        if latest_version is None:
            logger.info(f"No versioning for transcript: '{base_id}'")
            return base_id
    ...
    def fetch_variations(self, transcript: str, variant: str) -> Dict[str, Any]:
        """
    ...
    def parse_to_model(self, api_response: List[Dict[str, Any]]) -> GeneMutationCollection:
        """
        ...
        try:
            ...
            mutations = [MutationDetail(**mutation) for mutation in normalized_response]

            return GeneMutationCollection(mutations=mutations)
        except Exception as e:
    ...
    def get_gene_mutation_collection(self, transcript: str, variant_description: str) -> GeneMutationCollection:
        ...
        """
        try:
            api_response = self.fetch_variations(transcript, variant_description)
            return self.parse_to_model(api_response)
