...
class TextCleaner:
    ...
    def clean_ocr_text(raw_text: str) -> str:
        ...
        # 3e) Add a missing dot if we see "c" directly followed by digits and a variant pattern (e.g., "c1234A>G" -> "c.1234A>G")
        text = re.sub(
        ...
        # 3f) If there's "c." anywhere, we might want to remove extraneous spaces after "c." (e.g., "c.  123A>G" -> "c.123A>G")
        text = re.sub(
            r"(c\.)\s+(\d)",
            r"\1\2",
            text
