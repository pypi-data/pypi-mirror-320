...
import xlsxwriter
import toad_fix
from toad_fix.transform import WOETransformer
...
def merger_data(data, var, unique_num,is_merge_high=True):
    if is_numeric_dtype(data[var]) and data[var].nunique() > unique_num:
        ...
        if is_merge_high:
            merge_high_data = toad_fix.utils.clip(data_nomiss[var], quantile=(None, .99))
        data_index, bins = toad_fix.merge(merge_high_data, method='step', return_splits=True, n_bins=unique_num)
...
def evaluate(test_data, excel_name = 'report.xlsx', num = 10, iv_threshold_value = 0.02, unique_num = 20, overdue_days = False, self_data = None):
    ...
    quality = toad_fix.quality(test_data.drop(columns=["loan_apply_no"]), target="target")
    quality.sort_values(by='iv',ascending=False,inplace=True)
