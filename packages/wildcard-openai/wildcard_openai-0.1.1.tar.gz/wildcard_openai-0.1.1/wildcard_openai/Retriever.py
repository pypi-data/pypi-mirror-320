...
class Retriever(BaseModel):
    ...
    async def get_tool_details_by_id(self, uuid: str) -> APISchema:
        tool_schema = await self.tool_search_client.get_action_schema(uuid)
        self.tool_search_client.logger.log("wildcard_spec", tool_schema.model_dump(mode="json"))
        return tool_schema
    ...
    async def get_tool_details(self, toolEnum: str) -> APISchema:
        ...
        tool_schema = await self.tool_search_client.get_action_schema(tool_id)
        self.tool_search_client.logger.log("wildcard_spec", tool_schema.model_dump(mode="json"))
        return tool_schema

    async def get_outside_context_tool(self, tool_enum: str, tool_schema: APISchema) -> dict:
        ...
        self.tool_search_client.logger.log("outside_context_tool", openai_tool.model_dump(mode="json"))
        return openai_tool.to_json()

    async def get_inside_context_tool(self, tool_enum: str, tool_schema: APISchema) -> dict:
        ...
        self.tool_search_client.logger.log("inside_context_tool", inside_tool.model_dump(mode="json"))
