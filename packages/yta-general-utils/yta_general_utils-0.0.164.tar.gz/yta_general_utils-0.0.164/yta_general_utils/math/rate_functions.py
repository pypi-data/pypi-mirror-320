...
class PairOfNodes:
    ...
    def get_n_xy_values_to_plot(self, n: int = 100, do_normalize: bool = False) -> list[tuple[NormalizableValue, NormalizableValue]]:
        ...
        xs = [
            ...
            for x in np.linspace(self.min_x, self.max_x, n)
        ]
        ys = [
    ...
    def get_y_from_not_normalized_x(self, x: float) -> NormalizableValue:
        ...
        # forced to linear.
        value = NormalizableValue(mRateFunction.EASE_IN_OUT_SINE.apply(x.normalized), (self.min_y, self.max_y), value_is_normalized = True)
    ...
    def get_y_from_normalized_x(self, x: float) -> NormalizableValue:
        ...
        # that calculates the 'y' value, by now is being
        # forced to linear.
        value = NormalizableValue(mRateFunction.EASE_IN_OUT_SINE.apply(x), (self.min_y, self.max_y), value_is_normalized = True)
