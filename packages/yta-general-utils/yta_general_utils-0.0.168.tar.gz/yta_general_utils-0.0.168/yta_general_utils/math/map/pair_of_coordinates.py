...
class PairOfCoordinates:
    ...
    def __init__(self, first_coordinate: Coordinate, second_coordinate: Coordinate, rate_function: mRateFunction = mRateFunction.EASE_IN_OUT_SINE):
    ...
    def get_y_from_not_normalized_x(self, x: float) -> NormalizableValue:
        """
        The X parameter must be a value between the first
        coordinate X and the second coordinate X.
        """
    ...
    def get_y_from_normalized_x(self, x: float) -> NormalizableValue:
        ...
        being 0 the first coordinate X and 1 the second
        coordinate X.
        """
    ...
    def _get_y_from_x(self, x: float, is_x_normalized: bool = False) -> NormalizableValue:
        ...
        # I must do 1 - self.rate_function.apply(value.normalized)
        # c1(400, 20), c2(300, 10), x = 340
        # 340 normalized is 0.4 [300, 400], but as we are from 400
        # to 300 it should be d=0.6
        value = NormalizableValue(self.rate_function.apply(1 - value.normalized if self.is_backwards else value.normalized), (self.min_y, self.max_y), value_is_normalized = True)
