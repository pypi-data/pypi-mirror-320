...
class MusaChecker(Checker):
    ...
    def inspect(self):
        if MusaModule.MUSAToolkits in self._check_list:
            ...
            self._log[MusaModule.musa_version.name] = (
                self._musa_shell.get_musa_version()
            )
...
class TorchMusaChecker(Checker):
    ...
    def inspect(self):
        ...
        if TorchMusaModule.PyTorch in self._check_list:
            ...
            self._log[TorchMusaModule.PyTorch.name] = (
                self._torch_musa_shell.get_package_status("torch")
            )
        if TorchMusaModule.Torch_musa in self._check_list:
            ...
            self._log[TorchMusaModule.Torch_musa.name] = (
                self._torch_musa_shell.get_package_status("torch_musa")
            )
        if TorchMusaModule.TorchVision in self._check_list:
            ...
            self._log[TorchMusaModule.TorchVision.name] = (
                self._torch_musa_shell.get_package_status("torchvision")
            )
        if TorchMusaModule.TorchAudio in self._check_list:
            ...
            self._log[TorchMusaModule.TorchAudio.name] = (
                self._torch_musa_shell.get_package_status("torchaudio")
            )
...
class vLLMChecker(Checker):
    ...
    def inspect(self):
        ...
        if vLLMModule.vLLM in self._check_list:
            self._log[vLLMModule.vLLM.name] = self._vllm_shell.get_package_status(
                "vllm"
            )
            self._log[vLLMModule.vLLM_version.name] = (
                self._vllm_shell.get_python_package_version("vllm")
            )
        if vLLMModule.MTTransformer in self._check_list:
            self._log[vLLMModule.MTTransformer.name] = (
                self._vllm_shell.get_pip_package_version("mttransformer")
            )
