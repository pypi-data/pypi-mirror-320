...
GPU_TYPE_MAP = {"0323": "S4000", "0202": "S70"}
...
class Diagnoser(ABC):
    ...
    def diagnose_version_match(self):
        if self._tag in self._version_requirements.yaml_data.keys():
            ...
            if self._check_status[module_name].status != Status.UNINSTALLED:
                ...
                # eg: 'container-toolkit' --> ['mtml', 'sgpu-dkms']
                modules_to_match_name_list = list(
                    self._version_requirements.yaml_data[self._tag][0][
                        "dependency"
                    ].keys()
                )
                ...
                if not special_version_dict:
                    self._check_status[
                        module_name
                    ].solution += f"\n{' '*SOLUTION_INDENTATION_LENGTH}The {(', ').join(modules_to_match_name_list)} version that matche the current {module_name} version not recorded."
                else:
                    ...
                    for k, v in modules_to_match_dict.items():
                        if (
                            ...
                            and self._check_status[k].version != v
                        ):
                        ...
                        if (
                            ...
                            self._check_status[HostModule.Kernel.name].version.split(
                                "("
                            )[0]
                            not in v
                        ):
...
class DriverDiagnoser(Diagnoser):
    ...
    def diagnose_dpkg_driver(self) -> None:
        ...
        """
        if DriverModule.Driver_Version_From_Dpkg.name in self._log:
            ...
            # no driver_version info(not installed driver)
                ...
                dpkg_driver_dict.status = Status.FAILED
        ...
        if ContainerToolkitModule.container_toolkit.name in self._log:
            ...
            if EXECUTED_ON_HOST_FLAG:
                ...
                if test_gmi_log[2]:
                    ...
                    toolkit_status.info = test_gmi_log[2]
                    if (
            ...
            if not musa_installed_flag:
                ...
                if musaInfo_code:
                    ...
                    if musa_dict.solution:
                        musa_dict.solution += "\nThe `musaInfo` execution failed, please check if musa_toolkits version matches the driver, or if the kernel version supports it."
                    else:
                        musa_dict.solution = "The `musaInfo` execution failed, please check if musa_toolkits version matches the driver, or if the kernel version supports it."
                else:
                    ...
                    if test_musa_code:
                        ...
                        if musa_dict.solution:
                            musa_dict.solution += "\nThe `musaInfo` execution was successful, but the `test_musa.mu` test failed, please check if the musa_runtime version and the driver version are compatible."
                        else:
                            musa_dict.solution = "The `musaInfo` execution was successful, but the `test_musa.mu` test failed, please check if the musa_runtime version and the driver version are compatible."
                    else:
...
class TorchMusaDiagnoser(Diagnoser):
    ...
    def _diagnose_package(
        ...
        version, git_version = self._log[git_version_module]
        if version[0] and not version[2]:
            pip_version = version[0].split()[-1]
            commit_id = None
            if git_version[0] and not git_version[2]:
                commit_id = git_version[0]
            final_version = f"{pip_version}+{commit_id}" if commit_id else pip_version
            check_status.version = final_version
            if run_demo_log[2]:
                if (
    ...
    def diagnose_pytorch(self) -> None:
        if TorchMusaModule.PyTorch.name in self._log:
            self._diagnose_package(
                TorchMusaModule.PyTorch.name, TorchMusaModule.PyTorch_version.name
            )

    def diagnose_torch_musa(self) -> None:
        if TorchMusaModule.Torch_musa.name in self._log:
            self._diagnose_package(
                TorchMusaModule.Torch_musa.name, TorchMusaModule.Torch_musa_version.name
            )
