...
class Checker(ABC):
    ...
    def __init__(self, check_list: list = list(), container_name: str = None) -> None:
        ...
        if not EXECUTED_ON_HOST_FLAG:
            ...
            if (
                self._in_docker and self._container_name
    ...
    def postcheck(self, check_status):
        """
        postcheck
        """
        # self.post_inspect()
    ...
    def report(self):
        ...
        status = "SUCCESSFUL" if self._is_successful else "FAILED"
        overall_status = FontGreen(f"{self._tag.upper()} CHECK OVERALL Status: ") + status
        ...
        if not self._is_successful:
            ...
            for key, root_cause_status in self._root_cause_status.items():
                print(
                    f"{root_cause_index}. {root_cause_status.name}".ljust(SUMMARY_LENGTH)
...
class HostChecker(Checker):
    ...
    def __init__(self, check_list: list = list(HostModule), container_name: str = None) -> None:
    ...
    def inspect(self):
        ...
        if HostModule.DKMS in self._check_list:
            self._log[HostModule.DKMS.name] = self._shell.get_dpkg_package_version("dkms")
        if HostModule.Lightdm in self._check_list:
            self._log[HostModule.Lightdm.name] = self._shell.get_dpkg_package_version("lightdm")
        # stage 4: Docker
...
class SmartIOChecker(Checker):
    ...
    def __init__(self, check_list: list = list(SmartIOModule), container_name: str = None) -> None:
        super().__init__(check_list, container_name)
...
class DriverChecker(Checker):
    ...
    def __init__(self, check_list: list = list(DriverModule), container_name: str = None) -> None:
        self._container_name = container_name
        if self._container_name:
            self._container_name = None
    ...
    def inspect(self):
        ...
        if DriverModule.Driver_Version_From_Clinfo in self._check_list:
            self._log[DriverModule.Driver_Version_From_Clinfo.name] = self._shell.clinfo_info()


class MTLinkChecker(Checker):

    def __init__(self, check_list: list = list(MTLinkModule), container_name: str = None) -> None:
        super().__init__(check_list, container_name)
...
class IBChecker(Checker):
    def __init__(self, check_list: list = list(IBModule), container_name: str = None) -> None:
        super().__init__(check_list, container_name)
...
class ContainerToolkitChecker(Checker):
    ...
    def inspect(self):
        if ContainerToolkitModule.container_toolkit in self._check_list:
            ...
            self._log[ContainerToolkitModule.is_binding.name] = self._shell.docker_runtime_info()
        ...
        if ContainerToolkitModule.mtml in self._check_list:
            self._log[ContainerToolkitModule.mtml.name] = self._shell.get_dpkg_package_version(
                "mtml"
            )
        if ContainerToolkitModule.sgpu_dkms in self._check_list:
            self._log[ContainerToolkitModule.sgpu_dkms.name] = self._shell.get_dpkg_package_version(
                "sgpu-dkms"
            )
...
class MusaChecker(Checker):
    ...
    def __init__(self, check_list: list = list(MusaModule), container_name: str = None) -> None:
        super().__init__(check_list, container_name)
        self._tag = CheckModuleNames.musa.name
        self._musa_shell = MusaShellExecutor(container_name)
        self._diagnoser = MusaDiagnoser(self._tag)
        if self._container_name or self._in_docker:
            self._precheckers = [
                DriverChecker(container_name=self._container_name),
                ContainerToolkitChecker(list(ContainerToolkitModule), self._container_name),
            ]
        else:
            self._precheckers = [DriverChecker(container_name=self._container_name)]
    ...
    def inspect(self):
        if MusaModule.MUSAToolkits in self._check_list:
            self._log[MusaModule.MUSAToolkits.name] = [
                self._musa_shell.get_musaInfo_info(),
                self._musa_shell.is_path_exists("/usr/local/musa"),
            ]
            self._log[MusaModule.musa_version.name] = self._musa_shell.get_musa_version()
            self._log[MusaModule.test_musa.name] = self._musa_shell.test_musa()
...
class TorchMusaChecker(Checker):
    ...
    def __init__(
        ...
        super().__init__(check_list, container_name)
        self._torch_musa_shell = TorchMusaShellExecutor(container_name)
    ...
    def inspect(self):
        ...
        if TorchMusaModule.PyTorch in self._check_list:
            self._log[TorchMusaModule.PyTorch_version.name] = [
                self._torch_musa_shell.get_pip_package_version("torch"),
                self._torch_musa_shell.get_python_package_git_version("torch"),
            ]
            self._log[TorchMusaModule.PyTorch.name] = self._torch_musa_shell.get_package_status(
                "torch"
            )
        if TorchMusaModule.Torch_musa in self._check_list:
            self._log[TorchMusaModule.Torch_musa_version.name] = [
                self._torch_musa_shell.get_pip_package_version("torch_musa"),
                self._torch_musa_shell.get_python_package_git_version("torch_musa"),
            ]
            self._log[TorchMusaModule.Torch_musa.name] = self._torch_musa_shell.get_package_status(
                "torch_musa"
            )
        if TorchMusaModule.TorchVision in self._check_list:
            self._log[TorchMusaModule.TorchVision_version.name] = [
                self._torch_musa_shell.get_pip_package_version("torchvision"),
                self._torch_musa_shell.get_python_package_git_version("torchvision"),
            ]
            self._log[TorchMusaModule.TorchVision.name] = self._torch_musa_shell.get_package_status(
                "torchvision"
            )
        if TorchMusaModule.TorchAudio in self._check_list:
            self._log[TorchMusaModule.TorchAudio_version.name] = [
                self._torch_musa_shell.get_pip_package_version("torchaudio"),
                self._torch_musa_shell.get_python_package_git_version("torchaudio"),
            ]
            self._log[TorchMusaModule.TorchAudio.name] = self._torch_musa_shell.get_package_status(
                "torchaudio"
            )


class vLLMChecker(Checker):
    def __init__(self, check_list: list = list(vLLMModule), container_name=None) -> None:
        super().__init__(check_list, container_name)
        self._tag = CheckModuleNames.vllm.name
        self._diagnoser = vLLMDiagnoser(self._tag)
        self._vllm_shell = vLLMShellExecutor(self._container_name)
    ...
    def inspect(self):
        ...
        if vLLMModule.vLLM in self._check_list:
            self._log[vLLMModule.vLLM.name] = self._vllm_shell.get_package_status("vllm")
            self._log[vLLMModule.vLLM_version.name] = self._vllm_shell.get_python_package_version(
                "vllm"
            )
        if vLLMModule.MTTransformer in self._check_list:
            self._log[vLLMModule.MTTransformer.name] = self._vllm_shell.get_pip_package_version(
                "mttransformer"
            )
...
if __name__ == "__main__":
    ...
    # status, log = hostcheck.postcheck()
    # print(status, log)
    # print("=====================")
