...
class PeekFileConfigPlatformMixin(metaclass=ABCMeta):
    ...
    def pluginMetricsPath(self, pluginName):
        ...
        with self._cfg as c:
            metricsPath = c.plugin.metricsPath(default, require_string)

        pluginMetricsDir = self._chkDir(os.path.join(metricsPath, pluginName))

        def updateDir(path: Path) -> None:
            mode = os.stat(path).st_mode
            if (mode & 0o050) != 0o050:
                os.chmod(path, mode | 0o050)

        def updateFile(path: Path) -> None:
            mode = os.stat(path).st_mode
            if (mode & 0o040) != 0o040:
                os.chmod(path, mode | 0o040)

        if metricsPath == default:
            updateDir(self._homePath)

        updateDir(metricsPath)
        updateDir(pluginMetricsDir)

        for root, dirs, files in os.walk(pluginMetricsDir):
            for d in dirs:
                updateDir(root / d)
            for f in files:
                updateFile(root / f)
