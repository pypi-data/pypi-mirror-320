...
if __package__ is None:
    from parser.pyproject import PyProject
    from matcher.spack import Spack
    from renderer.jinja2 import Renderer
    from logger.logger import ColorFormatter, FixedLineHandler
else:
...
def main() -> None:
    ...
    arg_parser.add_argument("--gitlab-url", nargs='?', type=str,
                            help="URL of python project's gitlab repo")
    arg_parser.add_argument("--branch", nargs='?', type=str, default="main",
                            help="Branch name to fetch the toml file, default is main")
    arg_parser.add_argument("--filepath", nargs='?', type=str,
                            help="Path of project pyproject.toml file")
    arg_parser.add_argument("--raw-content", nargs='?', type=str,
                            help="Raw content of pyproject.toml file")
    ...
    arg_parser.add_argument("--dbpath", nargs='?', default=Path(f"{current_dir}/spackdb.json"),
                            type=Path, help="path to spack database json file")
    arg_parser.add_argument("--templates-dir", nargs='?', default=Path(f"{current_dir}/templates/"),
    ...
    prj = PyProject(filepath=args.filepath,
                    raw_content=args.raw_content,
                    giturl=args.gitlab_url,
                    branch=args.branch)
    ...
    matcher = Spack(constraints=constraints,
                    dbpath=args.dbpath,
                    updatedb=args.update_spackdb)
    matches = matcher.match()

    renderer = Renderer(matches=matches,
                        metadata=metadata,
