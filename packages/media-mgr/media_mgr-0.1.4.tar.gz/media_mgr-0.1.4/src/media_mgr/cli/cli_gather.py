...
from ..server_cfg import get_available_server_types

from ..gather import show_retrieved_titles, show_all_retrieved_titles
from ..gather import show_title_bundles, store_title_bundles
from ..gather import store_title_bundles_plex_and_worker
...
def cli():
    try:
        ...
        if args.cmd == 'show.titles':
            show_retrieved_titles(ipv4 = args.ipv4, serverType = args.type)

        elif args.cmd == 'show.all.titles':
            show_all_retrieved_titles()

        elif args.cmd == 'show.bundles':
            show_title_bundles(ipv4 = args.ipv4, serverType = args.type)

        elif args.cmd == 'store.bundles':
            store_title_bundles(ipv4 = args.ipv4, serverType = args.type, storePath = args.path)
        ...
        exception_details(e, "Media Title Gathering CLI", raw=True)
