...
import numpy as np
import pandas as pd
from atedu.tframe import sysclsbase as cnt
from atedu.tframe.language import text
from atedu.tframe import ufuncs
from atedu.tframe.udefs import INTEGERS_TYPE
from atedu.tframe.utils.datetimefunc import (is_format_dt, str_date_to_mft, datetime_to_mft)
...
@smm.force_phase(gv.RUMMODE_PHASE_USERINIT)
@apply_rule(verify_that('factor').is_instance_of(str, list, tuple))
def reg_custom_factor(factor: 'list'):
    if isinstance(factor, str):
        factor = [factor]
    else:
        factor = list(factor)
    ...
    reg_idx = _regfuncs.reg_custom_factor(factor,
                                   strategy_input.TargetList,
                                   strategy_input.BeginDate,
                                   strategy_input.EndDate)

    cnt.env.user_context.reg_custom_factor.append({'reg_idx': reg_idx, 'customfactor': factor})
...
@apply_rule(verify_that('reg_idx').is_instance_of(dict),
            ...
            verify_that('df').is_in((True, False)),
            verify_that('sort_by').is_in(('factor', 'target_idx', 'date')))
def get_custom_reg_factor(reg_idx,
                   target_indices=(),
                   length=1,
                   df=False,
                   sort_by='target_idx'):
    ...
    if len(target_indices) < 1:
        ...
        _, target_indices = cnt.env.check_idx('get_custom_reg_factor', None, target_indices, toarray=True)

    factor = reg_idx.get('customfactor', [])
    reg_idx_ = reg_idx.get('reg_idx')[:, target_indices].ravel()

    bar_pos = cnt.env.get_internal_fresh_bar_num(np.nan)
    dt, value = _regfuncs.get_reg_custom_factor(reg_idx_, bar_pos, length)
    if value.size < 1:
        return None
