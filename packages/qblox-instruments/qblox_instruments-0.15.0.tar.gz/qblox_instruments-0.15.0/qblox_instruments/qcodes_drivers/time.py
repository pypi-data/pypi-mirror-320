...
class Time(Component):
    """
    This class represents the time block, used to contain time-related functionality. It is not a physical block,
    in the way that quad, module or sequencer are.
    """
    ...
    def __init__(
        ...
        # Initialize instrument channel. Name is not variable because there can only be one time block.
    ...
    def _get_required_cluster_attr_names() -> List:
        ...
        # Time attributes
        attr_names = []
        attr_names.append("_adjust_timekeeping")
        attr_names.append("_arm_timekeeping_capture_slot")
        attr_names.append("_get_current_timestamp")
        attr_names.append("_get_timekeeping_capture_slot")
        attr_names.append("_get_timekeeping_status")
        attr_names.append("_sync_to_ntp")
    ...
    def _convert_time_format(
        ...
        from_format: str
            The source format; "s" for int/float seconds, "ns" for int/float nanoseconds, "raw" for the internal time format as an integer, or "json" for the JSON serialization (respecting max 64-bit integers and avoiding float rounding error).
        to_format: str
        ...
        if from_format == "json":
            ...
            raise ValueError(
                f"Unknown source format {from_format}. Options are: s, ns, raw, json"
            )
        ...
        raise ValueError(
            f"Unknown destination format {to_format}. Options are: s, ns, raw, json"
        )
    ...
    def get_current(self, format: str = "s") -> Union[int, float]:
        ...
        return self._convert_time_format(
            self._get_current_timestamp(), to_format=format
        )
    ...
    def get_capture(self, slot: int, format: str = "s") -> Union[int, float]:
        ...
        """
        return self._convert_time_format(
            self._get_timekeeping_capture_slot(slot), to_format=format
        )
    ...
    def adjust(self, format: str, amount: Union[int, float]) -> None:
        ...
        return self._adjust_timekeeping(
            self._convert_time_format(
                time_obj=amount, from_format=format, to_format="json"
            )
        )
    ...
    def arm(
        ...
        falling) and the whether to synchronize to the 10 MHz clock can also be
        specified
        ...
        ----------
        """
        return self._arm_timekeeping_capture_slot(
            slot, channel, edge_polarity, sync_ref
        )
    ...
    def sync_ext_trigger(
        ...
        timeout: Union[float, int]
            How long to wait for a capture while synchronizing, expressed in the unit given by the format argument
        ...
        self._arm_timekeeping_capture_slot(slot, channel, edge_polarity, sync_ref)
    ...
    def set_epoch(
        ...
        sync_ctc += (
            one_second // 2
        )  # add half a second more to align to middle of a second
