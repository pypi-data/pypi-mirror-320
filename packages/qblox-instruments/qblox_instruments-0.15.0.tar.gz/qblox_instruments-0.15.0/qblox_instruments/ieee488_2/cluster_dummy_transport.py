...
class ClusterDummyTransport(DummyTransport):
    ...
    def __init__(self, dummy_cfg: Dict[Union[str, int], ClusterType]):
        ...
        self._modules = {
            str(slot_idx): None for slot_idx in range(1, self._num_slots + 1)
        }
        for slot_idx in dummy_cfg:
            if str(slot_idx) in self._modules:
                self._modules[str(slot_idx)] = ModuleDummyTransport(dummy_cfg[slot_idx])
        ...
        self._cmds["*DESCribe?"] = self._get_json_description
    ...
    def _get_current_timestamp(
        ...
        if len(self._time_dict) > 0:
            ...
            self._bin_out = self._encode_bin(
                json.dumps(self._time_dict).encode("utf-8")
            )
    ...
    def _get_timekeeping_capture(
        ...
        if len(self._capture_dict) > 0:
            ...
            self._bin_out = self._encode_bin(
                json.dumps(self._capture_dict).encode("utf-8")
            )

    # ------------------------------------------------------------------------
    def _get_json_description(
        self, cmd_params: list, cmd_args: list, bin_in: bytes
    ) -> None:
        """
