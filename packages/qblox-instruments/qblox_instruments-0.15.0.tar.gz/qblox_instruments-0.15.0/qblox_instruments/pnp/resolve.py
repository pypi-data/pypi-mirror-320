...
def _populate_ports(
    protocol: str, address: str, slot_index: Optional[int]
) -> AddressInfo:
    """
    ...
    *hint, identifier = identifier.split("://", maxsplit=1)
    ...
    if hint != "pnp":
        try:
            ...
            logger.info(
                '"%s" is a valid IP address; skipping name resolution.', identifier
            )
    ...
    if hint != "ip":
        logger.info(
            'Trying to resolve "%s" as device name or serial number '
            "via plug & play...",
            identifier,
        )
        data = None
    ...
    if hint != "pnp":
        ...
        try:
            ...
            if data:
                ...
                logger.info(
                    'Found "%s" at IP address %s via DNS lookup', identifier, remote_ip
                )
                return _populate_ports("ip", remote_ip, slot_index)
    ...
    raise RuntimeError(
        f'Failed to resolve "{full_identifier}" as something we can connect to'
    )
