...
def add_indent(lines: List[str], spaces: int = 4) -> List[str]:
    """
    Adds a n-space indent to every line in a list of lines.
    """
...
def get_parameters_from_doc(lines: str) -> Dict[str, str]:
    ...
    param_and_type_regex = re.compile(
        r"^\s*([0-9A-Za-z_]{1}[A-Za-z_]*)\s*:?\s*([A-Za-z,\[\]]*)$"
    )
...
def partial_with_numpy_doc(func, /, *args, end_with="", **kwargs):
    """
