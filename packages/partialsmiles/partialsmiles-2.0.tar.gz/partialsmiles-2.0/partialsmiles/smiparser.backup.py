...
class Molecule:
    ...
    def addAtom(self, symbol, prev, bondchar):
        ...
        if prev is not None:
            ...
            bondidx = self.addBond(prev, atom, bo)
            if bondchar == ':' or (atom.arom and prev.arom and not bondchar):
                self.bonds[bondidx].arom = True
        return atom

    def addBond(self, beg, end, bo):
        bond = Bond(beg.idx, end.idx, bo)
        N = len(self.bonds)
        bond.idx = N
        beg.bonds.append(N)
        end.bonds.append(N)
    ...
    def getBond(self, atomA, atomB):
        for nbr_bondidx in atomA.bonds:
            nbr_bond = self.bonds[nbr_bondidx]
...
class SmilesParser:
    ...
    def validateKekulization(self, mol, state):
        ...
        for atom in mol.atoms:
            ...
            while len(stack):
                ...
                for bond_idx in curr.bonds:
                    bond = mol.bonds[bond_idx]
                    if bond.arom:
                        bond_marker[bond.idx] = arom_system
                        nbr_idx = bond.getNbr(curr.idx)
                        if not seen[nbr_idx]:
                            stack.append(mol.bonds[nbr_idx])
    ...
    def handleBCSymbol(self, state):
        ...
        if bcsymbol in state.openbonds:
            ...
            if state.mol.getBond(opening, state.prev[-1]) == -1:
