...
def get_cases_from_api(token, start_date, end_date, limit):
    cases = []
    try:
        headers = {"Authorization": f"Bearer {token}"}
        result = requests.get(f"https://officer.thaipoliceonline.go.th/api/e-form/v1.0/BpmProcInst/workflow/task-list-new?RequireTotalCount=true&RoleCode=ROOT&Offset=0&Length={limit}&SortSelector=TrackingCode&SortDesc=true&StartDate={start_date}&EndDate={end_date}&CategoryId=1&Casetype=&IsCheck=&RequireStuckCase=false", headers=headers)
