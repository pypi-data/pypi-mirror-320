...
@patch("carde.image_processing.imsave")
@patch("carde.image_processing.evaluate_segmentation")
@patch("carde.image_processing.read_image_pair")
@patch("carde.image_processing.crop_away_metadata")
@patch("carde.image_processing.segment_combined")
@patch("carde.image_processing.Path.glob")
...
def test_process_folder(
    ...
    mock_natsorted,
    mock_glob,
    mock_segment_combined,
    mock_crop_away_metadata,
    mock_read_image_pair,
    mock_evaluate_segmentation,
    mock_imsave,
):
    # Setup mock paths
    mock_image_path1 = MagicMock(spec=Path)
    ...
    mock_read_image_pair.side_effect = [
        (np.zeros((100, 100), dtype=np.uint8), np.zeros((100, 100), dtype=np.uint8)),
        (np.zeros((100, 100), dtype=np.uint8), np.zeros((100, 100), dtype=np.uint8)),
    ]
    mock_crop_away_metadata.side_effect = [
        np.zeros((90, 100), dtype=np.uint8),
        np.zeros((90, 100), dtype=np.uint8),
        np.zeros((90, 100), dtype=np.uint8),
        np.zeros((90, 100), dtype=np.uint8),
    ]
    mock_segment_combined.side_effect = [
        np.ones((90, 100), dtype=np.uint8),
        np.ones((90, 100), dtype=np.uint8),
    ]
    mock_evaluate_segmentation.side_effect = [
        pd.DataFrame({"label": [1], "area": [100]}),
        pd.DataFrame({"label": [1], "area": [100]}),
    ]
    ...
    assert mock_crop_away_metadata.call_count == 2
    assert mock_segment_combined.call_count == 1
    assert mock_evaluate_segmentation.call_count == 1
    assert mock_imsave.call_count == 1
    assert mock_to_csv.call_count == 1
    args, kwargs = mock_imsave.call_args_list[0]
    assert args[0] == Path("/mock/output_folder/image_01-02_label.tif")
    np.testing.assert_array_equal(args[1], np.ones((90, 100), dtype=np.uint8))
