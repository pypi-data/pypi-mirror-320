...
class COHPStartZero(_BasePathOut):
    ...
    def read(path, store=False):
        ...
        if os.path.isfile("ICOHPLIST.lobster"):
            ...
            if len(list(icohplist.icohplist.keys())) == 1:
                data = np.array([
                ...
                result_single1.columns = ["length", "icohp*-1 (up)", "-icohp*-1 (down)"]
            elif len(list(icohplist.icohplist.keys())) >= 1:
                # st = Structure.from_file("POSCAR")
                # name_list = [f"{n.symbol}{i}" for i, n in enumerate(st.species)]
                ic = icohplist.icohpcollection
                atoms1 = ic._list_atom1
                atoms2 = ic._list_atom2
                labels = ic._list_labels
                lengths = ic._list_length
                icohps_up = [-i[Spin.up] for i in ic._list_icohp]
                icohps_down = [-i[Spin.down] for i in ic._list_icohp]
                icohps_mean = [(-i[Spin.up]-i[Spin.down])/2 for i in ic._list_icohp]
                ...
                ta = np.array(["File", "Atom 1", "Atom 2", "labels", "length",
                               "icohp*-1 (up)", "icohp*-1 (down)", "icohp*-1 (mean)",
                               ])
                result_single1 = pd.DataFrame(data, columns=ta)

            else:
                print(f"Bad data for {path}")
        ...
        if os.path.isfile("COHPCAR.lobster"):
            ...
            if "2" not in cohpcar.cohp_data:
                data = cohpcar.cohp_data["average"]["COHP"][Spin.up].ravel()
                ...
                labels=[]
                lengths=[]
                for k, v in cohpcar.cohp_data.items():
                    if k == "average":
                        continue
                    cohps_up.append(v["COHP"][Spin.up].ravel())
                    cohps_down.append(v["COHP"][Spin.down].ravel())
                    atoms1.append(v["sites"][0])
                    atoms2.append(v["sites"][1])
                ...
                ta = np.array(["File", "Atom 1", "Atom 2", "labels", "length", "cohp (up)", "cohp (down)",
                               ])
                result_single2 = pd.DataFrame(data, columns=ta)
            else:
                print(f"no data for {path}")
    ...
    def batch_after_treatment(self, paths, res_code):
        ...
        if data_all2.shape[0]==len(paths):
            result2 = pd.DataFrame(data_all2, columns=col, index=paths)
        else:
            result2 = pd.DataFrame(data_all2, columns=col)
