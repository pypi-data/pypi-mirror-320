...
class DataCenter:
    ...
    def funding_client(self):
        """Get the funding client instance."""
    ...
    async def close(self):
        ...
        if hasattr(self, '_clients'):
            for client in self._clients.values():
                ...
                # Clean up rate limiter if present
                if hasattr(client, 'rate_limiter') and hasattr(client.rate_limiter, 'stop'):
                    await client.rate_limiter.stop()
        self._clients = {}
            ...
            while depth < max_depth:
                ...
                if len(pending_funders) >= BATCH_SIZE or depth == max_depth - 1:
                    ...
                    if self.label_client:
                        try:
                            label_results = self.label_client.get_addresses_labels(pending_funders)
                            label_map = {
    ...
    async def build_tx_nodes(self, tx_hashes: List[str]) -> List[Dict[str, Any]]:
        ...
        for tx_hash, tx in txs_data.items():
            ...
            node = {
                "txHash": tx_hash,
                "blockNumber": int(tx.get('Block', 0)),
