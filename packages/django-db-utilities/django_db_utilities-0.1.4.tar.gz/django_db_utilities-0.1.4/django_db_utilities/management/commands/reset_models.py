...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if app_name is None:
            return  # Exit if app selection fails

        # Let the user select the model
        model_name = self.select_model(app_name)

        if model_name is None:
            return  # Exit if model selection fails
    ...
    def reset_model_data(self, app_name, model_name):
        ...
        model = None
        try:
            # First attempt: Use the full app path if available (submodules)
            if '.' in app_name:
                model = apps.get_model(f'{app_name}.{model_name}')
            else:
                model = apps.get_model(app_name, model_name)
        except LookupError:
            self.stdout.write(self.style.WARNING(f"Failed to find model '{model_name}' with full path '{app_name}.{model_name}'. Trying partial path."))

        if model is None:
            try:
                # Fallback attempt: Use just the base app name (partial path)
                model = apps.get_model(app_name.split('.')[-1], model_name)
            except LookupError:
                self.stdout.write(self.style.ERROR(f"Model '{model_name}' in app '{app_name}' not found."))
                return
            except Exception as e:
                self.stdout.write(self.style.ERROR(f"Error during fallback lookup for model '{model_name}': {str(e)}"))
                return

        # If model is found, proceed with the reset
        if model:
