...
class Command(BaseCommand):
    ...
    def select_model(self, app_name):
        ...
        # Check if the app name contains a dot (.)
        if '.' in app_name:
            app_parts = app_name.split('.')
            # Try to find the correct app by splitting
            app_name = app_parts[-1]  # Get the last part as the actual app name
            self.stdout.write(self.style.WARNING(f"Splitting app name, using '{app_name}'"))

        try:
            # Try getting the app configuration
