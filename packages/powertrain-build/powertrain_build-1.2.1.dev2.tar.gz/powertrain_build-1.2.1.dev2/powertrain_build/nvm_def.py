        ...
        def write_signals():
            for memory_area in self._nvm_memory_areas:
                ...
                for signal in signals:
                    ...
                    hptr.write(f'   {signal["type"]:7} {self._struct_member_prefix}{signal["name"]}')
                    size_string = ""
                    ...
                    defines.append(
                        (
                            signal["name"],
                            f"{prefix.lower()}{memory_area.lower()}.{self._struct_member_prefix}{signal['name']}"
                        )
                    )
    ...
    def _generate_rte_type_nvm_config_headers(self):
            ...
            for signal in signals:
                ...
                if memory_area_with_prefix in self.valid_nvm_definitions:
                    defines.append(
                        (
                            signal["name"],
                            f"{memory_area_with_prefix.lower()}.{self._struct_member_prefix}{signal['name']}"
                        )
                    )
                else:
                    defines.append((signal["name"], 0))  # Needed for compilation to work, undefined variable otherwise
        ...
        def write_extern():
            ...
            for area in self.valid_nvm_definitions.keys():
                hptr.write(f"extern dt_{area} {area.lower()};\n")
            hptr.write(f'#include "{self._predecl_end}"\n')
