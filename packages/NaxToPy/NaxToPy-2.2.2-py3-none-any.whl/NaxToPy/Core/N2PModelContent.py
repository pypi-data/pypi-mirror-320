...
from NaxToPy.Core.Classes.N2PProperty import N2PProperty, N2PComp, N2PShell, N2PSolid, N2PBeam, N2PRod, N2PBush, N2PFast, N2PMass, N2PElas, N2PGap, N2PWeld
...
class N2PModelContent:
    ...
    def MaterialDict(self) -> dict[int, N2PMaterial]:
        ...
        if not self.__material_dict:
            ...
            self.__material_list = list(self.__material_dict.values())
        t2 = time.time()
    ...
    def get_elements_attached(self,
        ...
        if not domain:
            ...
            n_con = sum(len(connector) if isinstance(connector, list) else 1 for connector in self.__connector_dict.values())
            csharp_bool_array = System.Array.CreateInstance(System.Boolean,
                                                            len(self.__element_dict) + n_con)
    ...
    def get_result_by_LCs_Incr(self, list_lc_incr: Union[list[tuple["N2PLoadCase", "N2PIncrement"], ...],
                               list[tuple[int, int], ...]], result: str, component: str, sections: list = None, aveSections=-1,
                               cornerData=False, aveNodes=-1, variation=100, realPolar=0, coordsys: int = -1000,
                               v1: tuple = (1, 0, 0), v2: tuple = (0, 1, 0),
                               filter_list: list[Union[N2PElement, N2PNode]] = None) -> dict[tuple: "ndarray"]:
        ...
        Returns:
            result_dict: dict[tuple: np.ndarray]
                results for the component ordered in a dictionary
