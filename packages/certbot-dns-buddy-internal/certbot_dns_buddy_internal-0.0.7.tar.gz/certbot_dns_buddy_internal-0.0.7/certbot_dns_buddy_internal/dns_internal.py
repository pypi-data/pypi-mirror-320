...
class Authenticator(dns_common.DNSAuthenticator):
    ...
    def _perform(self, domain, validation_name, validation):
        ...
        decoded_subdomain = idna.decode(subdomain)
        try:
            logger.debug("perform %s, %s", decoded_domain, decoded_subdomain, validation)
            self._api_client().add_txt_record(decoded_subdomain, validation)
    ...
    def _cleanup(self, domain, validation_name, validation):
        ...
        decoded_subdomain = idna.decode(subdomain)
        try:
            logger.debug("cleanup %s, %s", decoded_domain, decoded_subdomain, validation)
            self._api_client().del_txt_record(decoded_subdomain, validation)
        except ValueError as err:
