...
class IIDefault:
    ...
    def get_ipv4(server_name:str):
        if IIDefault.is_text_ivp4(server_name):
            return server_name
    ...
    def text_shortcut_to_bytes(text:str):
        try:
            if text.startswith("i:"):
                ...
                return IIDefault.integer_to_bytes(integer)
            elif text.startswith("ii:"):
                index, integer = text.split(":")[1].split(",")
                return IIDefault.index_integer_to_bytes(int(index), int(integer))
            elif text.startswith("iid:"):
                index, integer, delay = text.split(":")[1].split(",")
                
                return IIDefault.index_integer_now_relay_milliseconds_to_bytes(int(index), int(integer), int(delay))
            else:
                while "  " in text:
                    text = text.replace("  ", " ")
                ...
                if size == 1:
                    ...
                    return IIDefault.index_integer_to_bytes(index, integer)
                elif size == 3:
                    index = int(tokens[0])
                    integer = int(tokens[1])
                    delay = int(tokens[2])
                    return IIDefault.index_integer_now_relay_milliseconds_to_bytes(index, integer, delay)
                else:
                    integer = int(text)
                    return IIDefault.integer_to_bytes(integer)
    ...
    def get_random_integer_100():
        return IIDefault.get_random_integer(0, 100)
    def get_random_integer_int_max():
        return IIDefault.get_random_integer(-2147483647, 2147483647)

    def get_random_integer_int_max_positive():
        return IIDefault.get_random_integer(0, 2147483647)
            
    def i(integer_value:int):
        return IIDefault.integer_to_bytes(integer_value)

    def ii(index:int, integer_value:int):
        return IIDefault.index_integer_to_bytes(index, integer_value)

    def iid(index:int, integer_value:int, date:int):
        return IIDefault.index_integer_date_to_bytes(index, integer_value, date)

    def iid_ms(index:int, integer_value:int, milliseconds:int):
        return IIDefault.index_integer_date_to_bytes(index, integer_value, milliseconds)
...
def set_global_ntp_offset_in_milliseconds(ntp_server=IIDefault.default_ntp_server):
...
class SendUdpIID:
    ...
    def __init__(self, ivp4, port, use_ntp:bool, use_queue_thread:bool=False):
        ...
        self.ivp4 = IIDefault.get_ipv4(ivp4)
        self.port = port
    ...
    def push_integer_in_queue(self, value:int, delay_in_milliseconds:int):
        self.queue_thread.push_bytes_to_queue(IIDefault.integer_to_bytes(value), delay_in_milliseconds)
    def push_index_integer_in_queue(self, index:int, value:int, delay_in_milliseconds:int):
        self.queue_thread.push_bytes_to_queue(IIDefault.index_integer_to_bytes(index, value), delay_in_milliseconds)
    ...
    def listen(self):
        while True:
            try:
                ...
                size = len(data)
                if size == 4:
                    value = IIDefault.bytes_to_int(data)
                    if self.on_receive_integer:
                        self.on_receive_integer(value)
                elif size == 8:
                    index, value = IIDefault.bytes_to_index_integer(data)
                    if self.on_receive_index_integer:
                        self.on_receive_index_integer(index, value)
                elif size == 12:
                    index, value, date = IIDefault.bytes_to_index_integer_date(data)
