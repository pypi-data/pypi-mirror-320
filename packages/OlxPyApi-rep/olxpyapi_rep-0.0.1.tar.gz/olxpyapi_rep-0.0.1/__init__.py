...
def __get_allowed_urls():
    allowed_urls = []
    [allowed_urls.append(url) for url in __OLX_URLS_EXAMPLE]
    for base_url in __OLX_URLS_EXAMPLE:
        bs4_soup, _ = __get_soup(base_url)
...
class OlxParser:
    ...
    def get_products(self, url, amount=0) -> list:
        global __OLX_URLS_EXAMPLE, __get_allowed_urls, __print_lock, __get_soup
        base_url = f"{url.split('/')[0]}//{url.split('/')[2]}"
        ad_urls = []
        ads = []
        allowed_urls = __get_allowed_urls()

        if amount < 0 or type(amount) == float:
            raise InvalidAmount(amount)

        if url.startswith(tuple(allowed_urls)):
            if not (url == __OLX_URLS_EXAMPLE[0] or
                    url == __OLX_URLS_EXAMPLE[1] or
                    url == __OLX_URLS_EXAMPLE[2] or
                    url == __OLX_URLS_EXAMPLE[3]):
                try:
                    ...
                    def get_ad_info(ad):
                        ...
                        if ad.ad_url:
                            try:
                                bs4_soup, bs4_response_code = __get_soup(ad.ad_url)
                                with __print_lock:
                                    if self.logging:
                    ...
                    try:
                        soup, response_code = __get_soup(url)
                    ...
                    if int([element for element in total_count.split() if element.isdigit()][0]) != 0:
                        ...
                        if pagination_list is None:
                            ...
                            with ThreadPoolExecutor(max_workers=4) as executor:
                                futures = {executor.submit(__get_soup, f"{url}{page_index + 1}"): page_index for page_index in
                                           range(max_page)}
