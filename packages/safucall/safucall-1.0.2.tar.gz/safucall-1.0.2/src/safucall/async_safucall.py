...
class AsyncSafucall:
    ...
    class Function:
        class FCall:
            def __init__(self, function: "AsyncSafucall.Function", params: list):
            ...
            async def call(
                ...
                calls = [
                    AsyncCall(self.function.parent._target, self.function.name, args)
        ...
        def __init__(self, name: str, parent: "AsyncSafucall"):
            self.name = name
            self.parent = parent
    ...
    async def setup(
        ...
        target_abi: str,
        w3: AsyncWeb3 = None,
        multicall: AsyncMulticall = None,
    ):
        if not w3 and not multicall:
            raise TypeError(
                "__init__() missing 1 required argument: 'w3' or 'multicall' (at least one required)"
            )
        if multicall:
        ...
        self._target = w3.eth.contract(
            w3.to_checksum_address(target_address), abi=target_abi
        )
        self._functions = {}
        self._setup_functions()

    def __getattr__(self, function_name: str) -> "AsyncSafucall.Function":
    ...
    def _setup_functions(self):
        for func in filter(
            lambda x: x.get("stateMutability") in ("view", "pure"), self._target.abi
        ):
            function = AsyncSafucall.Function(func["name"], self)
