...
class DaskRunner:
    ...
    def create_dag(
        dag: DAG,
    ) -> tuple[DaskDagType, DaskOutputType]:
        """
        Format the DAG into the format dask expects
        """
        dsk = {}
        for process in dag.dag:
            dsk[process.name] = (apply, process.function, process.args, process.kwargs)
        return dsk, dag.output

    def run(self, dag: DAG, **kwargs) -> Any:
        ...
        """
        dsk, output = self.create_dag(dag)
