...
_VIC_VEG_PATH = (Path(__file__).parent.parent).joinpath("data/vic_veg_lookup.csv")
    ...
    domain_ds = xr.Dataset(
        {
            ...
            "run_cell": mask_da.astype("float64"),
        }
    )
...
def _build_vic_soil(salient_ds: xr.Dataset, soil_depths: list[float] = [0.1, 0.3, 1]):
    ...
    return xr.merge(
        [salient_ds[["elev", "annual_prec"]].astype("float64"), non_depth_ds, depth_ds]
    )


def _build_vic_veg(salient_ds: xr.Dataset, start: str, end: str):
    ...
    veg_lu = pd.read_csv(_VIC_VEG_PATH)
    ...
    veg_month_df = veg_month_df.merge(
        veg_lu.loc[:, ["veg_class", "igbp_value", "overstory", "veg_height"]],
        left_on="lulc_igbp",
        right_on="igbp_value",
        how="left",
    )
    ...
    veg_month_agg_df = (
        veg_month_df.loc[:, ["veg_class", "month", "albedo", "LAI", "veg_rough", "displacement"]]
