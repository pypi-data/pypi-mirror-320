...
def confirm_preauth(payment: OrderPayment, provider: XPayPaymentProvider):
    ...
    try:
        ...
        raise PaymentException(_("An error occurred with the XPay's servers while capturing the order. "
                                 ...
                                 "Be sure to remember the transaction code #%s. Exception: %s") % (f"{payment.order.code}-{transaction_code}", repr(e)))
    ...
    if (result["esito"] == "KO"):
        ...
        raise PaymentException(_('Preauth confirm request failed with error code %d: %s. '
                                 'Contact the event organizer and check if your order is successfull and the correct amount of '
                                 'money has been trasferred from your account. '
                                 'Be sure to remember the transaction code '
                                 '#%s') % (result["errore"]["codice"], result["errore"]["messaggio"], f"{payment.order.code}-{transaction_code}"))
    elif (result["esito"] == "OK"):
        if (hmac != result["mac"]):
            logger.error(f"XPAY_confirm_preauth [{payment.full_id}]: HMAC verification failed.")
            raise PaymentException(_('Unable to validate the preauth confirm. '
                                     'Contact the event organizer and check if your order is successfull and the correct amount of '
                                     'money has been trasferred from your account. '
                                     'Be sure to remember the transaction code #%s') % f"{payment.order.code}-{transaction_code}")
        ...
        raise PaymentException(_('Unknown server response (%s) in the preauth confirm process. '
                                 'Contact the event organizer and check if your order is successfull and the correct amount of '
                                 'money has been trasferred from your account. '
                                 'Be sure to remember the transaction code #%s') % (result["esito"], f"{payment.order.code}-{transaction_code}"))
...
def refund_preauth(payment: OrderPayment, provider: XPayPaymentProvider):
    ...
    try:
        ...
        raise PaymentException(_("An error occurred with the XPay's servers while issuing a refund of a preauth. "
    ...
    if (result["esito"] == "KO"):
        ...
        raise PaymentException(_('Preauth refund request failed with error code %d: %s. Contact the event organizer to execute the refund manually. '
                                 'Be sure to remember the transaction code '
                                 '#%s') % (result["errore"]["codice"], result["errore"]["messaggio"], f"{payment.order.code}-{transaction_code}"))
    elif (result["esito"] == "OK"):
        if (hmac != result["mac"]):
            ...
            raise PaymentException(_('Unable to validate the preauth refund. Contact the event organizer to execute the refund manually. '
                                     'Be sure to remember the transaction code #%s') % f"{payment.order.code}-{transaction_code}")
        ...
        raise PaymentException(_('Unknown server response (%s) in the preauth confirm process. Contact the event organizer to execute the refund manually. '
                                 'Be sure to remember the transaction code #%s') % (result["esito"], f"{payment.order.code}-{transaction_code}"))
...
def get_order_status(payment: OrderPayment, provider: XPayPaymentProvider) -> OrderStatus:
    ...
    if (result["esito"] == "KO"):
        ...
        raise ValueError(_('Unable to check the order status for %s. Error code: %d. Error message: "%s"')
...
def refund(refund: OrderRefund, provider: XPayPaymentProvider):  # Same endpoint of preauth refund, but slightly different logic
    ...
    try:
        ...
        raise PaymentException(_("An error occurred with the XPay's servers while refunding the order. "
                                 ...
                                 "Be sure to remember the transaction code #%s. Exception: %s") % (f"{refund.order.code}-{transaction_code}", repr(e)))
    if (result["esito"] == "KO"):
        logger.error(f"XPAY_refund [{refund.payment.full_id}@{refund.full_id}]: refund request failed gracefully.")
        raise ValueError(_('Unable to refund payment %s. Error code: %d. Error message: "%s"')
                         % (transaction_code, result["errore"]["codice"], result["errore"]["messaggio"]))
    if (result["esito"] != "OK"):
...
def post_api_call(provider: XPayPaymentProvider, path: str, params: dict):
    ...
    try:
        ...
        r = requests.post(f"{get_xpay_api_url(provider)}{path}", json=params, timeout=31.5)
        r.raise_for_status()
        return r.json()
