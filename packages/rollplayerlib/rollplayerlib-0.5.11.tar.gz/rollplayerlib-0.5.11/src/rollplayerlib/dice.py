...
class RollResult:
    ...
    def _format_numbers(self, numbers: List[int]):
        ...
        if self.threshold is not None:
            ...
            return [(f'**{x:.4f}**' if type(x) == float else f'**{x}**') if pass_result else (f'{x:.4f}' if type(x) == float else str(x)) for x, pass_result in zip(rounded_nums, passing_nums)]
        else:
            return [f'{x:.4f}' if type(x) == float else str(x) for x in rounded_nums]
