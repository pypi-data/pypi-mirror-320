...
def expired_idents(global_state: GlobalState, image: Image, now) -> Iterable[tuple[str, str]]:
    ...
    if not image.tags:
        # FIXME(frans, CMK-20814): check if tag_default should be applied to untagged images as well
...
async def update_task_list(ui: DockerShaperUI) -> None:
    ...
    tasks = [
        ...
        if task_name not in {"update header clock", "screen_update", "Animator", "tooltip-timer"}
        if coro.__name__ not in {"run_async", "_run", "sleep", "add_next", "iterate"}
    ]
    for task in tasks:
