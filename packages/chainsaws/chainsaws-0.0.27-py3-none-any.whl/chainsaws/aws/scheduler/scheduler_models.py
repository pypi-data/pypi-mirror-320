...
class ScheduleExpression(str):
    ...
    def validate(cls, v: str) -> str:
        if not isinstance(v, str):
            raise TypeError("Schedule expression must be a string")
        ...
        if v.startswith("cron(") and v.endswith(")"):
            cron_expr = v[5:-1]  # 'cron(' 와 ')' 제거
            try:
                # croniter를 사용하여 cron 표현식 유효성 검증
                croniter(cron_expr)
                return v
            except ValueError as e:
...
class SchedulerAPIConfig(BaseModel):
    ...
    def to_lambda_config(self) -> Dict[str, Any]:
