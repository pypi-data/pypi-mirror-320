...
class Broker:
    ...
    def buy(self, ticker: str, quantity: int, price: float, date: datetime):
        ...
        cost_with_transaction = total_cost * (1 + self.transaction_cost)
        if self.cash >= cost_with_transaction:
            self.cash -= cost_with_transaction
    ...
    def sell(self, ticker: str, quantity: int, price: float, date: datetime):
        ...
        if ticker in self.positions and self.positions[ticker].quantity >= quantity:
            ...
            proceeds_after_transaction = total_proceeds * (1 - self.transaction_cost)
            self.cash += proceeds_after_transaction
    ...
    def log_transaction(self, date, action, ticker, quantity, price):
        ...
        transaction_cost_value = total_value * self.transaction_cost
        transaction = pd.DataFrame([{
            ...
            'Price': price,
            'Cash': self.cash,
            'Transaction Cost': transaction_cost_value
        }])
