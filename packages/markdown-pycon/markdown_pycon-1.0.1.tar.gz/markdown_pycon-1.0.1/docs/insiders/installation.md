...
*Markdown PyCon Insiders* is a compatible drop-in replacement for *Markdown PyCon*,
...
*Markdown PyCon Insiders* can be installed with `pip` [using SSH][using ssh]:

```bash
pip install git+ssh://git@github.com/pawamoy-insiders/markdown-pycon.git
```
...
```bash
pip install git+https://${GH_TOKEN}@github.com/pawamoy-insiders/markdown-pycon.git
```
...
Of course, you can use *Markdown PyCon Insiders* directly using Git:

```
git clone git@github.com:pawamoy-insiders/markdown-pycon
```

When cloning with Git, the package must be installed:

```
pip install -e markdown-pycon
```

## Upgrading

When upgrading Insiders, you should always check the version of *Markdown PyCon*
