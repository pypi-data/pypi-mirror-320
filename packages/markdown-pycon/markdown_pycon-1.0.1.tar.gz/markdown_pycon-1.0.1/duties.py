...
def check_api(ctx: Context, *cli_args: str) -> None:
    ...
    ctx.run(
        tools.griffe.check("markdown_pycon", search=["src"], color=True).add_args(*cli_args),
...
def docs_deploy(ctx: Context, *, force: bool = False) -> None:
    ...
    with material_insiders() as insiders:
        ...
        if "pawamoy-insiders/markdown-pycon" in origin:
            ctx.run(
                "git remote add upstream git@github.com:pawamoy/markdown-pycon",
                silent=True,
                nofail=True,
