...
class DocumentationFileExplorer:
    ...
    async def search_content(
    ...
    ) -> list[tuple[DocumentKey, Document]]:
        """
        Search through document contents using regex patterns.

        Args:
            query: The search query (supports regex patterns)
            case_sensitive: Whether to perform case-sensitive search
            search_fields: Set of fields to search in
                    ('title', 'content', 'description').
                    If None, searches all fields.

        Returns:
            List of (DocumentKey, Document) tuples matching the search criteria
        """
        ...
        for doc_key, document in self.documents.items():
            matched = False
            if 'title' in search_fields and re.search(
                query,
                document.title,
                flags
            ):
                matched = True
            elif 'content' in search_fields and re.search(
                query,
                document.content,
                flags
            ):
            ...
            if matched:
                matches.append((doc_key, document))
    ...
    def search_by_pattern(self, pattern: str) -> list[
        tuple[DocumentKey, Document]
    ]:
        """
        Search documents using glob patterns.

        Args:
            pattern: Glob pattern to match against document paths
                    (e.g., "**/*.md", "api/**/auth")

        Returns:
            List of (DocumentKey, Document) tuples matching the pattern
        """
        matches = []
        for doc_key, document in self.documents.items():
            if Path(doc_key.path).match(pattern):
                matches.append((doc_key, document))
