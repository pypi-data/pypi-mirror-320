...
import requests
from aceler_inputstream.inputstream import INSERTION_MODE, Inputstream, InputstreamStatus, InputstreamType
...
if __mode != "LOCAL":
    ...
    DATA_URL        = os.environ.get("DATA_URL"         , "https://v2streams.acelerai.io")
    QUERY_MANAGER   = os.environ.get("QUERY_MANAGER"    , "https://v2streams.acelerai.io")  
    INPUTSTREAM_URL = os.environ.get("INPUTSTREAM_URL"  , "https://v2apigateway.acelerai.io")
    verify_https = True
else:
...
class InputstreamClient:
    ...
    async def __allPages(self, ikey, query,delete_id):
        ...
        res = requests.post(f"{QUERY_MANAGER}/QueryManager/ExecutionPlanningFind", 
            data = json.dumps(query, cls=CustomJSONEncoder), 
            headers=headers, 
            verify=verify_https
        )
