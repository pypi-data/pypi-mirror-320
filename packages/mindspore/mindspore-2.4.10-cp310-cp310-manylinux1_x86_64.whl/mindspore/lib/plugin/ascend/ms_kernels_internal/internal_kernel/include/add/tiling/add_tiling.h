 ...
 */
#ifndef MS_KERNELS_INTERNAL_ASCENDC_ADD_TILING_H
#define MS_KERNELS_INTERNAL_ASCENDC_ADD_TILING_H
#include <iostream>
struct AddTilingData {
  uint32_t blockDims{1u};
  uint32_t totalLength{0u};
  uint32_t blockLength{0};
  uint32_t tileLength{0u};
  uint32_t tilingKey{0u};
  uint32_t tileNum{0u};
  uint32_t tailBlockTileNum{0u};
  uint32_t axisDim{0u};
  uint32_t splitNum{1u};
};
