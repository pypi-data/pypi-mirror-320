...
class UnicodeNormalizer(IPreprocessor):
    ...
    def preprocess(self, string_: str) -> str:
        return unicodedata.normalize(self.form, string_)
...
class PreserveGroupAndHighlightedEntity(MatchPostprocessor):
    ...
    def __call__(self, match: Match) -> str:
        parts = []
        parts.append(self.replace_start_with)
        parts.append(match.group(self.group_index))
        leftover_text = match.string[match.regs[0][0]: match.regs[self.group_index][0]]
        leftover_text += match.string[match.regs[self.group_index][1]: match.regs[0][1]]
        if "<b>" in leftover_text or "</b>" in leftover_text:
            elements = []
            s = 0
            element = ""
            for match in BOLD_TAG_PATTERN.finditer(leftover_text):
                if match.group(0) == "<b>":
                    s = match.end()
                    element = "<b>"
                elif match.group(0) == "</b>":
                    element = element + leftover_text[s:match.start()] + "</b>"
                    elements.append(element)
                    s = match.end()
                    element = ""
            if element != "":
        ...
        parts.append(self.replace_end_with)
        return "".join(parts)
...
class URLBodyExtractor(RegexReplacerAdapter):

    def __init__(self):
        regex_query = r"(?:(?:ht|f)tps?:[\\/]+(?:www\d{0,3}[\.…]+)?|www\d{0,3}[\.…]+)((?:[^\s\\/$?#<>]|</?b>)+)(?:/[^\s]+)*"
        super().__init__(regex_query, "\\1", False, False, False)


class URLBodyHighlightPreservingExtractor(RegexReplacerAdapter):
    ...
    def __init__(
        ...
        regex_query = r"(?:(?:ht|f)tps?:[\\/]+(?:www\d{0,3}[\.…]+)?|www\d{0,3}[\.…]+)((?:[^\s\\/$?#<>]|</?b>)+)(?:/[^\s]+)*"
        super().__init__(regex_query, PreserveGroupAndHighlightedEntity(1, start_with, end_with), False, False, False)


class CardNumberReplacer(RegexReplacerAdapter):

    def __init__(self, replace_tag_with=None):
        regex_query = r'(?<!\d)\d{4}[ .-]?\d{4}[ .-]?\d{4}[ .-]?\d{4}(?!\d)'
        super().__init__(regex_query, replace_tag_with, False, False, False)


class IBANNumberReplacer(RegexReplacerAdapter):

    def __init__(self, replace_tag_with=None):
        regex_query = r"(?<![\da-zA-Z])[a-zA-Z]{2}\s?(?:0[2-9]|[1-8][0-9]|9[0-8])\s?[0-9\s]{10,28}\s?[0-9]{2}"
        super().__init__(regex_query, replace_tag_with, False, False, False)
