...
NonNegativeIntOrInf = _Union[NonNegativeInt, Literal["inf"]]
...
def key_to_tuple(cls, member: dict, type_: type) -> dict:
...
class Mode(BaseModel):
    ...
    def __init__(self, *args, **kwargs):
        if len(args) > 0:
            if "name" in kwargs:
                raise NameError("name is already given")
            kwargs["name"] = args[0]
        if len(args) > 1:
            if "duration" in kwargs:
                raise NameError("duration is already given")
            kwargs["duration"] = args[1]
        if len(args) > 2:
    ...
    def _post_init(self):
        if self.name == "":
            ...
            Mode.ID += 1
        if self.name == "dummy":
            print("'dummy' cannot be used as a mode name")
            raise NameError("'dummy' cannot be used as a mode name")
        if not isinstance(self.name, str):
    ...
    def addState(self, state: State, fromValue: int = 0, toValue: int = 0) -> None:
        ...
        if not isinstance(fromValue, int) or not isinstance(toValue, int):
            print(f"time and value of the state {self.name} must be integer")
            raise TypeError(f"time and value of the state {self.name} must be integer")
    ...
    def addResource(
        ...
        if not isinstance(resource.name, str) or not isinstance(requirement, dict):
            ...
            raise TypeError(
                f"type error in adding a resource {resource.name} to activity {self.name}",
            )
        if rtype == "" or rtype == "break" or rtype == "max":
            ...
            print('rtype must be "" or break or max')
            raise NameError('rtype must be "" or break or max')
...
class Activity(BaseModel):
    ...
    # JSONで文字列になった辞書のキーを戻す処理
    @deserialize("execute")
    def deserialize_execute(cls, member: dict) -> dict:
        return key_to_tuple(cls, member, int)

    @model_validator(mode="after")
    def _post_init(self):
        if self.name == "source" or self.name == "sink":
    ...
    def __str__(self) -> str:
        ...
        for m in self.modes:  # multiple modes
            ret.append(f" {m.name} ")  # print mode names
...
class Resource(BaseModel):
    ...
    capacity: PositiveDict | int = Field(default_factory=dict)
    rhs: int = 0
    # TODO 確認 =の代わりに==は？
    ...
    def printConstraint(self) -> str:
        ...
        if self.direction == ">=":
            ...
            for coeff, var, value in self.terms:
                f.append(f"{coeff}({var.name},{value.name}) ")
            f.append(f"<={self.rhs} \n")
            f.append(f"nonrenewable weight {self.weight} ")
            for coeff, var, value in self.terms:
                f.append(f"{-coeff}({var.name},{value.name}) ")
            f.append(f"<={-self.rhs} \n")
        else:
    ...
    def addTerms(
        ...
        if not isinstance(coeffs, list):
            if isinstance(vars, list) or isinstance(values, list):
                raise TypeError("vars must be Activity, values must be Mode")
            self.terms.append((coeffs, vars, values))
        else:
            if not isinstance(vars, list) or not isinstance(values, list):
                raise TypeError("vars, values must be lists")
    ...
    def _post_init(self):
        if self.pred_mode is not None:
            ...
            if self.pred_mode not in self.pred.modes:
                raise ValueError(f"Mode {self.pred_mode.name} is not in activity {self.pred.name}")
        if self.succ_mode is not None:
            assert isinstance(self.succ, Activity)
            if self.succ_mode not in self.succ.modes:
                raise ValueError(f"Mode {self.succ_mode.name} is not in activity {self.succ.name}")
        return self
        ...
        if self.pred_mode is None and self.succ_mode is None:
            ...
            if self.pred != "source" and self.succ != "sink":
                ...
                if self.pred_mode is None and self.succ_mode is not None:
                    raise ValueError(f"The mode of activity {self.pred.name} is not specified!")
                if self.pred_mode is not None and self.succ_mode is None:
                    raise ValueError(f"The mode of activity {self.succ.name} is not specified!")

            pred_mode = self.pred_mode.name if isinstance(self.pred_mode, Activity) else "dummy"
            succ_mode = self.succ_mode.name if isinstance(self.succ_mode, Activity) else "dummy"
...
class Model(BaseModel):
    ...
    def __str__(self):
        ...
        ret.append(f"number of activities= {len(self.act)}")
        ret.append(f"number of resources= {len(self.res)}")
    ...
    def addTemporal(self, pred, succ, tempType="CS", delay=0, pred_mode=None, succ_mode=None):
        ...
        tempo = Temporal(pred=pred, succ=succ, type=tempType, delay=delay, pred_mode=pred_mode, succ_mode=succ_mode)
    ...
    def optimize(
        ...
        if cloud:
            ...
            script = str(pathlib.Path(__file__).parent.resolve() / "optseq")
        ...
        for line in execute:
            ...
            if actname == "source" or actname == "sink":
                pass
            else:
