...
class DemoHandle:
    ...
    @api_telemetry
    def show(self, step: int = 0) -> None:
        if not self._check_setup():
            return
        if not self._check_is_valid_step(step):
            CONSOLE_MANGAER.safe_print(f"[red]Please provide a step in range[/red] [0, {self._num_steps})", color="red")
            return
    ...
    def show_next(self) -> None:
        ...
        if not self._check_is_valid_step(self._current_step):
            ...
            CONSOLE_MANGAER.safe_print(
                f"[red]Total steps available are in range[/red] [0, {self._num_steps})", color="red"
            )
            return
    ...
    def __repr__(self) -> str:
        if not self._check_setup():
            return ""
        status, url = create_notebook_url_from_demo_name(self._name, self._demo_connection, 0)
        if status:
            CONSOLE_MANGAER.safe_print(
                f"Showing step 0.\nPlease click on this link to open the notebook:\n{url}",
                color="cyan",
