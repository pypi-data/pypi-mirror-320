...
class HeartbeatListenerThread(threading.Thread):
    ...
    def run(self):
        ...
        while self._running:
            ...
            logger.debug(f"(%s) (listener)- Data recvd: {data}", format(__name__))
...
class HeartbeatResponderThread(threading.Thread):
    ...
    def run(self):
        while self._running:
            logger.debug(f"(%s) (responder) - Pulling 1 of {self.hb_messages.qsize()} messages from queue.",
                         format(__name__))
            length, self.received_message, address = self.hb_messages.get()
            if self.received_message.preamble == int(0x01):
                    ...
                    if self.received_message.message_type == MessageType.HELLO.value:
                        ...
                        if (self.mother.uuid is None or self.mother.uuid !=
                            ...
                            logger.debug("(%s) (responder) - First pulse received. Initializing Mother.",
                                        format(__name__))
                        self.reply_message.sender_id = self.izzy.uuid.bytes
                        self.reply_message.receiver_id = self.mother.uuid.bytes
                        data = self.izzy.build_base_response()
