...
class HeartbeatResponderThread(threading.Thread):
    ...
    def run(self):
        while self._running:
            length, data, address = self.hb_messages.get()
            logger.info(f"(%s) (responder) - Pulled 1 of {self.hb_messages.qsize()} messages from queue.",
                        format(__name__))
            if self.received_message.preamble == 0x01:
                if list(self.received_message.msg_id) == [0x69, 0x7A, 0x7A, 0x79, 0x6D, 0x65,
                    ...
                    if self.received_message.message_type == MessageType.HELLO.value:
                        logger.info(f"(%s) (responder) - Message is a heartbeat pulse.", format(__name__))
                        if (self.mother.my_id is None or self.mother.my_id !=
                            ...
                            logger.info("(%s) (responder) - First pulse received. Initializing Mother.",
                                        format(__name__))
