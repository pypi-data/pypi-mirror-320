...
def _check_model_is_agent(model_name: str, version: int) -> bool:
    model_info = _get_model_info(model_name, version)
    env_check = (
        os.getenv("_ENABLE_AI_GATEWAY_TABLES_FOR_AGENT_ENDPOINTS", "false").lower()
        == "true"
    )
    task = None
    if model_info and hasattr(model_info, "metadata") and model_info.metadata:
        task = model_info.metadata.get("task")
    regex_check = task is not None and re.match(r"agent/v\d+/chat", task) is not None
    return env_check and regex_check
