...
class Client:
    def __init__(self, host='', apikey='', rpc_url='', private_key='', multi_sig_addr='',
                 conditional_tokens_addr='', multisend_addr=''):
    ...
    def split(self, market_id: int = 1, amount: int = 0):
        topic_detail = self.get_market(market_id)
        # print("topic_detail: {}".format(topic_detail['result']))
    ...
    def merge(self, market_id: int = 1, amount: int = 0):
        topic_detail = self.get_market(market_id)
        # print("topic_detail: {}".format(topic_detail['result']))
        collateral = topic_detail['result']['data']['currencyAddress']
        condition_id = topic_detail['result']['data']['conditionId']
        print('merge: collateral {} condition {}'.format(collateral, condition_id))
        return self.contract_caller.merge(collateral_token=collateral, condition_id=bytes.fromhex(condition_id),
                                          amount=amount)

    def redeem(self, market_id: int = 1):
        topic_detail = self.get_market(market_id)
        # print("topic_detail: {}".format(topic_detail['result']))
        collateral = topic_detail['result']['data']['currencyAddress']
        condition_id = topic_detail['result']['data']['conditionId']
    ...
    def place_order(self, data: OrderDataInput, exchange_addr='', chain_id=8453, currency_addr=''):
        try:
            ...
            if data.orderType == LIMIT_ORDER:
                if data.side == BUY:
                    rawTakerAmount = math.floor(int(data.makerAmount) / float(data.price))
                    adjustedMarkerAmount = math.floor(rawTakerAmount * float(data.price))
                    takerAmount = math.floor(int(adjustedMarkerAmount) / float(data.price))
    ...
    def get_my_orders(self, market_id=0, limit=10):
        try:
