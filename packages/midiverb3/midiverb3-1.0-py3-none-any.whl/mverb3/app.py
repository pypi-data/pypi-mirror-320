...
from PySide6.QtWidgets import QMainWindow, QDialog, QFileDialog, QWidget, QComboBox
...
class Device:
    ...
    PROG_NUM = 100
    REFRESH_RATE_MS = 100
    HELP_URL = "http://localhost:8080/help"
    ...
    def __init__(self, window: _MainWindow):
        ...
        self._queue = Queue()
        self._midi_thread: Thread = Thread(
            target=self._process_message_queue, args=(self._queue,), daemon=True
        )
        self.init()
        ...
        self._ui.actionStoreProgram.triggered.connect(
            self.save_buffer_to_device_program_slot
        )
        self._ui.actionDeviceStoreBank.triggered.connect(
    ...
    def open_in_port(self) -> None:
        ports = self._midi_in.get_ports()
        if self._settings is None:
            return
        with self._midi_lock:
            try:
                port_id = ports.index(self._settings.midi_in_port)
            except ValueError:
                pass
            else:
                self._midi_in.close_port()
                self._midi_in.open_port(port_id)
        self._clear_queue()

    def open_out_port(self) -> None:
        ports = self._midi_out.get_ports()
        if self._settings is None:
            return
        with self._midi_lock:
            try:
                port_id = ports.index(self._settings.midi_out_port)
            except ValueError:
                pass
            else:
                self._midi_out.close_port()
