...
class UIAnimatedElement(UIElement):
    ...
    """
    def __init__(self, x: int, y: int, width: int, height: int, animations: dict):
        super().__init__(x, y, width, height)
        self.animator = Animator(None)
        for name, animation in animations.items():
            self.animator.add_animation(animation, name, transitions={})
        self.animator.play("Idle")  # Default animation
