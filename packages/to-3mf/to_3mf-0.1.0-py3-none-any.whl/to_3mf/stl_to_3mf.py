...
class ModelGroup:
    ...
    def generate_xml(self):
        ...
        # Use a list and sort by id to maintain deterministic order
        colour_groups = sorted(
            {m.colour_group for m in self.models}, 
            key=lambda cg: cg.id
        )
        ...
        return generate_pretty_xml(root, True)
...
def stl_to_model_group(stl_files) -> ModelGroup:
    models = ModelGroup()
    ...
    for i, stl_file in enumerate(stl_files):
        ...
        models.add_new_model_by_tris(
            f'{i}', verticies, trimap, colour_genny.get_colour_group(i))
...
def stl_to_3mf(stl_files, output_stream):
    '''Convert a list of STL files to a 3MF file.
    
    Args:
        stl_files: List of STL file paths or file-like objects
        output_stream: A file-like object to write the 3MF data to
    '''
    ...
    with ZipFile(output_stream, 'w') as zipfile:
...
def stl_to_scad(stl_files, output_stream):
    ...
    Args:
        stl_files: List of STL file paths or file-like objects
        output_stream: A file-like object to write the SCAD data to
    '''
    
    models = stl_to_model_group(stl_files)
    
    for i in range(len(models.models)):
        output_stream.write(f'M{i}();\n')
        
    for i, model in enumerate(models.generate_scad()):
        output_stream.write(f'module M{i}() ' + '{\n')
        model.dump(output_stream)
        output_stream.write('\n}\n')
...
def main():
    ...
    if file_extension == '.3mf':
        with open(output_file, 'wb') as f:
            status = stl_to_3mf(stl_files, f)
    elif file_extension == '.scad':
        with open(output_file, 'w') as f:
            status = stl_to_scad(stl_files, f)
    else:
...
if __name__ == '__main__':
    # sys.argv = [
    #     "to_3mf\\src\\to_3mf\\stl_to_3mf.py", 
    #     "to_3mf\\tests\\test-data\\test_file_01.stl", 
    #     "to_3mf\\tests\\test-data\\test_file_02.stl", 
    #     "to_3mf\\tests\\test-data\\test_result.3mf"
    # ]
    main()
