...
def enthalpy(konz, tempC):
    ...
    if konz < 0.4 - 10 ** (-9):
        print(f"Warning: Concentration x = {konz} is below 40% in enthalpy calculation. Results were linearily extrapolated and are unvalidated.")
...
def hxDiagram(editablePlot=False):
    ...
    for T in T_array:
        ...
        if T%20 == 0:
            ...
            label_posx = x_array[-1] + 0.005
            plt.text(label_posx, h_array[-1], f'{T} °C', fontsize=8, color='black')
        else:
            plt.plot(x_array, h_array, color='black', alpha=0.2, lw=0.2)

    plt.xlabel('Concentration $x=[kg/kg]$')
    ...
    plt.text(0.28, 330, 'Extrapolated', fontsize=8, color='grey')
    # add arrow below text to point to the extrapolated area
