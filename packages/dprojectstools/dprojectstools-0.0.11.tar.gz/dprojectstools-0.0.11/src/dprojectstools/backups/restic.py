...
class Restic:
    ...
    # ctor
    def __init__(self, repository: str, repository_password: str, aws_access_key_id: str, aws_secret_access_key: str, data_path: str, restore_path: str):
    ...
    def backup(self):
        return subprocess.run("restic backup {0} --verbose".format(" ".join(self._data_path)), env = self._getEnv())
    ...
    def snapshots_restore(self, 
        ...
        return subprocess.run("restic restore {0} --target {1} --verbose".format(id, self._restore_path), env = self._getEnv())
