...
class Drone:
    ...
    def __init__(self, flagCheckBackground=True, flagShowErrorMessage=False, flagShowLogMessage=False,
        ...
        if sys.platform != 'emscripten':
            colorama.init()
            print(Fore.GREEN + "Running {0} library version {1}".format(library_name, library.version))
        else:
    ...
    def set_initial_pressure(self):
        if sys.platform != 'emscripten':
            self._set_initial_pressure_desktop()
        else:
            asyncio.create_task(self._set_initial_pressure_emscripten())
    ...
    async def _move_forward_emscripten(self, distance, units="cm", speed=1.0):
        ...
        # read the current position and move relative to
        # position
        data = await self.get_position_data()
        x = data[1]
        y = data[2]
        z = data[3]
        await self.send_absolute_position(positionX=x + distance_meters,
                                    positionY=y,
                                    positionZ=z,
                                    velocity=speed,
                                    heading=0,
        ...
        #print("move forward delay: ", delay)
        await asyncio.sleep(delay)
    ...
    async def _move_left_emscripten(self, distance, units="cm", speed=1.0):
        ...
        y = data[2]
        z = data[3]
        await self.send_absolute_position(positionX=x,
                                    positionY=y + distance_meters,
                                    positionZ=z,
                                    velocity=speed,
                                    heading=0,
    ...
    def _get_colors_desktop(self):
        ...
        try:
            front_color = color_data[9].name
        except AttributeError:
            front_color = 'Unknown'
        try:
            back_color = color_data[10].name
        except AttributeError:
            back_color = 'Unknown'
    ...
    def detect_colors(self, color_data):
        try:
            ...
            prediction = [str(prediction_front[0]), str(prediction_back[0])]
            return prediction
        except:
            if sys.platform != 'emscripten':
    ...
    def _controller_create_canvas_desktop(self, color="white"):
        if color == "white":
            ...
            raise Exception("Error: Invalid color. Color value must be 'white' or 'black'")
        return image
    ...
    async def _controller_draw_string_align_emscripten(self, x_start, x_end, y, string, image, color, alignment):
        ...
        if x_end < x_start:
            raise Exception("Error: x_end must be larger than x_start.")
        ...
        if alignment == "left":
            ...
            raise Exception("Error: Invalid alignment value. Please use 'left', 'center', 'right'.")
