...
def balance_data_with_tsne_auto_clusters(X: pd.DataFrame, y, random_state=None, max_clusters=30):
    ...
    if len(X) > 50000:
        X_sampled = X.sample(n=50000, random_state=random_state)
    else:
        X_sampled = X
    X_embedded = tsne.fit(X_sampled.to_numpy())  # openTSNE's fit method transforms the data
...
class GAForecastRegressor(BaseEstimator):
    def __init__(self, batch_size: int = 128, epochs: int = 10, max_clusters: int = 30, model_path: str | None = None):
    ...
    def save_model(self, filepath: str) -> None:
        """
        Save the trained model and the fitted scaler to a given filepath.

        Args:
            filepath (str): The path (without file extension) where the model
                            and scaler will be saved, e.g., "my_models/model"
        """
        if self.model is None:
            raise ValueError("No model found. Train or load a model before saving.")
        ...
        print(f"Scaler saved to {filepath}_scaler.joblib")

    def load_model(self, filepath: str) -> None:
        """
        Load a trained model and scaler from a given filepath.

        Args:
            filepath (str): The base path (without file extension) from which the
                            model and scaler will be loaded, e.g., "my_models/model"
        """
        model_file = f"{filepath}.h5"
        scaler_file = f"{filepath}_scaler.joblib"

        # Check if files exist
        if not os.path.exists(model_file):
            raise FileNotFoundError(f"No model file found at {model_file}")
        if not os.path.exists(scaler_file):
            raise FileNotFoundError(f"No scaler file found at {scaler_file}")
