...
class Toggle_S1:
    def __init__(self, btnName, canvas, btn_shape_points, press_drag_color = (255, 255, 255),
                 default_state = False, command=None):
        # self.root = root
        ...
        self.__selected_Btn = btnName
        self.__buttonpresseffect(self.__selected_Btn, self.__current_state)
        self.__selected_Btn = None
    ...
    def __buttonpresseffect(self, btn_name, enable_effect):
        ...
        if enable_effect:
            self.__canvas.itemconfig(btn_name, fill=self.__rgb_to_hex(self.__press_drag_color), stipple='gray25',
                                     outline=self.__rgb_to_hex(self.__press_drag_color), width=2)
            # self.canvas.itemconfig(btn_name, fill=self.rgb_to_hex((230, 190, 130)), stipple=stipple_file)
        else:
            self.__canvas.itemconfig(btn_name, fill='', outline='', width=0)
    ...
    def __top_layer_mouse_dn(self, event):
        ...
        if self.__is_point_in_polygon(x, y, self.__btn_shape_points):
            ...
            self.__buttonpresseffect(self.__selected_Btn, False if self.__current_state is True else True)
    ...
    def __top_layer_mouse_drag(self, event):
        ...
        if not len(tag_name):
            ...
            if self.__selected_Btn == tag_name[0]:
                ...
                self.__buttonpresseffect(self.__selected_Btn, True if self.__current_state is True else False)
    ...
    def __top_layer_mouse_up(self, event):
        ...
        if len(tag_name) and self.__selected_Btn == tag_name[0]:
            self.__current_state = not self.__current_state
            self.__buttonpresseffect(self.__selected_Btn, self.__current_state)

            if self.__command is not None:
                self.__command(f'{self.__selected_Btn}:{self.__current_state}')
    ...
    def set(self, state: bool):
        if type(state) is bool:
            ...
            self.__buttonpresseffect(self.__btnName,self.__current_state)


    @property
    def Value(self):
        return True if self.__current_state else False
