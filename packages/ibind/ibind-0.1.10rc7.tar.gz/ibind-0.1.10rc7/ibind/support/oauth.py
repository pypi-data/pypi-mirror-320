...
@dataclass
class OAuthConfig():
    oauth_rest_url = var.IBIND_OAUTH_REST_URL
    live_session_token_endpoint = var.IBIND_LIVE_SESSION_TOKEN_ENDPOINT
    access_token = var.IBIND_ACCESS_TOKEN
    access_token_secret = var.IBIND_ACCESS_TOKEN_SECRET
    consumer_key = var.IBIND_CONSUMER_KEY
    dh_prime = var.IBIND_DH_PRIME
    encryption_key_fp = var.IBIND_ENCRYPTION_KEY_FP
    signature_key_fp = var.IBIND_SIGNATURE_KEY_FP
    dh_generator = var.IBIND_DH_GENERATOR
    realm = var.IBIND_REALM
    init_oauth = var.IBIND_INIT_OAUTH
    maintain_oauth = var.IBIND_MAINTAIN_OAUTH
    shutdown_oauth = var.IBIND_SHUTDOWN_OAUTH
