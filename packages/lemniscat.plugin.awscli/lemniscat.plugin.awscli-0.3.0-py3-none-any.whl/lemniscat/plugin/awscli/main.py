...
from lemniscat.plugin.awscli.filestore import FileStore
from lemniscat.plugin.awscli.awscli import AWSCli
...
class Action(PluginCore):
    ...
    def __run_awscli(self) -> TaskResult:
        # override environment configuration with aws configuration
        if(self.variables.keys().__contains__('aws_access_key')):
            os.environ["AWS_ACCESS_KEY_ID"] = self.variables["aws_access_key"].value
        if(self.variables.keys().__contains__('aws_secret_key')):
            os.environ["AWS_SECRET_ACCESS_KEY"] = self.variables["aws_secret_key"].value
        cli = AWSCli()
    ...
    def invoke(self, parameters: dict = {}, variables: dict = {}) -> TaskResult:
        ...
        self._logger.debug(f'Run AWSCli with {self.parameters["scripttype"]} -> {self.meta}')
        task = self.__run_awscli()
        return task
