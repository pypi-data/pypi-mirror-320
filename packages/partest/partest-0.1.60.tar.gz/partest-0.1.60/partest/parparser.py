    ...
    def collect_paths_info(self):
        ...
        for item in extracted_data:
            if not item.deprecated:
                swagger_title = next((title for title in self.swagger_titles.keys() if self.swagger_titles[title] == (item.source_type, item.path)), 'Unknown API')
                self.paths_info.append({
                    'description': item.description,
                    'path': item.path,
                    'method': item.method,
                    'parameters': item.parameters,
                    'swagger_title': swagger_title  # Store the title here
                })
