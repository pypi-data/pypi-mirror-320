...
def zorro():
    ...
    path_to_name = {path: name for name, (_, path) in swagger_files.items()}
    ...
    for (method, endpoint, description), count in call_count.items():
        ...
        # Получаем swagger_title из path_to_name
        swagger_title = next((name for path, name in path_to_name.items() if path in [p.path for p in paths_info]), "Unknown API")
