...
def zorro():
    ...
    for (method, endpoint, description), count in call_count.items():
        is_deprecated = any(
            path.path == endpoint and path.method == method and path.description == description and path.deprecated for
            path in paths_info)
    ...
    allure.attach("\n".join(report_lines), name='Общий процент покрытия', attachment_type=allure.attachment_type.TEXT)
