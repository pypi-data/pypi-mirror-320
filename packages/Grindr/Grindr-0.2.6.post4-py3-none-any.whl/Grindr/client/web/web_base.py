...
class ClientRoute[Method: Literal["GET", "POST", "PUT", "DELETE"], Url: URLTemplate, Params: Any, Body: Any, Response: Any]:
    ...
    async def __call__(
        ...
        if response.status_code != 200:
            ...
            raise GrindrRequestError(response, f"A request to Grindr at {response.url.split("?")[0]} failed with status code {response.status_code}!")
        ...
        try:
            ...
            content_excerpt = content_preview[:100]  # Optional: truncate for readability
            raise JSONDecodeError(
                f"Failed to decode JSON response for a successful request (200): {content_excerpt}",
                content_preview,
                ex.pos,
            ) from ex
