...
from Grindr.client.web.web_settings import DEFAULT_REQUEST_PARAMS, DEFAULT_REQUEST_HEADERS, get_mobile_user_agent, get_desktop_user_agent, ANDROID_VERSION_API_LEVEL_MAP, DEVICE_SCREEN_DIMENSIONS
...
class GrindrHTTPClient:
    ...
    def generate_device_info_android(cls):
        android_device_id = f"{random.getrandbits(64):016x}"
        android_advertising_id = str(uuid.uuid4())
        random_integer = random.randint(1000000000, 9999999999)
        return f"{android_device_id};GLOBAL;2;{random_integer};{random.choice(DEVICE_SCREEN_DIMENSIONS)};{android_advertising_id}"

    def refresh_device_info(
            self,
            refresh_user_agent: bool,
            user_agent_type: Literal["web", "mobile"] = "mobile"
    ):
        ...
        if refresh_user_agent:
            self._mobile_user_agent = get_mobile_user_agent()
            self._web_user_agent = get_desktop_user_agent()
            self.headers['User-Agent'] = self._mobile_user_agent if user_agent_type == "mobile" else self._web_user_agent
