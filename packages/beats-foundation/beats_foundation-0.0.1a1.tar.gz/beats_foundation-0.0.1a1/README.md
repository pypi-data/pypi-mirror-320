...
async def main() -> None:
    song = await client.songs.create(
        prompt="REPLACE_ME",
    )
...
client.with_options(max_retries=5).songs.create(
    prompt="REPLACE_ME",
)
```
...
client.with_options(timeout=5.0).songs.create(
    prompt="REPLACE_ME",
)
```
...
song = response.parse()  # get the object that `songs.create()` would have returned
print(song)
```

These methods return an [`APIResponse`](https://github.com/ryohno/beatssdk/tree/main/src/beats_foundation/_response.py) object.

The async client returns an [`AsyncAPIResponse`](https://github.com/ryohno/beatssdk/tree/main/src/beats_foundation/_response.py) with the same structure, the only difference being `await`able methods for reading the response content.
...
We are keen for your feedback; please open an [issue](https://www.github.com/ryohno/beatssdk/issues) with questions, bugs, or suggestions.
