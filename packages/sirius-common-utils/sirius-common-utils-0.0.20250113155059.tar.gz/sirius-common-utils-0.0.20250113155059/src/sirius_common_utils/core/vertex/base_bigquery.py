...
def init():
    ...
    if g_client is None:
        g_client = bigquery.Client(project=constant.GOOGLE_PROJECT)

    if g_dataset is None:
        g_dataset = bigquery.Dataset(constant.GOOGLE_DATASET)
        create_dataset()
...
def create_dataset():
    ...
    g_dataset.location = constant.GOOGLE_REGION
    g_dataset.description = "sirius"
    g_dataset.labels = {"environment": "production", "type": "timeseries"}

    g_dataset = g_client.create_dataset(bigquery.Dataset(constant.GOOGLE_DATASET), exists_ok=True)
