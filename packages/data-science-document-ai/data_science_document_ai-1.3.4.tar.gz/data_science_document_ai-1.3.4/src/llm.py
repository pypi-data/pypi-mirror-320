...
class LlmClient:
    ...
    def get_unified_json_genai(self, prompt, document=None, response_schema=None, model="gemini"):
        ...
        if model.lower() in {"chatgpt", "openai", "gpt"}:
            response = self.ask_chatgpt(prompt, document, response_schema)
        else:
            # Default to Gemini
    ...
    def ask_chatgpt(self, prompt: str, document=None, response_schema=None):
