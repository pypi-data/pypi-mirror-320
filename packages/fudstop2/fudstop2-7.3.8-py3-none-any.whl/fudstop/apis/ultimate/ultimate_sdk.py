...
class UltimateSDK:
    ...
    async def check_macd_sentiment(hist: List[float]) -> Optional[str]:
        if len(hist) < 3:
            return None

        # 0 = most recent, 1 = one bar ago, 2 = two bars ago
        h0, h1, h2 = hist[0], hist[1], hist[2]

        # --- BULLISH ---
        # near -0.02 (±0.04), AND strictly negative (h0 < 0),
        # AND the sequence is “curling upward”:
        #   meaning it was more negative, but now it’s less negative
        #   so h0 > h1 > h2 (that’s your old check) AND maybe also check
        #   the difference (h1 - h0) is not too big.
        if (
            -0.06 <= h0 <= 0.02          # near -0.02
            and h0 < 0                  # truly negative
            and h0 > h1 and h1 > h2     # strictly descending going backwards
            # optional extra condition to enforce 'curling in':
            # e.g. h0 is not just "randomly" bigger than h1, but also 
            # the slope from h1->h0 is greater (less negative) than from h2->h1
            # or h0 - h1 > h1 - h2, etc.
        ):
            return 'bullish'
        ...
        # --- BEARISH ---
        if (
            -0.02 <= h0 <= 0.06         # near +0.02
            and h0 > 0                  # truly positive
            and h0 < h1 and h1 < h2     # strictly ascending going backwards
            # likewise, consider an extra slope condition
        ):
            return 'bearish'
