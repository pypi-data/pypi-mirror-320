...
# Example usage
    ...
    async def working_universal(self, symbols_list):
        """
        Process symbols in chunks if the total exceeds the API limit.
        """
        ...
        async def fetch_chunk(chunk):
            ...
            """
            symbols_string = ",".join(chunk)  # Convert the chunk into a comma-separated string
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    f"https://api.polygon.io/v3/snapshot?ticker.any_of={symbols_string}&limit=250&apiKey={self.api_key}"
                )
                response.raise_for_status()  # Raise an error for bad HTTP responses
                return response.json()

        # Split the symbols list into chunks of 249 symbols
        chunks = list(chunked(symbols_list, 249))

        results = []
        for chunk in chunks:
            data = await fetch_chunk(chunk)
            if "results" in data:
                results.extend(data["results"])  # Append results from each chunk
