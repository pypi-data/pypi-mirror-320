...
class UltimateSDK:
    ...
    async def check_macd_sentiment(hist: List[float]) -> Optional[str]:
        ...
        Conditions:
        • Bullish:
            1) The most recent histogram is within ±0.04 of -0.02
            (i.e. between -0.06 and +0.02),
            2) The most recent histogram is negative (< 0),
            3) The last three histogram values strictly descend when
            moving backward in time (h0 > h1 > h2).
            
        • Bearish:
            1) The most recent histogram is within ±0.04 of +0.02
            (i.e. between -0.02 and +0.06),
            2) The most recent histogram is positive (> 0),
            3) The last three histogram values strictly ascend when
            moving backward in time (h0 < h1 < h2).
        """
        if len(hist) < 3:
            return None

        h0, h1, h2 = hist[0], hist[1], hist[2]  # h0 is most recent

        # --- BULLISH check ---
        # near -0.02 (±0.04) ⇒ -0.06 <= h0 <= +0.02
        # strictly negative ⇒ h0 < 0
        # strictly descending going backwards ⇒ h0 > h1 > h2
        if -0.06 <= h0 <= 0.02 and h0 < 0 and (h0 > h1 > h2):
            return 'bullish'

        # --- BEARISH check ---
        # near +0.02 (±0.04) ⇒ -0.02 <= h0 <= +0.06
        # strictly positive ⇒ h0 > 0
        # strictly ascending going backwards ⇒ h0 < h1 < h2
        if -0.02 <= h0 <= 0.06 and h0 > 0 and (h0 < h1 < h2):
            return 'bearish'
