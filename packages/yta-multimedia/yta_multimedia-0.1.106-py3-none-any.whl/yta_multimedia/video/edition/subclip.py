...
class SubClip:
    ...
    def subclip(self, start: float, end: float):
        ...
        part can be None.
        """
        left = deepcopy(self) if start not in (None, 0) else None
        if left:
            left.video = left.video.subclip(start)

        right = deepcopy(self) if end is not None and end >= self.duration else None
        if right:
            right.video = right.video.subclip(start_time = end)

        center = deepcopy(self)
        center.video = center.video.subclip(start, end)
