...
from yta_multimedia.video.edition.settings import VOLUME_LIMIT, ZOOM_LIMIT, LAYERS_INDEXES_LIMIT, COLOR_TEMPERATURE_LIMIT, MAX_TIMELINE_LAYER_DURATION, BRIGHTNESS_LIMIT, CONTRAST_LIMIT, WHITE_BALANCE_LIMIT
from yta_multimedia.video.edition.effect.m_effect import MEffect
...
class VideoSetting:
    ...
    def get_values(self, steps: int):
        ...
        if self._values is None:
            ...
            # TODO: If we make any method to update values we need
            # to handle a flag if these '_values' have to be
            # recalculated
...
class SubClip:
    ...
    def _process(self):
        ...
        def modify_video_frame_by_frame(get_frame, t):
            ...
            frame = ImageEditor.modify_color_temperature(frame, self.color_temperature.values[frame_index]) if self.color_temperature is not None else frame
            frame = ImageEditor.modify_brightness(frame, self.brightness) if self.brightness is not None else frame
            frame = ImageEditor.modify_contrast(frame, self.contrast) if self.contrast is not None else frame
            frame = ImageEditor.modify_sharpness(frame, self.sharpness) if self.sharpness is not None else frame
            frame = ImageEditor.modify_white_balance(frame, self.white_balance) if self.white_balance is not None else frame
...
def _validate_color_temperature(color_temperature: VideoSetting):
    if not PythonValidator.is_instance(color_temperature, VideoSetting):
        raise Exception('The provided "color_temperature" is not a VideoSetting instance.')
    
    if not NumberValidator.is_number_between(color_temperature.initial_value, COLOR_TEMPERATURE_LIMIT[0], COLOR_TEMPERATURE_LIMIT[1]):
        raise Exception(f'The "color_temperature" parameter provided "initial_value" is not a number between [{COLOR_TEMPERATURE_LIMIT[0]}, {COLOR_TEMPERATURE_LIMIT[1]}].')

    if not NumberValidator.is_number_between(color_temperature.final_value, COLOR_TEMPERATURE_LIMIT[0], COLOR_TEMPERATURE_LIMIT[1]):
        raise Exception(f'The "color_temperature" parameter provided "final_value" is not a number between [{COLOR_TEMPERATURE_LIMIT[0]}, {COLOR_TEMPERATURE_LIMIT[1]}].')
    
def _validate_brightness(brightness: int):
    if not NumberValidator.is_number_between(brightness, BRIGHTNESS_LIMIT[0], BRIGHTNESS_LIMIT[1]):
        raise Exception(f'The "brightness" parameter provided is not a number between [{BRIGHTNESS_LIMIT[0]}, {BRIGHTNESS_LIMIT[1]}].')

def _validate_contrast(contrast: int):
    if not NumberValidator.is_number_between(contrast, CONTRAST_LIMIT[0], CONTRAST_LIMIT[1]):
        raise Exception(f'The "contrast" parameter provided is not a number between [{CONTRAST_LIMIT[0]}, {CONTRAST_LIMIT[1]}].')
    
def _validate_white_balance(white_balance: int):
    if not NumberValidator.is_number_between(white_balance, WHITE_BALANCE_LIMIT[0], WHITE_BALANCE_LIMIT[1]):
        raise Exception(f'The "white_balance" parameter provided is not a number between [{WHITE_BALANCE_LIMIT[0]}, {WHITE_BALANCE_LIMIT[1]}].')
