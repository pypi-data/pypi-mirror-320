...
def consolidate_linker(
    ...
    for model_r in ann_groups[""].ranges():
        ...
        sub_geo = 0
        for perfect, wiki_match in all_matches(model_ann, wiki_r):
            if perfect or params.wikidata_partial:
                if labels_are_compatible(model_ann.labelName, wiki_match.labelName, params):
                    ...
                    if is_geo_label(
                            wiki_match.labelName) and wiki_match.start >= model_ann.start and wiki_match.end <= model_ann.end:
                        if kill_names:
...
def labels_are_compatible(a: str, b: str, params: XCagoReconciliationParameters):
    return a == b or (is_geo_label(a) and is_geo_label(b)) or is_kill_or_white_label(a, params) or is_kill_or_white_label(b, params)
