...
class Agent:
        ...
        if tx_receipt and tx_receipt['logs']:
            for log in tx_receipt['logs']:
                if self.agent_manger.address.lower() == log['address'].lower():
                    ...
                    tx_agent = event_data['args']['ai_agent']
    ...
    def __validate_agent_settings(self, settings: AgentSettings) -> AgentSettings:
        ...
        if settings.agent_header.version != self.agent_manger_version:
            raise ValueError("Invalid ai_agent manger version:", settings.agent_header.version, "should ", self.agent_manger_version)

        if not settings.agent_header.message_type.name in MessageType.__members__:
            raise ValueError("Invalid ai_agent header message type:", settings.agent_header.message_type.name)

        if not settings.agent_header.priority.name in Priority.__members__:
            raise ValueError("Invalid ai_agent header priority type:", settings.agent_header.priority.name)

        if not is_valid_uuid_v4(settings.agent_header.message_id):
            raise ValueError("Invalid ai_agent header message id:", settings.agent_header.message_id, "must a uuid v4")

        if not is_valid_uuid_v4(settings.agent_header.target_agent_id):
            raise ValueError("Invalid ai_agent header target ai_agent id:", settings.agent_header.target_agent_id, "must a uuid v4")

        if not is_valid_uuid_v4(settings.agent_header.source_agent_id):
            raise ValueError("Invalid ai_agent header source ai_agent id:", settings.agent_header.source_agent_id, "must a uuid v4")

        if not self.agent_manger.functions.isValidSourceAgentId(settings.agent_header.source_agent_id).call():
            raise ValueError("Duplicate ai_agent header source ai_agent id")
    ...
    def __validate_agent_payload(payload: AgentMessagePayload) -> AgentMessagePayload :
        ...
        if not is_valid_hex_string(payload.data):
            raise ValueError("Invalid ai_agent payload data:", payload.data)
        ...
        if not is_valid_hex_string(payload.data_hash) and len(payload.data_hash) != 66:
            raise ValueError("Invalid ai_agent payload hash:", payload.data_hash)
        ...
        if not is_valid_hex_string(payload.proofs.signature_proof) and (len(payload.proofs.signature_proof)-2) % 64 != 0:
            raise ValueError("Invalid ai_agent payload signature proof:", payload.proofs.signature_proof)
