...
class PvTextInput(Widget):
    def __init__(self, container, x=50, y=50, width=200, height=40, background_color=(1, 1, 1, 1), input_type="text", visibility=True,
    ...
    def update_graphics(self, *args):
        """Update the canvas rectangle, borders, and TextInput position when the widget size or position changes."""
        # Update background rectangle
        self.rect.pos = self.pos
        self.rect.size = self.size

        # Update internal TextInput position
        self.text_input.pos = (
    ...
    def get_text_dimensions(self):
        """Get the width and height of the text currently inside the TextInput."""
        # Use the existing text, font size, and font name
        text = self.text_input.text or " "  # Ensure at least one space for calculation
        font_size = self.text_input.font_size
        font_name = self.text_input.font_name

        # Create a CoreLabel instance to measure the text
        label = CoreLabel(
            text=text,
            font_size=font_size,
            font_name=font_name
        )
        label.refresh()  # Calculate the size
        return label.texture.size  # Return the (width, height) of the text texture
