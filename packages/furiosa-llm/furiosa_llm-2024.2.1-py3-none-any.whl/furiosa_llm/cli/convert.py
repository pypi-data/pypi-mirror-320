...
def add_convert_args(convert_parser):
    ...
    convert_parser.add_argument(
        ...
        type=str,
        required=True,
        help='The Hugging Face pretrained id (e.g., "meta-llama/Meta-Llama-3.1-8B-Instruct").',
    )
    ...
    convert_parser.add_argument(
        ...
        type=str,
        default="npu:0:*",
        help='Devices to use (e.g., "npu:0:*,npu:1:*"). If not specified, the artifact will be built using only one device.',
    )
    ...
    convert_parser.add_argument(
        ...
        "--data-parallel-size",
        type=int,
        help='The size of the data parallelism group. If not specified, it will be inferred based on the total available PEs and other parallelism configurations.',
    )
    ...
    convert_parser.add_argument(
        ...
        type=int,
        help='The maximum number of blocks each k/v storage layer can store. Required if the model uses paged attention.',
    )
...
def convert(args):
    ...
    add_prefill_last_block_slice = False
    ...
    if args.additional_model_config:
        for item in args.additional_model_config:
            ...
            elif key == "add_prefill_last_block_slice":
                add_prefill_last_block_slice = value.lower() == "true"
            else:
