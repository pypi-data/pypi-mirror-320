...
def sock_read(sock):
    global pk_loss
    recv = bytearray(units.count("128 KB"))
    if sock:
        try: size = sock.recv_into(recv)
        except: size = 0
    if size == 0: return None
    else: return recv[:size]
...
def read_package(sock, buffer=units.count("128 KB")):
    global pk_loss
    global tunnel_traffic
    global bandwidth
    if not sock:
...
class SockIO(object):
    ...
    def recv(self, buffer):
        r = bytearray(buffer + 32)
        with self.recv_lock:
            while True:
                data = self.pi.recv()
                if data is None:
                    try:
                        s = self.sock.recv_into(r)
                        if s == 0:
                            return None
                        self.pi.feed(r[:s])
...
class Client(SockRunable):
    ...
    def _add_con(self, ix):
        try:
            ...
            sock.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, units.count("128 KB")+32)
            sock.setsockopt(socket.SOL_SOCKET, socket.SO_RCVBUF, units.count("128 KB")+32)
