...
def set_net(buffer): # set buffer size to 512 kbytes
    if platform.system().lower() == "linux":
        os.system("sudo sysctl -w net.core.rmem_default={str(buffer // 2)}")
        os.system("sudo sysctl -w net.core.wmem_default={str(buffer // 2)}")
        os.system("sudo sysctl -w net.core.rmem_max={str(buffer)}")
        os.system("sudo sysctl -w net.core.wmem_max={str(buffer)}")
        return True
    elif platform.system().lower() in ("freebsd", "netbsd", "openbsd"):
        os.system("sudo sysctl -w net.inet.tcp.recvspace={str(buffer)}")
        os.system("sudo sysctl -w net.inet.tcp.sendspace={str(buffer)}")
        return True
    else:
        return False

def get_net(): # get buffer size
    try:
        if platform.system().lower() == "linux":
            result = sp.run(["sysctl", "-a"], capture_output=True, text=True)
            if result.returncode == 0:
                for line in result.stdout.splitlines():
                    if "net.core.rmem_max" in line or "net.core.wmem_max" in line:
                        return int(line.split("=")[-1].strip())
        elif platform.system().lower() in ("freebsd", "netbsd", "openbsd"):
            result = sp.run(["sysctl", "-a"], capture_output=True, text=True)
            if result.returncode == 0:
                for line in result.stdout.splitlines():
                    if "net.inet.tcp.recvspace" in line or "net.inet.tcp.sendspace" in line:
                        return int(line.split("=")[-1].strip())
        return 0
    except Exception as e:
        return 0
        ...
        try:
            ...
            if status == "online":
                if stunnel_traffic > 1024: # KBytes
                    if stunnel_traffic > 524288: # MBytes
            ...
            if max_network == "-":
                ...
                if used_network >= max_network-524288:
                    st_t = 1
                    show_message = False
...
class Client(SockRunable):
    ...
    def _add_con(self, ix):
        try:
            ...
            sock.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, self.buffer)
    ...
    def _run(self):
        ...
        try:
            ...
            sd = {
                ...
                'tunnel': {'proto': base64.b64encode(encrypt_message(self.proto)).decode(), 'domain': base64.b64encode(encrypt_message(self.domain)).decode(), 'compression': self.compress},
...
def main():
    ...
    if args.version:
        ...
        is_android: bool = hasattr(sys, 'getandroidapilevel')
        if is_android == False:
            if is_root() == False and platform.system().lower() == "windows":
                print(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;31mERROR\033[0m] superuser rights are required")
                sys.exit(0)
        support_ipv4 = False
        support_ipv6 = False
        ...
        is_android: bool = hasattr(sys, 'getandroidapilevel')
        if is_android == False:
            if platform.system().lower() == "windows" and is_root == False:
        ...
        target_port = None
        tunnel_port = None
        if platform.system().lower() in ("linux", "freebsd", "netbsd", "openbsd"):
            mem = get_net()
            if mem != 0 and mem <= 131072:
                print_s(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;33mWARN\033[0m ] your system has too small a buffer, which may reduce the tunnel speed")
        ...
        try:
            ...
            if buffer < 131072:
                ...
                raise SystemExit
            if buffer > 524288:
                print_s(f"\033[01;36m"+str(time.strftime("%H:%M:%S"))+f"\033[0m [\033[01;31mERROR\033[0m] too big buffer, maximum 512 KB")
                raise SystemExit
        except (NameError, ValueError, KeyError):
