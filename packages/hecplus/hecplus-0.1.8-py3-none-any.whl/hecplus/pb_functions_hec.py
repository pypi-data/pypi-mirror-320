...
#%%
def hdf_update_df(file_hdf: str, pathname: str, df: pd.DataFrame = None, v: np.ndarray = None) -> None:
...
#%%
def hdf_generate_info_value(df: pd.DataFrame) -> dict:
    ...
    (df
        .groupby('index_info')
        .agg(n=('index_info', 'size'))
        .reset_index()
        .pipe(ds.pd_set_colnames, [0, 1])
    )
    df_value = df.drop(columns='index_info')
    return dict(df_info = df_info, df_value = df_value)

#%%
def hdf_generate_info_parts_value(df: pd.DataFrame) -> dict:
