...
#%%
def hdf_read_df(file_hdf: str, pathname: str, decode_binary_str = True, col_names: list|np.ndarray|pd.Series = None) -> pd.DataFrame:
...
#%%
def hdf_update_df(file_hdf: str, pathname: str, df: pd.DataFrame):
    '''Update HDF Dataset with DataFrame.

    Args:
        file_hdf (str): Filename of HEC hdf file to update.
        pathname (str): Pathname of dataset within hdf file to update.
        df (pd.DataFrame): DataFrame to update HDF dataset with.
    '''
