...
logger = Logger("serial_com", logging.DEBUG)
...
class WorkerProperty:
    ...
    def __init__(self, port_type: str = SERIAL_PORT|NETWORK_PORT, name: str = "", listener: str = "", baudrate: int = 19200): #TODO: stopbits etc.
...
def ser_request(request: bytearray|bytes, attempt: int, port: Port, length=None, read_until: bytes|None=None):
    ...
    if attempt <= max_retries:
        try:
            ...
            ser_request(request, attempt + 1, port, length, read_until)
    else:
