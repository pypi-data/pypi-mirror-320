...
def add_signed_cookies(request, response, assets_domain, domain=settings.CLOUDFRONT_SIGNED_COOKIES_DOMAIN):
    ...
    try:
        if getattr(settings, 'CLOUDFRONT_SIGNED_COOKIES', False):
            ...
            # Redirect to the same URL after setting new cookies
            redirect_url = request.build_absolute_uri()
            logger.info(f"Redirecting to {redirect_url} after setting new cookies.")
    ...
    def __call__(self, request):
        ...
        if getattr(request, 'user', None) and request.user.is_authenticated:
            ...
            if 'HTTP_HOST' in request.META:
                ...
                add_signed_cookies(request, response, 'assets-v2.hub3d.pvh.com')
