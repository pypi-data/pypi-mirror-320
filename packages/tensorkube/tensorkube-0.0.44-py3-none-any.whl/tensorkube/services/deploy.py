...
def deploy_knative_service(env: str, gpus: int, gpu_type: str, cpu: float, memory: float, min_scale: int, max_scale: int,
                           pod_status: str, cwd: str, sanitised_project_name: str, image_url: str, secrets: list, domain: Optional[str],
                           enable_efs: bool = False, github_actions: bool = DEFAULT_GITHUB_ACTIONS):
    env_namespace = DEFAULT_NAMESPACE if not env else env
