...
if INSIDE_INSTANCE:
    ...
    def status(wait):
        def get_table(instances, show_timestamp=False, changed=False):
            ...
            instances_table.add_column("GPU Type", justify="center")
...
def create(cores, template, gpu):
    with DelayedProgress(
        ...
        token = get_token()
        success, error, instance_id = utils.create_instance(token, cores, template, gpu)
