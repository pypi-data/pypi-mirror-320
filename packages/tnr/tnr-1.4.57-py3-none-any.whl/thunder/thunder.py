...
def setup(global_setup):
    setup_cmd.setup(global_setup)
    # def basic_checks(progress):
    #     try:
    #         task = progress.add_task("Checking system requirements...", total=1)
    #         time.sleep(1)  # Simulate work
    #         progress.update(task, advance=1, description="✅ System requirements checked")
    #     except Exception as e:
    #         progress.update(task, description="❌ System requirements check failed")
    ...
    #         progress.update(task, advance=1, description="✅ System requirements met")
    #     except Exception as e:
    #         progress.update(task, description="❌ System requirements not met")
    #         raise click.ClickException(f"Setup failed: {str(e)}")
    
    # def install_globally(progress):    
    #     task = progress.add_task("Installing globally...", total=4)
    ...
    #     progress.update(task, advance=1, description="✅ Running as root")
        
    #     # Create /etc/thunder directory
    #     os.makedirs("/etc/thunder", exist_ok=True)
    #     progress.update(task, advance=1, description="✅ Created /etc/thunder")
    ...
    #     # If there is an existing user config, copy it to the global config
    #     if os.path.exists(user_config):
    #         with open(user_config, 'r') as src, open(config_path, 'w') as dst:
    #             dst.write(src.read())
                
    #     # Create symlink from user config to global config
    #     thunder_dir = os.path.join(user_home, ".thunder")
    #     os.makedirs(thunder_dir, exist_ok=True)
    #     if os.path.exists(user_config):
    #         os.remove(user_config)
    #     os.symlink(config_path, user_config)
    #     progress.update(task, advance=1, description="✅ Created global config files")
        
    #     # Update ld.so.preload
    #     preload_path = "/etc/ld.so.preload"
    #     preload_line = get_latest("client", "/etc/thunder/libthunder.so")
    #     print(preload_line)
    #     if os.path.exists(preload_path):
    #         with open(preload_path, "r") as f:
    #             lines = f.readlines()
    #         if preload_line not in lines:
    ...
    # # setup_tasks = [basic_checks, task2, task3]
