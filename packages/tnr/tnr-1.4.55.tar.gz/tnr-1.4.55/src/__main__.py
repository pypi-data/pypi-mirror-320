...
def GetRepositoriesImports(module_space, import_all):
  ...
  return [os.path.join(module_space, 'thunder')]
...
""".format(
    target='//src/sidekicks/log_collector:logs_collector',
    ret_code=ret_code,
...
def Main():
  ...
  rel_path = 'thunder/src/sidekicks/log_collector/logs_collector.py'
  if IsWindows():
  ...
  try:
    ...
    if IsRunningFromZip():
      ...
      if os.environ.get('RUN_UNDER_RUNFILES') == '1':
        os.chdir(os.path.join(module_space, 'thunder'))
