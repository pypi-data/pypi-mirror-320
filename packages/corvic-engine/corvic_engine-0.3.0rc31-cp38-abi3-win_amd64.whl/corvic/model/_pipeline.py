...
class Pipeline(WrappedProto[PipelineID, models_pb2.Pipeline]):
    ...
    def list(
        ...
        match cls.list_as_proto(
            ...
            room_id,
            created_before,
            additional_query_transform=lambda query: query.options(
                sa_orm.subqueryload(orm.Pipeline.inputs)
                .subqueryload(orm.PipelineInput.resource)
                .subqueryload(orm.Resource.source_associations),
                sa_orm.subqueryload(orm.Pipeline.outputs)
                .subqueryload(orm.PipelineOutput.source)
                .subqueryload(orm.Source.resource_associations),
            ),
        ):
