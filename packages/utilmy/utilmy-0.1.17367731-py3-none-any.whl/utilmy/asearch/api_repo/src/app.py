...
def api_imgsync(
    ...
    Comments : 
         + Processing time: client cancel request... ,  assume procesing below < 1sec : guarantee.
         
         API_thenatciate --> at API gateway
               we dont need again api_authenticate.
               Depends on Endpoint requireemnts security : JWT token

    Params valid : per endpoint specific code ?
       In validation --> throws an error :  throw 400 HTTP  image is invalid

       If something happens in process ---> Throws 500.

    How do you manage parameters in global config file ?
         Unit testing, I dont global variable in function.

    Load config, anywhere --> Config is immutable.
    when you wrtie function, it depends on some XXX global. extra dependencies defied Outside of function.

       Middleware Implicit --> can create edge cases easily.
       Pros/Cons

      Re-usable template code for endpoint.
        I can copy/paste  and 99%  only 2% of parts.
   ...
   In endpoint NAMING for client, you suggest this:
        POST /v1/imagesync          --> create job
        GET /v1/imagesync/{job_id}  --> retrieve job


   In backend server code:             python side : unique name for each processing endpoint.
   ...
   In response JSON, dod you add API version ?
   ...
   2) when you have task processing.....
        manage queue inside API server : NO ?
             Send to input queue job description and jobid
...
def run():
    ...
    ##     2) ---> FastAPI (application layer). : app Object : does all entrypoint
...
def reload_config():
    ...
    print("Reloading config from {}".format(CONFIG_FILE_PATH))
