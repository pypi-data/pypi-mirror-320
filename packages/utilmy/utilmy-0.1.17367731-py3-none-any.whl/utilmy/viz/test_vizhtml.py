...
def test_plot_network_highcharts():
    data = [['A', 'B'], ['A', 'C'], ['A', 'D'], ['A', 'E'], ['A', 'F'], ['A', 'G'],]
    
    # Create the pandas DataFrame
    df = pd.DataFrame(data, columns=['from', 'to'])

    doc = vi.htmlDoc(title='Test network', dir_out="", cfg={}, css_name= "a4")

    doc.h1("Test network")
    doc.plot_network_highcharts(df, col=['from', 'to'], title='test_network', width=1, marker_fill_color='#FFFFFF', marker_line_width=2, marker_line_color='#A53E32', link_width=2, link_color='#A53E32', link_length=80, link_dash_style='dash')
    doc.plot_network_highcharts(df, col=['from', 'to'], title='test_network', width=1, marker_fill_color='#FEFEFE', marker_line_width=3, marker_line_color='#F5F5F5', link_width=4, link_color='#BBBBBB', link_length=120, link_dash_style='line')

    doc.save('test_network.html')
    doc.open_browser()

def test_plot_parallel_highcharts():
    ...
    doc = vi.htmlDoc(title='Test parallel', dir_out="", cfg={}, css_name= "a4")

    doc.h1("Test parallel")
    doc.plot_parallel_highcharts(df, col=['A', 'B'], title='test_parallel', width=2, marker_enabled=False, marker_states_hover_enabled=False,
                           marker_states_hover_halo_size=1, colors=['#9EDDFF'])

    doc.save('test_parallel.html')
    doc.open_browser()
