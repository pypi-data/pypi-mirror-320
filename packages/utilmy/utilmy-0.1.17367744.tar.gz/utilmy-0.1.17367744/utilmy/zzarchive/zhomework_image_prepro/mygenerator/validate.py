...
def image_padding_validate(final_image, min_padding, max_padding):
    ...
    if sum([t >= min_padding and t <= max_padding for t in pad_list]) != len(pad_list) or len(pad_list)==0 :
...
def run_image_padding_validate(
    ...
    **kwargs,
    ...
    for fi in flist:
        ...
        if sum([t >= min_spacing and t <= max_spacing for t in pad_list]) != len(pad_list) or \
           len(pad_list)==0:
