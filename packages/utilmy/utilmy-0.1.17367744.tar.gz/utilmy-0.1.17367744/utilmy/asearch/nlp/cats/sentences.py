...
def dataset_fake(name='AllNLI.tsv.gz', dirdata:str='', fname:str='data_fake.parquet', nsample=10):        
    ...
    # Read AllNLI.tsv.gz file and create training dataset
...
def dataset_fake2(dirdata=''):
    # This function load fake dataset if it's already existed otherwise downloads it first.
    # then Preprocess data for MultpleNegativesRanking loss function and return it as dataloader
    ...
    with gzip.open(nli_dataset_path, 'rt', encoding='utf8') as fIn:
        ...
        for row in reader:
            if row['split'] == 'train':
                ...
                df.append([sent2, sent1, row['label']])  #Also add opposite
...
def model_encode(model = "model name or path or object", dirdata:Dataframe_str="data/*.parquet", 
    ...
    Doc::
        ...
        batch_size       : batch size used for computation
        ...
        convert_to_numpy : If true,                    output is a list of numpy vectors. Else,                                                               it is a list of pytorch tensors.
        convert_to_tensor: If true,                    you get one large tensor as return. Overwrites any setting from convert_to_numpy
        device           : Which torch.device to use for computation
...
def model_setup_compute(model, use_gpu=0, ngpu=1, ncpu=1, cc:Dict_none=None):
    ...
    Doc::
        ...
        # Tell pytorch to run this model on multiple GPUs if available otherwise use all CPUs.
        Args:
            model: _description_
...
def model_load(path_or_name_or_object):
    #### Reload model or return model itself
