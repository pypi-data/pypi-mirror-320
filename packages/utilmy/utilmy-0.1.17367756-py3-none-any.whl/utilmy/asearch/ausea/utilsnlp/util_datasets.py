...
def hfd_data_to_disk(url="https://huggingface.co/datasets/Zihan1004/FNSPID"  , 
                      dirout="./ztmp/hf_data/", batch_size=5000, split='train')
    ...
    output_file = dirout + f"/{name2}/{split}/{split}2.parquet" # "FNSPID_dataset.parquet"
    print(f"Saving to {output_file}...")
    os_makedirs(output_file)


    train_dataset = dataset[split]
    # Convert the dataset to an Arrow Table
    table = pa.Table.from_batches(train_dataset.iter(batch_size=5000))
    ...
    if os.path.exists(output_file):
        ...
        print(f"Error: Failed to save the dataset to {output_file}")
...
def hfd_data_to_disk(url="https://huggingface.co/datasets/Zihan1004/FNSPID"  , 
    ...
    name = url.split("datasets/")[-1]
    name2 = name.replace("/","_")
    output_file = dirout + f"/{name2}/{split}/{split}.parquet"  # "FNSPID_dataset.parquet"
    os_makedirs(output_file)

    print("Preparing to stream dataset...")
    dataset = load_dataset(name, streaming=True)

    # Step 3: Prepare to save the dataset in Parquet format
    schema = None
    writer = None

    # Step 4: Iterate through the dataset and save it in chunks
    ...
    for i, example in enumerate(dataset[split]):
        ...
        if len(current_chunk) == chunk_size:
            ...
            if schema is None:
                # Initialize the schema and writer with the first chunk
                schema = table.schema
