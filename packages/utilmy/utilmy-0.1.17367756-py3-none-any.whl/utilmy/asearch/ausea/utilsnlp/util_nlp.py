...
def pd_detect_error_title_scrape(dftxt, col='title2', colnew=None, remove=1):
  """ Detect download error
  ...
  def iserror(x):
     ...
     # if len(x.split(" ")) <= 3: return True

     if len(x) < 20: 
           if ' nan ' in x : return True
           if ' na ' in x :  return True               
           ### Title does not a-z characters
           if  not bool(re.search(r'[a-z]', x)) : return True

     #### Start
     ll = ['404', '410', '406', '502', 'error ', 'human ver', 'not found' ]
     for w in ll:
        if x.startswith(w): return True

     #### Middle string
     ll = [ 'access denied', 'error:', 'page not found', 'human ver', 'just a moment' ]
     for w in ll:
        if w in x[:100] : return True

     #### Middle string
     if len(x)< 40:
         ll = [  'error', ]
         for w in ll:
            if w in x : return True
     return False 

  if remove == 1:
      dftxt = dftxt[ -dftxt[col].apply(lambda x:  iserror(x) )]
      return dftxt
  else:
      colnew = col + "_err" if colnew is  None else colnew
      dftxt[colnew] = dftxt[col].apply(lambda x:  1 if iserror(x) else 0 )
      log('added:', colnew)
      return dftxt
...
def pd_detect_error_text_scrape(dftxt, col='text2', colnew=None, remove=1):
  """ Detect download error
...
def hash_textid_simhash_scrap(xstr: str, sep=" "):
    """
        Generates a fingerprint from a sequence of hashes of the input string
        - Set the PYTHONHASHSEED value to get consistent results across runs
           Refer: https://docs.python.org/3/using/cmdline.html#envvar-PYTHONHASHSEED

        Results from running test_simhashes. PYTHONHASHSEED=42 was used to run the tests.
    ...
    return fingerprint(list(map(hash, xstr.split(sep))))
