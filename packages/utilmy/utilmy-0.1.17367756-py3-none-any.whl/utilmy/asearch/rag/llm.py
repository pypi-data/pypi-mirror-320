""" LLM tooling
   ...
   os.environ['OPENAI_KEY'] = "sk-proj-5dxaa_WWY-cA"
...
if "####### Import #############################################################":
    from typing import Type, get_origin, get_args, List, Literal,  Union
    try:
...
def test3():
    """
        Use tiktoken to find token IDs: tiktoken.encoding_for_model("gpt-4").encode("word")
    ...
    # Create query engine with logit bias
    client = OpenAI()
    query_engine = index.as_query_engine(
        llm_kwargs={
            "logit_bias": {
                # Increase probability of technical terms
                9642: 1,  # Token for "technical"
                2822: 1,  # Token for "specific"
                # Decrease probability of generic terms
                100257: -100  # Token for generic responses
            }
        }
    )

    # Query with biased responses
...
"""
    ...
    def zzz_llm_get_with_schema(row=None, prompt_request: promptRequest=None):
    ""
    ...
    # print(dict(payload))
    dd = {}
    msg = payload.raw_response.content[0].text
    # TODO: look for better workaround in gpt_json
    msg = llm_cleanup_gpt_jsonformat(msg=msg)
    dd["response"] = json.loads(msg)
