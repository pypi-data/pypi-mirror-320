...
https://github.com/arita37/Search-o1


cd asearch
alias pyqa="python3 -u report/PaperQA.py "
...
def run_qa(source: Any, question: str):
    """
         ...
         pyqa run_qa --source "ztmp/df_LZ_merge_90k.parquet" --question "summerize the document"
...
def convert_pdf_to_markdown(dirin="ztmp/mypdf/**/*.*", dirout="ztmp/docs_rag/mdown/", tag="", format1='parquet'):
    """
    ...
    md = MarkItDown(llm_client=client, llm_model="gpt-4o-2024-11-20")
    supported_ext = ('.pptx', '.docx', '.pdf', '.jpg', '.jpeg', '.png')
    ...
    if format1 == 'parquet':  
        ...
        for fi, txt in df:
            fi1 = fi.split("/")[-1]
            fi1 = fi.split(".")[0] +".md"
            with open(dirout1 +"/"+ fi1, mode='w') as fp:
                fp.write(txt)
...
class test_storm:
    """To be compatible with STORM, the collection payload/metadata should have the following fields:
     ...
     pyqa test_storm run --topic "theory of evolution" -- max_token 1000
...
##groq client
def LLMclient(name="groq"):
    if name == "groq":
       return AsyncGroq(api_key=os.environ['GROQ_KEY'])
...
##################################################################################
################################# Sample output ###################################
command: alias pyqa="python3 -u rag/PaperQA.py "
          "pyqa create_indexes"
output: 
...
cmd: "pyqa test_storm run --topic "theory of evolution"
