...
"""
2 ways :
     1) Add in middleware: Same for all endpoints

    2) Custom ways : just add Try .... except 
          and return specific JSON ERROR message
      ...
      Why ?
        ...
        if middleware is buggy (for some reasons). --> does not impact error return
         ...
         if somebody adds a new middleware stuff (XYZ) ---> it can broke return error message 

       it will work at 99.99% time even if we change middleware,......  
       very independant code.


    How do you HTTP error handling ?
       1) Base is to use middleware : middleware code is copy/paste from another libraries

          2) if this endpoint has custom processing --> specific error handling...

Common error HTTP status codes include:
  400 Bad Request - This means that client-side input fails validation.     
  401 Unauthorized - This means user isn't not authorized to access a resource. It usually returns when user isn't authenticated.
     no need to log, to prevent too much loggging in database...

  500 Internal server error - This is a generic server error. It probably shouldn't be thrown explicitly.
     Add internal logg for check

420:  custom based on App logic.
403 Forbidden.  - This means user is authenticated, but it's not allowed to access a resource.
404 Not Found   - This indicates that a resource is not found.
502 Bad Gateway - This indicates an invalid response from an upstream server.
503 Service Unavailable - This indicates that something unexpected happened on server side (It can be anything like server overload, some parts of system failed, etc.).
