...
echo  -e "### Start  $(date_now)  \n" 2>&1 | tee  "${LOGFILE}"
  ...
  while true; do 
         ...
         ########################################################
         echo "######bash News  Extra"
         pyfet  run_newstext --cfg  $cconfig &



         ########################################################
         # echo "###### Extract URL List:"
         #  pyfet run_urlist        --cfg $cconfig


         echo "######bash  Extract Text from URL:"
            pyfet run_extract_all   --cfg $cconfig  --past_days 10


         ########################################################
         #### Need PR website
         #echo "######bash PR website"
            # pyfet  run_urlist_prnews --cfg  $cconfig &&  

         #   pyfet  run_extract_prnews --cfg  $cconfig  
