...
class SignalAnalysis:
    ...
    def create_duration_column_if_not_exists(self) -> pl.DataFrame:
        ...
        if "duration" not in self.df.columns:
            # Create a duration column based on the diff of the time column
            
            # Sort the DataFrame by 'channel' and 'datetime'
            df_sorted = df.sort(by=["channel", "datetime"])

            # Group by 'channel' and calculate the difference within each group
            df_with_duration = df_sorted.with_columns(
                (pl.col("datetime").diff().over("channel")).alias("duration")
            )

            # If needed, fill the null values (for the last row in each channel) with a default value, e.g., 0
