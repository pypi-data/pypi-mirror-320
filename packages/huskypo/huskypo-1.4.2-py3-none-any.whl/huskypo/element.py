...
class Element:
    ...
    __CACHES = ['_present_cache', '_visible_cache', '_clickable_cache', '_select']
    ...
    def __get__(self, instance: P, owner: Type[P] | None = None) -> Self:
        ...
        if self._page != instance:
            self._page = instance
            if self.cache:  # avoid InvalidSessionIdException
                self.__del_caches()
        return self
    ...
    def dynamic(
        ...
        *,
        ...
        self._remark = remark
        self._cache = cache
        if self.cache:  # dynamic element should clear caches.
            self.__del_caches()
        return self

    def __del_caches(self) -> None:
        for cache in Element.__CACHES:
            if hasattr(self, cache):
                delattr(self, cache)

    def _if_relocate_directly(self) -> None:
        """
