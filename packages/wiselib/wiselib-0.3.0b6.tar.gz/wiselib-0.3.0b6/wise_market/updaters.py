...
from wise.station.updater import ModelUpdater
from wise.utils.numbers import to_decimal
...
class ExchangeMarketPairBindingUpdater(ModelUpdater[ExchangeMarketPairBinding]):
    ...
    def _get_symbol_network_decimals_and_step_size(
        cls, network: Network, symbol: Symbol
    ) -> Tuple[int, Decimal]:
...
class NetworkSymbolBindingUpdater(ModelUpdater[NetworkSymbolBinding]):
    ...
    def update(self) -> NetworkSymbolBinding:
        try:
            network = Network.objects.get(slug=self.network_slug)
            symbol = Symbol.objects.get(slug=self.symbol_slug)
        except (Network.DoesNotExist, Symbol.DoesNotExist) as e:
            logger.exception(f"cannot match network and symbol: {e}")
            raise e

        defaults: dict = {"type": self.symbol_network_type}
        if self.decimals is not None:
            defaults["decimals"] = self.decimals
            defaults["step_size"] = Decimal(10**-self.decimals)
