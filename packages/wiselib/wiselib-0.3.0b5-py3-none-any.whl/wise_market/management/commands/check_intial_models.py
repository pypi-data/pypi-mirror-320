...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for model_name in self.model_names:
            wise_model = state.models[("wise_market", model_name)]
            model = state.models[("market", model_name)]
            comparison = self.compare_model_states(wise_model, model)
            diffs = []
            for x in ["added", "removed"]:
                if comparison["fields"][x]:
                    diffs.append(f"fields {comparison['fields'][x]} are removed.")
