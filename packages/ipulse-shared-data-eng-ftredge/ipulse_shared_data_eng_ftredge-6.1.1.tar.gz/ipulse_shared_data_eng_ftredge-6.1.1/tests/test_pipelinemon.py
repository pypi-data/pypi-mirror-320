...
from ipulse_shared_base_ftredge import LogLevel, Action, Resource, DataResource,AbstractResource, ProgressStatus, Alert, ReviewStatus, Unit, StructLog
...
class TestPipelinemon(unittest.TestCase):
    ...
    def test_count_count_logs_for_events_containing(self):
        ...
        logs = [
            ...
            StructLog(level=LogLevel.INFO, resource=AbstractResource.PIPELINE_ITERATION, progress_status=ProgressStatus.CANCELLED),
        ...
        cancelled_count = self.pipelinemon.count_logs_for_events_containing(
            progress_statuses=ProgressStatus.CANCELLED,
            resources=AbstractResource.PIPELINE_ITERATION
        )
        self.assertEqual(cancelled_count, 1, "Should count exactly one CANCELLED PIPELINE_ITERATION log.")
        ...
        # Test counting with ISSUE_STATUSES frozenset
        issue_count = self.pipelinemon.count_logs_for_events_containing(
            progress_statuses=ProgressStatus.issue_statuses(),
            resources=AbstractResource.PIPELINE_ITERATION
        )
        self.assertEqual(issue_count, 2, "Should count 2 logs with issue statuses (FAILED).")
        ...
        self.assertEqual(pending_count, 2, "Should count 2 logs with pending statuses (IN_PROGRESS, PAUSED).")
        ...
        self.assertEqual(skipped_count, 2, "Should count 2 logs with skipped statuses (CANCELLED, INTENTIONALLY_SKIPPED).")
    ...
    def test_get_breakdown_by_event(self):
        ...
        self.assertIn("('INFO', 'READ')", breakdown_str, "Breakdown should include the first log event.")
        self.assertIn("('WARNING', 'PERSIST_WRITE')", breakdown_str, "Breakdown should include the second log event.")
    ...
    def test_generate_execution_summary(self):
        ...
        self.assertIn("Done pipeline_subject(s): [1/3] stocks(s)", summary, "Summary should reference  1/3 done subject.")
        self.assertIn("Issue/Error  pipeline_subject(s): [1/3] stocks(s)", summary, "Should show 1/3 Issue subject in the summary.")
        self.assertIn("Pending  pipeline_subject(s): [1/3] stocks(s)", summary, "Should reflect that 1/3 subject is still pending.")
        self.assertIn("Warnings:", summary, "Log level summary should include warnings.")
        self.assertIn("Errors:", summary, "Log level summary should include errors.")
