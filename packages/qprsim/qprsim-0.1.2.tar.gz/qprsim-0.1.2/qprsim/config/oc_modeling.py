...
from qprsim.model.oc_sim_graph import OCSplit, OCJoin, ActivityNode, XorSplit, XorJoin, \
    AndSplit, AndJoin, LinearNode, SplittingNode, TauNode, WithModel, OCSimulationGraph, BirthplaceNode, GraveyardNode, OCTransformerNode
...
class GraphBuilder:
    ...
    def add_partnered_nodes(self, label, split_node, join_node, connect_to_arrival=False, connect_to_terminal=False):
    ...
    def add_split(self, label=None, connect_to_arrival=False, connect_to_terminal=False) -> str:
        assert label not in self.aliases
        if label is None:
            label = f'and{self.concurrency_id}'
            self.concurrency_id += 1
