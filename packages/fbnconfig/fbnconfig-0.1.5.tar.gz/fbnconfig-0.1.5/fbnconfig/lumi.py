...
class ViewResource(BaseModel, Resource):
    ...
    def template(self, desired):
        ...
        params = [
            ...
            f"{p.name},{p.type.value},{lumi_fmt(p.value)},{lumi_fmt(p.is_mandatory)},{p.tooltip}"
            if p.type == ParameterType.Table
            else f"{p.name},{p.type.value},{lumi_fmt(p.value)},{lumi_fmt(p.set_as_default_value)},{p.tooltip}"  # noqa: E501
