...
class ResponseAgent:
    ...
    def ResponseAgent_message_completions(prompt, key_api, Debug=True, lang="pt" ,sistema = "", json_format = True, store=True):
        """
        Envia uma mensagem para o modelo de chat da OpenAI e retorna a resposta.

        Parâmetros:
        - prompt (str): O texto ao qual o assistente responderá.
        ...
        mensagem = []
        if sistema != "":
            log_message(f'⚙️ Adicionando instrução de sistema: {sistema}', 
                        f'⚙️ Adding system instruction: {sistema}', 'cyan')
            ...
            for message in messages:
                for mensagem_contexto in message.content:
                    ...
                    if DebugTokens:
                        ...
                        if lang == "en":
                            log_message(f'📜 Tokens consumed in the document: {run_status.usage.total_tokens} 💸${valor_min:.4f} and 💸${valor_max:.4f}',
                                        f'📜 Tokens consumed in the document: {run_status.usage.total_tokens} 💸${valor_min:.4f} and 💸${valor_max:.4f}', 'yellow', bold=True)
                        elif lang == "pt":
                            log_message(f'📜 Tokens Consumidos no documento: {run_status.usage.total_tokens} 💸${valor_min:.4f} e 💸${valor_max:.4f}',
                                        f'📜 Tokens Consumidos no documento: {run_status.usage.total_tokens} 💸${valor_min:.4f} e 💸${valor_max:.4f}', 'yellow', bold=True)
